...
static inline void get_i4x4_horizontal(imgpel **cur_pred, imgpel *PredPel)
{
  ...
  memset(*cur_pred++, (imgpel) (&P_I)[0], BLOCK_SIZE * sizeof(imgpel));
  memset(*cur_pred++, (imgpel) (&P_I)[1], BLOCK_SIZE * sizeof(imgpel));
  memset(*cur_pred++, (imgpel) (&P_I)[2], BLOCK_SIZE * sizeof(imgpel));
  memset(*cur_pred  , (imgpel) (&P_I)[3], BLOCK_SIZE * sizeof(imgpel));
...
static inline void get_i4x4_dc(imgpel **cur_pred, imgpel *PredPel, int left_available, int up_available)
{
  ...
  memset(cur_pred[0], (imgpel) s0, BLOCK_SIZE * sizeof(imgpel));
  memset(cur_pred[1], (imgpel) s0, BLOCK_SIZE * sizeof(imgpel));
  memset(cur_pred[2], (imgpel) s0, BLOCK_SIZE * sizeof(imgpel));
  memset(cur_pred[3], (imgpel) s0, BLOCK_SIZE * sizeof(imgpel));
