					...
					)
{
	...
	{
		print_dec_rpu_test_usage_info(pi8_arguments[0]);		
		exit(-1);
	}
	/* The RPU process format is a mandatory argument. Hence, the number of options is at least 5*/
	if((!strcmp(pi8_arguments[i32_i], HELP_SWITCH)) || (i32_num_arguments < 3))
	{
		print_dec_rpu_test_usage_info(pi8_arguments[0]);		
		exit(-1);
	} /* HELP_SWITCH */
	...
	*(pi8_yuv_file_names+RPU_BL_OUTFILE) = (char *) malloc(sizeof(char) * (u32length+1));
	strcpy(*(pi8_yuv_file_names+RPU_BL_OUTFILE) , pi8_arguments[2]);
 ...
 *
 * \brief Prints the usage info for running the c3d_decoder_rpu_test application.
 *
