...
int main(int argc, char *argv[])
{
	...
	ImageData *p_actual_el = NULL;	
    ImageData *p_predenhlayer = NULL;

    /* Output Images */
	ImageData *p_view0 = NULL;
	ImageData *p_view1 = NULL;
	...
	ImageData p_enhlayer_dup ;
	ImageData p_predenhlayer_dup;
	ImageData p_view0_dup ;
	ImageData p_view1_dup ;
	

	
	
    uint16_t            isInterlaced                          = 0;
    uint16_t            deMuxMode                             = DEFAULT_DEMUX_MODE; 
	...
	char				*yuv_file_names[NUM_YUV_READ_FILES + NUM_YUV_WRITE_FILES] = {NULL,NULL,NULL,NULL,NULL}; /*base layer,enhancement layer, predicted enhancement layer , view0 , view1"*/
	FILE				*pf_yuv_files[NUM_YUV_READ_FILES + NUM_YUV_WRITE_FILES] ={NULL,NULL,NULL,NULL,NULL} ;
	...
	i32_status = read_input_params_recon_test(argc,
									argv,
									p_init_params,
									&isInterlaced,
                                    &deMuxMode,
 ...
 /* Finalize the maximum number of frames to process based on file size)*/
    ...
    u16_num_iterations = MIN_VAL(u16_num_iterations,(GetNumberYUVFrames(pf_yuv_files[EL_INFILE],u16_actual_size_x,u16_actual_size_y,0)));
    u16_num_iterations = MIN_VAL(u16_num_iterations,(GetNumberYUVFrames(pf_yuv_files[RPU_BL_INFILE],u16_actual_size_x,u16_actual_size_y,0)));
	...
	i32_status = allocate_image(&p_actual_el,
							&ref_image,					
							p_init_params->u8_packed_UV);
	...
	i32_status = allocate_image(&p_view0,
							&ref_image,							
							p_init_params->u8_packed_UV);
	i32_status = allocate_image(&p_view1,
							&ref_image,						
							p_init_params->u8_packed_UV);							  	
	...
	{
		printf("\nRunning MFC Recon Test App : %d out of %d\n", u32_k + 1, u16_num_iterations);
		...
		{
			printf("\nEnd of YUV Data in BL File.");
			printf("\nTotal Frames Completed: %d\n\n", u32_k + 1);
			break;
		}

		/* Read a Frame of EL */
		i32_status = read_yuv_image(p_actual_el , p_tempImage1,pf_yuv_files[EL_INFILE],p_init_params->u8_packed_UV);
		
		if(i32_status != SUCCESS)
		{
			printf("\nEnd of YUV Data in EL File. \n");
			printf("\nTotal Iterations: %d\n", u32_k + 1);
			break;
		}


	    /* Read a Frame of RPU_BL */
		i32_status = read_yuv_image(p_predenhlayer , p_tempImage1,pf_yuv_files[RPU_BL_INFILE],p_init_params->u8_packed_UV);
		
		if(i32_status != SUCCESS)
		{
			printf("\nEnd of YUV Data in RPU_BL File. \n");
			printf("\nTotal Iterations: %d\n", u32_k + 1);
			break;
		}
		...
		for(i32_loop=isInterlaced;i32_loop<2*isInterlaced	 + 1;i32_loop++)
		{
			...
			change_image_format(&p_view0_dup		,p_view0		,(PICTURE_TYPE)i32_loop);
			change_image_format(&p_view1_dup		,p_view1		,(PICTURE_TYPE)i32_loop);
			change_image_format(&p_baselayer_dup	,p_actual_bl	,(PICTURE_TYPE)i32_loop);
			change_image_format(&p_enhlayer_dup	    ,p_actual_el	,(PICTURE_TYPE)i32_loop);
			change_image_format(&p_predenhlayer_dup	,p_predenhlayer	,(PICTURE_TYPE)i32_loop);
			
            if(deMuxMode==1)
            {
                    /* OM Demux */
                    i32_status = process3DDeMux_OM(pv_dec_layer_ctxt,
                                     &p_view0_dup,
                                     &p_view1_dup,
                                     &p_baselayer_dup,
                                     &p_enhlayer_dup,
            ...
            {
                    /* FC Demux */
                    i32_status = process3DDeMux_FC(pv_dec_layer_ctxt,
                                     &p_view0_dup,
                                     &p_view1_dup,
                                     &p_baselayer_dup                    
                                    );                
			...
			/* Pad output Images*/
			pad_ImageData(p_view0,u16_actual_size_x,u16_actual_size_y);
			pad_ImageData(p_view1,u16_actual_size_x,u16_actual_size_y);
            
            /* write output yuv files */
			write_yuv_image(p_view0 ,p_tempImage1, pf_yuv_files[VIEW0_OUTFILE],p_init_params->u8_packed_UV);
			write_yuv_image(p_view1 ,p_tempImage1, pf_yuv_files[VIEW1_OUTFILE],p_init_params->u8_packed_UV);
	...
	free_image_mem(p_actual_el);
    free_image_mem(p_predenhlayer);	
	free_image_mem(p_view0);
	free_image_mem(p_view1);	
	...
	free(yuv_file_names[EL_INFILE]);
	free(yuv_file_names[RPU_BL_INFILE]);
	free(yuv_file_names[VIEW0_OUTFILE]);
	free(yuv_file_names[VIEW1_OUTFILE]);
