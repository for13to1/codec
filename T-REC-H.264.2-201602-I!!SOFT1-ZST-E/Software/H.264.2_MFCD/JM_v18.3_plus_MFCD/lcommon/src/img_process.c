...
void process_image( VideoParameters *p_Vid, InputParameters *p_Inp )
{
 ...
 case MFC_PROCESS_INPUT:
     /* Create Base Layer and Enhancement Layer Views from
     Left and Right Views */

     if(!p_Inp->PicInterlace)
     {
         /* Frame Processing */       
        /* Setup BL Image Format as Frame */
        reset_mfc_image_format(&p_Vid->imgData,0);          
        
        if(0 == p_Vid->view_id)
        {
           /* Setup View0 Image Format as Frame */
           reset_mfc_image_format(&p_Vid->imgData0,0);     
            
           /* Setup View1 Image Format as Frame */
           reset_mfc_image_format(&p_Vid->imgData1,0);     

           #if MFC_PROFILING
           gettime (&(p_Vid->mfc_start_time));              
           #endif    
            
           /* Create Base Layer Frame*/
           process3DMuxWrapper(p_Vid->imgData.pv_mfc_img_data,
           ...
           p_Vid->mfc_BL_Muxing_time += timediff(&(p_Vid->mfc_start_time), &(p_Vid->mfc_end_time));    
           #endif
        }
        else
        {
           /* Enhancemnet layer was already created in process_picture_in_dpb() while handling BL, just copy the EL*/
           reset_mfc_image_format(&p_Vid->imgData2,0);     
           copy_mfc_ImageData(p_Vid->imgData.pv_mfc_img_data,p_Vid->imgData2.pv_mfc_img_data);
        }
     }
     else
     {
        ...
        for(i_loop=1;i_loop<3;i_loop++)
        {
          ...
          /* Setup BL Image Format as Field */   
          reset_mfc_image_format(&p_Vid->imgData,i_loop);     
     
          if(0 == p_Vid->view_id)
          {   
             /* Setup View0 Image Format as Field */
             reset_mfc_image_format(&p_Vid->imgData0,i_loop);     

             /* Setup View1 Image Format as Field */
             reset_mfc_image_format(&p_Vid->imgData1,i_loop);     
            
             #if MFC_PROFILING
             gettime (&(p_Vid->mfc_start_time));              
             #endif    

             /* Create Base Layer  Field*/
             process3DMuxWrapper(p_Vid->imgData.pv_mfc_img_data,
                            p_Vid->imgData0.pv_mfc_img_data,
                            p_Vid->imgData1.pv_mfc_img_data,                    
                            NULL,
          ...
          {
            reset_mfc_image_format(&p_Vid->imgData2,i_loop);   
            copy_mfc_ImageData(p_Vid->imgData.pv_mfc_img_data,p_Vid->imgData2.pv_mfc_img_data);
          }
        }
    ...
    /*Output Debug YUV Files */
    if(p_Inp->EnableDbgYUVFiles)
    {
        if(0==p_Vid->view_id)
        {
        write_yuv_file_wrapper( p_Inp->fp_mux_baselayer,
                                p_Vid->imgData.pv_mfc_img_data);
        
        }
        if(1==p_Vid->view_id)
        {
        write_yuv_file_wrapper( p_Inp->fp_mux_enhlayer,
                                p_Vid->imgData.pv_mfc_img_data);
