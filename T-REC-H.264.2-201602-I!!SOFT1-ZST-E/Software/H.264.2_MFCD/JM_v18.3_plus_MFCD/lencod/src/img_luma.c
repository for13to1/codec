 ...
 ************************************************************************
 * \brief
 *    extend the boundary of up-sampled depth map
 ************************************************************************
 */
void pad_depth(imgpel** img_Y, int width, int height, int stride,int pad_x, int pad_y)
{
	
		int j;
		imgpel* img_buf=*img_Y;
		imgpel *pLine0 = img_buf - pad_x, *pLine;
#if (IMGTYPE==0)
		int pad_width = pad_x + width;
		fast_memset(img_buf - pad_x, *img_buf, pad_x * sizeof(imgpel));
		fast_memset(img_buf+ width, *(img_buf + width - 1), pad_x * sizeof(imgpel));

		pLine = pLine0 - pad_y * stride;
		for(j = -pad_y; j < 0; j++)
		{
			fast_memcpy(pLine, pLine0, stride * sizeof(imgpel));
			pLine += stride;
		}
		...
		{
			pLine += stride;
			fast_memset(pLine, *(pLine + pad_x), pad_x * sizeof(imgpel));
			fast_memset(pLine + pad_width, *(pLine + pad_width - 1), pad_x * sizeof(imgpel));
		}

		pLine0 = pLine + stride;

		for(j = height; j < height + pad_y; j++)
		{
			fast_memcpy(pLine0,  pLine, stride * sizeof(imgpel));
			pLine0 += stride;
		}
#else
		int i;
		for(i=-pad_x; i<0; i++)
			img_buf[i] = *img_buf;
		for(i=0; i<pad_x; i++)
			img_buf[i+width] = *(img_buf+width-1);

		for(j=-pad_y; j<0; j++)
			memcpy(pLine0+j*stride, pLine0, stride*sizeof(imgpel));
		for(j=1; j<height; j++)
		{
			pLine = pLine0 + j*stride;
			for(i=0; i<pad_x; i++)
				pLine[i] = pLine[pad_x];
			pLine += pad_x+width-1;
			for(i=1; i<pad_x+1; i++)
		...
		pLine = pLine0 + (height-1)*stride;
		for(j=height; j<height+pad_y; j++)
			memcpy(pLine0+j*stride,  pLine, stride*sizeof(imgpel));
#endif
}
#endif
/*!
