...
static void alloc_video_params( VideoParameters **p_Vid)
{
  ...
  #if DISPLAY_LAYERLEVEL_BITUSAGE
  {
    ...
    {      
       if ((((*p_Vid)->p_layer_Stats[i]) = (StatParameters *) calloc(1, sizeof(StatParameters)))==NULL) 
            no_mem_exit("alloc_video_params: p_Stats");    
    }
  } 
  #endif
...
static void check_coding_structure(VideoParameters* p_Vid)
{
	...
	if((!valid_struct)&&(p_Inp->num_of_views!=1))
	{
	...
	//if((p_Inp->HierarchicalCoding)&&(p_Inp->NumberBFrames==0)&&(p_Inp->num_of_views>1))
	//{
	//	snprintf(errortext,120,"This Hierarchical coding structure(HierarchicalCoding:%d NumberBFrames:%d) for MVC is not supported by this software now \n",
	//		p_Inp->HierarchicalCoding,p_Inp->NumberBFrames);
	//	error(errortext,400);
	//}
}
#endif
...
int main(int argc, char **argv)
{
  ...
  alloc_encoder(&p_Enc);
#if (MFC_DEPTH_ENC)
  Configure (p_Enc->p_Vid, p_Enc->p_Inp, argc, argv,0);	//<!texture
//#if (G0239_HLS_FIX)
  if (p_Enc->p_Inp->ThreeDVCoding)
  {
    p_Enc->p_Vid->p_Inp=p_Enc->p_Inp;
    p_Enc->p_Vid->p_DualInp=p_Enc->p_InpDepth;
    p_Enc->p_Vid->p_DualVid=p_Enc->p_VidDepth;
    p_Enc->p_VidDepth->p_DualInp=p_Enc->p_Inp;
    p_Enc->p_VidDepth->p_DualVid=p_Enc->p_Vid;
...
#if (MFC_DEPTH_ENC)
	JointTextureDepthEnable=p_Enc->p_Inp->ThreeDVCoding;
	if(JointTextureDepthEnable)
	{
		assert(p_Enc->p_InpDepth->num_of_views);
  ...
  // encode sequence
#if (MFC_DEPTH_ENC)
	encode_sequence(p_Enc->p_Vid,p_Enc->p_Inp,p_Enc->p_VidDepth,p_Enc->p_InpDepth);
#else
...
static void init_encoder(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  init_stats (p_Inp, p_Vid->p_Stats);
    ...
    {
    init_stats (p_Inp, p_Vid->p_layer_Stats[i]);    
    }
  #endif
  ...
  {
    p_Vid->proc_picture[0] = NULL;
    p_Vid->proc_picture[1] = NULL;
  }

  if(is_MFC3D_profile(p_Inp->ProfileIDC))
  {
      p_Inp->mfc_rpu_data.rpu_field_processing_flag = ( (!!p_Inp->PicInterlace) || (!!p_Inp->MbInterlace) ) ;
      
      /* Allocate memeory for MFC RPU Structure */  
      if((p_Vid->pv_mfc_rpu_data  = alloc_mfc_rpu_data()) == NULL)
      {
           error("\nMFC HIGH PROFILE : alloc_mfc_rpu_data() Failed",400);
      }

      /* Set MFC RPU based on JM Input*/  
      if(set_mfc_rpu_data(&p_Inp->mfc_rpu_data,p_Vid->pv_mfc_rpu_data) != 0)
      {
           error("\nMFC HIGH PROFILE : set_mfc_rpu_data() Failed",400);
      }
      
      /* Allocate and Set MFC Muxing Parameters based on JM Input */  
      if((p_Vid->pv_mfc_muxing_params      = allocate_and_set_muxing_params(&p_Inp->mfc_muxing_params)) == NULL)
      {
           error("\nMFC HIGH PROFILE : allocate_and_set_muxing_params() Failed",400);
      }
      
      /* Allocate Memory of MFC Encoder Layer Context */  
      if((p_Vid->pv_mfc_encoder_layer_context     = allocate_mfc_encoder_wrapper()) == NULL)
      {
           error("\nMFC HIGH PROFILE : allocate_mfc_encoder_wrapper() Failed",400);
      }
        
      /* Initialize MFC Encoder Layer Context */  
      initialize_mfc_encoder_wrapper(p_Vid->pv_mfc_encoder_layer_context,p_Vid->pv_mfc_rpu_data);

      if(p_Inp->EnableDbgYUVFiles)
      {
        char temp_fname[50];    
        sprintf(temp_fname,"%s_P420_%dx%d.yuv","BaseLayer",p_Vid->width,p_Vid->height);
        if(!(p_Inp->fp_mux_baselayer = fopen(temp_fname,"wb")))
        {
            printf("\nMFC HIGH PROFILE :Error Unable to open BL file for Writing\n");
        }

        sprintf(temp_fname,"%s_P420_%dx%d.yuv","EnhLayer",p_Vid->width,p_Vid->height);
        if(!(p_Inp->fp_mux_enhlayer = fopen(temp_fname,"wb")))
        {
            printf("\nMFC HIGH PROFILE :Error Unable to open EL file for Writing\n");
        }

        sprintf(temp_fname,"%s_P420_%dx%d.yuv","ReconBaseLayer",p_Vid->width,p_Vid->height);
        if(!(p_Inp->fp_recon_baselayer = fopen(temp_fname,"wb")))
        {
            printf("\nMFC HIGH PROFILE :Error Unable to open Recon BL file for Writing\n");
        }

        sprintf(temp_fname,"%s_P420_%dx%d.yuv","PredEnhLayer",p_Vid->width,p_Vid->height);
        if(!(p_Inp->fp_pred_enhlayer = fopen(temp_fname,"wb")))
        {
        ...
        p_Vid->mfc_BL_Muxing_time = 0;
        p_Vid->mfc_EL_Muxing_time = 0;
    #endif
  ...
  if ((org_depth_w & 0x0F) !=0)
    p_Vid->p_Inp->OriginalWidth =org_depth_w+16-(org_depth_w & 0x0F);

  if (p_Inp->PicInterlace || p_Inp->MbInterlace)
  {
    if ((org_depth_h & 0x1F) !=0)
      p_Vid->p_Inp->OriginalHeight =org_depth_h+32-(org_depth_h & 0x1F);
  }
  else
  {
    if ((org_depth_h & 0x0F) !=0)
      p_Vid->p_Inp->OriginalHeight =org_depth_h+16-(org_depth_h & 0x0F);
  }
...
#if MFC_JMCODE_FIX
       p_Vid->p_EncodePar[i]->last_ref_idc = 0;
#endif
    }
    p_Vid->frame_num = 0;
  }
 ...
 static void encode_sequence(VideoParameters *p_Vid, InputParameters *p_Inp,VideoParameters*p_VidDepth,InputParameters*p_InpDepth)
{
	...
	int NumOfViewsTexture=p_Inp->num_of_views;
	int NumofViewsDepth=p_Inp->ThreeDVCoding?p_InpDepth->num_of_views:0;
	int texture=1;
	int MMCOUpdate=0;
	int voidx;
    ...
    for(i=0;i<NumofViewsDepth+NumOfViewsTexture;++i)
		{
			...
			p_VidTmp->PicPos=texture?p_Vid->PicPos:p_VidDepth->PicPos;
			...
			set_dpb_layer_id(p_VidTmp, p_VidTmp->view_id);
			...
			if(voidx)
			{
				...
				p_VidTmp->BaseViewAverageFrameQP + p_InpTmp->View1QPOffset );
			}
			else
			...
			if ( !frame_coded )
			{
				p_VidTmp->frame_num = p_VidTmp->p_CurrEncodePar->frame_num = frame_num_bak;
				continue;
			}
			p_VidTmp->p_CurrEncodePar->last_ref_idc = p_VidTmp->last_ref_idc = p_VidTmp->nal_reference_idc ? 1 : 0;
    ...
    if (p_InpTmp->EnableOpenGOP && p_VidTmp->p_curr_frm_struct->random_access)
    {
      if (p_InpTmp->PicInterlace)
      {
        if (p_VidTmp->p_curr_frm_struct->p_top_fld_pic->p_Slice[0].type == I_SLICE && p_VidTmp->p_curr_frm_struct->random_access) //Currently encoder always codes top field as I
        {
          p_VidTmp->last_valid_reference = p_VidTmp->ThisPOC & (~( (signed int)1 ));
          //printf("last valid ref: %d", p_Vid->last_valid_reference);
        }
      }
      else if (p_VidTmp->type == I_SLICE)
      {
...
#if EOS_OUTPUT
	end_of_stream(p_Vid);
	end_of_stream(p_VidDepth);
#endif
	if(p_Inp->num_of_views == 2)
	{
		p_Inp->RCEnable=tmp_rate_control_enable;
	}
	if((p_Inp->ThreeDVCoding)&&(p_InpDepth->num_of_views == 2))
	{
...
#endif
{
  ...
  
  {
	  ...
	  int num_of_views=p_Inp->num_of_views;
	  ...
	  if(p_VidDepth&&p_InpDepth)
	  {
		  ...
		  num_of_views+=p_InpDepth->num_of_views;
  ...
  if(p_Inp->EnableDbgYUVFiles)
  {
    if(p_Inp->fp_mux_baselayer != NULL)    fclose(p_Inp->fp_mux_baselayer);
    if(p_Inp->fp_mux_enhlayer != NULL)    fclose(p_Inp->fp_mux_enhlayer);
    if(p_Inp->fp_recon_baselayer != NULL)    fclose(p_Inp->fp_recon_baselayer);
    if(p_Inp->fp_pred_enhlayer != NULL)    fclose(p_Inp->fp_pred_enhlayer);
  }
  ...
  flush_dpb(p_VidDepth->p_Dpb_layer[0], &p_InpDepth->output);
  flush_dpb(p_VidDepth->p_Dpb_layer[1], &p_InpDepth->output);
  CloseFiles(&p_InpDepth->input_file1);
  
  if (-1 != p_VidDepth->p_dec)
    close(p_VidDepth->p_dec);

#if (MVC_EXTENSION_ENABLE)
  if(p_VidDepth->num_of_layers==2)
    CloseFiles(&p_InpDepth->input_file2);
  if (-1 != p_VidDepth->p_dec2)
    close(p_VidDepth->p_dec2);
#endif
...
#ifdef _LEAKYBUCKET_
  free_pointer(p_VidDepth->Bit_Buffer);
#endif
  free_dpb(p_VidDepth->p_Dpb_layer[0]);
  free_dpb(p_VidDepth->p_Dpb_layer[1]);
...
static void free_img (VideoParameters *p_Vid, InputParameters *p_Inp)
{
 ...
 // Delete Frame memory 
...
#if DISPLAY_LAYERLEVEL_BITUSAGE
  {
    ...
    for(i  =0 ; i < MAX_NUM_DPB_LAYERS ; i++)
    {
    free_pointer (p_Vid->p_layer_Stats[i]);  
    }
  }
#endif
...
void free_depth_img()
{
	...
	if(p_Enc->p_VidDepth->p_Dpb_layer[0])
		free(p_Enc->p_VidDepth->p_Dpb_layer[0]);
	if(p_Enc->p_VidDepth->p_Dpb_layer[1])
		free(p_Enc->p_VidDepth->p_Dpb_layer[1]);
...
#if(MFC_ENC_3D_FCFR)
   imgData->pv_mfc_img_data        = allocate_mfc_image_format();    //Allocate memory to hold mfc_sdk ImageData format.
#endif
  return memory_size;
}
...
static int init_global_buffers(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  memory_size += init_orig_buffers(p_Vid, &p_Vid->imgData0);            //left view
#if (MFC_ENC_3D_FCFR)        
    memory_size += init_orig_buffers(p_Vid, &p_Vid->imgData1);            //right view
    memory_size += init_orig_buffers(p_Vid, &p_Vid->imgData2);            //Enhancement Layer when recon_bl is available
#endif
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag)
  {
    for(j=0; j<p_Vid->num_of_layers; j++)
    {
      ...
      if (cps->yuv_format != YUV400 || (p_Vid->force_yuv400 == 1))
      {
...
#if(MFC_ENC_3D_FCFR)
   free_mfc_format(imgData->pv_mfc_img_data);  
#endif
...
#if (MFC_ENC_3D_FCFR)        
    free_orig_planes(p_Vid, &p_Vid->imgData1);            //right view
    free_orig_planes(p_Vid, &p_Vid->imgData2);            //Enhancement Layer when recon_bl is available
#endif
...
#if(MFC_ENC_3D_FCFR)
  if(is_MFC3D_profile(p_Vid->p_Inp->ProfileIDC))
  {
      free_pointer(p_Vid->pv_mfc_rpu_data);
      free_pointer(p_Vid->pv_mfc_muxing_params);
      free_mfc_encoder_wrapper(p_Vid->pv_mfc_encoder_layer_context);
...
int init_process_image( VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  case 0:
    ...
    break;
#if(MFC_ENC_3D_FCFR)
    case MFC_PROCESS_INPUT:
    if( (p_Vid->num_of_layers == 2) && is_MFC3D_profile(p_Inp->ProfileIDC) )
    {
      memory_size += init_orig_buffers(p_Vid, &p_Vid->tempData3);//used for predicted EL
      memory_size += init_orig_buffers(p_Vid, &p_Vid->tempData4);//used for debugging
    }
    break;
#endif
...
void clear_process_image( VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  case 0:
    ...
    {
      free_orig_planes(p_Vid, &p_Vid->tempData3);
      free_orig_planes(p_Vid, &p_Vid->tempData4);
    }
    break;
#endif
