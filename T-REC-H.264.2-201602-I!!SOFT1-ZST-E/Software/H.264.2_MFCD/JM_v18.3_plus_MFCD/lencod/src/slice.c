...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_ENC)
			if((p_Inp->num_of_views>1)||((p_Inp->num_of_views==1)&&(p_Vid->p_DualVid)))
			{
				if(p_Vid->is_depth)
					nalu->nal_unit_type=NALU_TYPE_3DV_EXT;
				else
					nalu->nal_unit_type = (voidx==0) ? NALU_TYPE_SLICE : NALU_TYPE_SLC_EXT;
			}
			else
...
void init_slice (VideoParameters *p_Vid, Slice **currSlice, int start_mb_addr)
{
  ...
  *currSlice = currPic->slices[currPic->no_slices-1];
  ...
  else if (p_Vid->nal_reference_idc && p_Inp->PocMemoryManagement == 1)
  {
    ...
    else if (p_Vid->structure == TOP_FIELD && p_Dpb->ref_frames_in_buffer== active_sps->num_ref_frames && p_Dpb->layer_id == 0)
#else
...
#if (MFC_INTERLACE_POC_MANAGEMENT)
    else if (p_Vid->structure == BOTTOM_FIELD && p_Dpb->layer_id == 0)
#else
