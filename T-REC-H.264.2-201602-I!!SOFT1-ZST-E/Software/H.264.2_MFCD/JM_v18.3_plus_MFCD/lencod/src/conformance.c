...
void profile_check(InputParameters *p_Inp)
{
  if((p_Inp->ProfileIDC != BASELINE ) &&
     ...
     #if (MFC_ENC_3D_FCFR)
     (p_Inp->ProfileIDC != MFC3D_HIGH )         &&  // MFC3D_HIGH profile
    #endif
	#if (MFC_DEPTH_ENC)
	(p_Inp->ProfileIDC != ThreeDV_HIGH)         &&  // Multiview Depth High Profile 
	(p_Inp->ProfileIDC != MFC_DEPTH_HIGH)       &&  // MFC Depth High
	#endif
#endif
...
void level_check(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if(p_Vid->is_depth)
  {
	  if((p_Inp->ProfileIDC!=ThreeDV_HIGH)&&(p_Inp->ProfileIDC!=MFC_DEPTH_HIGH))
	  {
		  snprintf(errortext,ET_SIZE,"The ProfileIDC for depth shall be set to 138 or 135");
		  error(errortext,500);
	  }
  }
  else
  {
	  if((0==p_Inp->ThreeDVCoding))
	  {
		  if((p_Inp->ProfileIDC==ThreeDV_HIGH)||(p_Inp->ProfileIDC==MFC_DEPTH_HIGH) )
		  {
			  snprintf(errortext,ET_SIZE,"The profileIDC cannot be larger than 134 for Multi-view compatible");
			  error(errortext,500);
		  }
	  }
  }
...
{
#if MFC_MV_RANGE
  return ((mv->mv_x < p_Vid->MaxHmvR[4]) || (mv->mv_x > (p_Vid->MaxHmvR[5]-1) ) || (mv->mv_y < p_Vid->MaxVmvR[4]) || (mv->mv_y > (p_Vid->MaxVmvR[5]-1)));
#else
...
int InvalidMotionVectors(VideoParameters *p_Vid, Slice *currSlice, Block8x8Info* b8x8info, int mode)
{
  ...
  if (mode == P8x8)
  {
    for (cur_blk = 0; cur_blk < 4; cur_blk ++)
    {
      i = (cur_blk & 0x01) << 1;
      j = (cur_blk >> 1) << 1;
      b8x8_mode = b8x8info->best[mode][cur_blk].mode;

      //peng_debug
      /*if ( (p_Vid->ThisPOC == 32) && (p_Vid->current_mb_nr == 874) && (cur_blk==2) )
      {
        
        l0ref = (int) b8x8info->best[mode][cur_blk].ref[LIST_0];
        printf("POC=%d mb_nr=%d mode=%d l0ref=%d i=%d j=%d\n", p_Vid->ThisPOC, p_Vid->current_mb_nr, b8x8_mode, l0ref, i, j);
        for (m=0; m<=1; m++)
          for (n=0; n<=1; n++)
          { 
            printf("m=%d n=%d: mvx=%d mvy=%d\n", m, n, currSlice->all_mv [LIST_0][l0ref][ b8x8_mode][j+m][i+n].mv_x, currSlice->all_mv [LIST_0][l0ref][ b8x8_mode][j+m][i+n].mv_y);
          }
      ...
      case 0:
        ...
        for (m=0; m<=1; m++)
          for (n=0; n<=1; n++)
            if (out_of_bounds_mvs(p_Vid, &currSlice->all_mv [LIST_0][l0ref][b8x8_mode][j+m][i+n]))
            {
              invalid_mode = 1;
              return invalid_mode;
            }
      ...
      case 1:
        ...
        for (m=0; m<=1; m++)
          for (n=0; n<=1; n++)
            if (out_of_bounds_mvs(p_Vid, &currSlice->all_mv [LIST_1][l1ref][b8x8_mode][j+m][i+n]))
            {
              invalid_mode = 1;
              return invalid_mode;
            }
