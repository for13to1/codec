...
#if(MFC_ENC_3D_FCFR)
static void process_picture_in_dpb(VideoParameters *p_Vid, StorablePicture *p_pic);
#endif
...
void store_picture_in_dpb(DecodedPictureBuffer *p_Dpb, StorablePicture* p, FrameFormat *output)
{
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_ENC_3D_FCFR)
  if(p_Vid->p_Inp->num_of_views > 1 && (p_Vid->view_id > 0 || p_Vid->proc_picture[0] == NULL) )
#else
...
#endif
  {
    ...
    #if (MFC_ENC_3D_FCFR)
        if (p_Vid->p_Inp->ProfileIDC == MFC3D_HIGH && p_Vid->view_id == 0 )
        {
        // create the prediction of the EL from the BL reconstruction
        process_picture_in_dpb(p_Vid, p);
        p_Vid->proc_picture[0] = clone_storable_picture(p_Vid, p);
        }
        else if(p_Vid->view_id < (p_Vid->num_of_layers - 1)) // n-layer MVC support
        {
  ...
  if (p_Vid->currentPicture->idr_flag)
  {
    ...
    for ( layer_idx = 1; layer_idx < p_Vid->num_of_layers; layer_idx++ )
    {
      idr_memory_management(p_Vid->p_Dpb_layer[layer_idx], p, output);
    }
...
{
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  StorablePicture *proc_picture = p_Vid->proc_picture[0];
  FrameStore* fs = NULL;
  unsigned found;//i, 
...
#if MFC_ENC_3D_FCFR
 assert (p_Vid->p_Inp->ProfileIDC <=MFC3D_HIGH);
  if (p_Vid->p_Inp->ProfileIDC == MFC3D_HIGH )
  {
    // create the prediction of the EL from the BL reconstruction
    process_picture_in_dpb(p_Vid, p);
  }
...
static StorablePicture * clone_storable_picture( VideoParameters *p_Vid, StorablePicture *p_pic  )
{
  ...
  assert(p_pic->view_id == p_Vid->view_id);
#endif
  return p_stored_pic;
}
                      ...
                      int       picType)
{
    ...
    int status;
      ...
      reset_mfc_image_format(&p_Vid->imgData0,picType);
      reset_mfc_image_format(&p_Vid->imgData1,picType);
      reset_mfc_image_format(&p_Vid->imgData2,picType);
  ...
  // Create Enhancement Layer when reconstructed base layer is available
     status = process3DMuxWrapper(p_Vid->imgData2.pv_mfc_img_data,
                        p_Vid->imgData0.pv_mfc_img_data,
                        p_Vid->imgData1.pv_mfc_img_data,    
                        img_in->pv_mfc_img_data,
                        p_Vid->pv_mfc_muxing_params,                        
                        p_Vid->pv_mfc_encoder_layer_context,
                        1,
                        0,
                        0);
    ...
    if(0 != status)
    {
        error ("MFC Error :process3DMuxWrapper() Failed!!! ", status);
    }
    
    #if MFC_PROFILING
    gettime (&(p_Vid->mfc_end_time));              
    p_Vid->mfc_EL_Muxing_time += timediff(&(p_Vid->mfc_start_time), &(p_Vid->mfc_end_time));    
    #endif
    ...
    // Apply RPU to Recon BL */
    status = processEncRpuWrapper(p_Vid->pv_mfc_encoder_layer_context,
    ...
    if(0 != status)
    {
        error ("MFC Error :processEncRpuWrapper() Failed!!! ", status);
    }
...
static void process_picture_in_dpb(VideoParameters *p_Vid, StorablePicture *p_pic)
{
  ...
  img_in.format        = p_Vid->imgData.format;
  img_in.frm_stride[0] = p_pic->size_x_padded;
  img_in.frm_stride[1] = img_in.frm_stride[2] = p_pic->size_x_cr + (p_pic->pad_size_uv_x << 1);
  ...
  if (p_pic->structure == FRAME)
  {
    ...
    if (p_Vid->p_Inp->mfc_rpu_data.rpu_field_processing_flag)
    {
        img_in.top_data[0] = p_pic->imgY;
        img_in.top_data[1] = p_pic->imgUV[0];
        img_in.top_data[2] = p_pic->imgUV[1];
