...
{
#if (MFC_DEPTH_ENC)
  free_encoder_memory(p_Enc->p_Vid, p_Enc->p_Inp, p_Enc->p_VidDepth, p_Enc->p_InpDepth);
#else
  free_encoder_memory(p_Enc->p_Vid, p_Enc->p_Inp);
#endif
  exit (-1);
}
#if DISPLAY_LAYERLEVEL_BITUSAGE
void report_layer_stats(VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats, int64 bit_use[NUM_SLICE_TYPES][2],int i_layer_id)
{
  DistortionParams *p_Dist = p_Vid->p_Dist;
  ...
  char *nameString;
  double bitrate_fact,other_tot_bits;

  nameString =(char *) malloc((strlen(p_Inp->StatsFile) + 20) * sizeof(char));
  memset(nameString,0,(strlen(p_Inp->StatsFile) + 20) * sizeof(char));
  if (strlen(p_Inp->StatsFile) == 0)
  {
    strcpy (nameString,"stats");
  }
  else
  {
    strncpy (nameString,p_Inp->StatsFile,strlen(p_Inp->StatsFile)-4);
  }

    
    //p_Inp->StatsFile[strlen(p_Inp->StatsFile)-4] = '\0';

    sprintf( tempString, "_%d.dat", i_layer_id );  
    strcat(nameString, tempString);
    
    

  if ((p_stat = fopen(nameString, "wt")) == 0)
  {
    snprintf(errortext, ET_SIZE, "Error open file %s",nameString);
    error(errortext, 500);
  }

  fprintf(p_stat," -------------------------------------------------------------- \n");
  fprintf(p_stat,"  This file contains statistics for the last encoded sequence (Layer - %d)  \n",i_layer_id);
  fprintf(p_stat," -------------------------------------------------------------- \n");
  ...
  fprintf(p_stat,"\n Num Frames           |",i);
  fprintf(p_stat," %10.2f     |", (float) bit_use[I_SLICE][0]);
  fprintf(p_stat," %10.2f     |", (float) bit_use[P_SLICE][0]);   
  fprintf(p_stat," %10.2f     |", (float) bit_use[B_SLICE][0]);
  fprintf(p_stat," %10.2f     |", (float) bit_use[SP_SLICE][0]);   
  ...
  fprintf(p_stat,"\n ---------------------|----------------|----------------|----------------|----------------|");
   ...
   //header,mode,cbp y/c
   other_tot_bits =    (float) p_Stats->bit_use_header[I_SLICE] + p_Stats->bit_use_header[P_SLICE]  + p_Stats->bit_use_header[B_SLICE]
                    +(float) p_Stats->bit_use_mb_type[I_SLICE] + p_Stats->bit_use_mb_type[P_SLICE]  + p_Stats->bit_use_mb_type[B_SLICE]
                    +(float) p_Stats->tmp_bit_use_cbp[I_SLICE] + p_Stats->tmp_bit_use_cbp[P_SLICE]  + p_Stats->tmp_bit_use_cbp[B_SLICE]
                    +(float) p_Stats->bit_use_delta_quant[I_SLICE] + p_Stats->bit_use_delta_quant[P_SLICE]  + p_Stats->bit_use_delta_quant[B_SLICE]
                    +(float) p_Stats->bit_use_stuffing_bits[I_SLICE] + p_Stats->bit_use_stuffing_bits[P_SLICE]  + p_Stats->bit_use_stuffing_bits[B_SLICE];

  fprintf(p_stat,"\n Total Other Bits (Header+Mode+CBP Y/C,Delta Quant,StuffingBits for I+P+B)           :");
  fprintf(p_stat,"\n                                       :");
  fprintf(p_stat," %10.2f     |", other_tot_bits);


  fprintf(p_stat,"\n Total Coeffs. Y (I+P+B+SP)            :");
  fprintf(p_stat," %10.2f     |", (float) p_Stats->bit_use_coeff[0][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][SP_SLICE] );

  fprintf(p_stat,"\n Total Coeffs. C (I+P+B+SP)            :");
  fprintf(p_stat," %10.2f     |", (float) p_Stats->bit_use_coeffC[I_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[P_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[B_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[SP_SLICE] );

  
  fprintf(p_stat,"\n Total Coeffs. CB (I+P+B+SP)           :");
  fprintf(p_stat," %10.2f     |", (float) p_Stats->bit_use_coeff[1][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][SP_SLICE] );

  fprintf(p_stat,"\n Total Coeffs. CR (I+P+B+SP)           :");
  fprintf(p_stat," %10.2f     |", (float) p_Stats->bit_use_coeff[2][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][SP_SLICE] );

  bitrate_fact = (p_Vid->num_of_layers * p_Inp->output.frame_rate) /(1000 * p_Vid->total_frame_buffer);



  fprintf(p_stat,"\n Avg BR Coeffs. Y (I+P+B+SP) (kbits/s) :");
  fprintf(p_stat," %10.2f     |", ((float) p_Stats->bit_use_coeff[0][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[0][SP_SLICE])*bitrate_fact);

  fprintf(p_stat,"\n Avg BR Coeffs. C (I+P+B+SP) (kbits/s) :");
  fprintf(p_stat," %10.2f     |", ((float) p_Stats->bit_use_coeffC[I_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[P_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[B_SLICE] 
                                + (float) p_Stats->bit_use_coeffC[SP_SLICE])*bitrate_fact);

  
  fprintf(p_stat,"\n Avg BR Coeffs. CB (I+P+B+SP)(kbits/s) :");
  fprintf(p_stat," %10.2f     |", ((float) p_Stats->bit_use_coeff[1][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[1][SP_SLICE])*bitrate_fact);

  fprintf(p_stat,"\n Avg BR Coeffs. CR (I+P+B+SP)(kbits/s) :");
  fprintf(p_stat," %10.2f     |", ((float) p_Stats->bit_use_coeff[2][I_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][P_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][B_SLICE] 
                                + (float) p_Stats->bit_use_coeff[2][SP_SLICE])*bitrate_fact);

  fprintf(p_stat,"\n Avg BR Others                 (kbits/s) :");
  fprintf(p_stat," %10.2f     |", other_tot_bits*bitrate_fact);
...
void report( VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats)
{
  ...
  case 0:
	  ...
	  case 1:  
	default:
		fprintf(stdout,"-------------------------------------- %s Average data all frames  ------------------------------------\n\n",p_Vid->is_depth?"Depth":"Texture");
		break;
	case 2:
		fprintf(stdout,"-------------------------------------- %s Average data all frames  ------------------------------------\n\n",p_Vid->is_depth?"Depth":"Texture");
		break;
  ...
  if (p_Inp->Verbose != 0)
  {
    ...
    fprintf(stdout,  " Total ME time for sequence        : %7.3f sec \n\n", (float)p_Vid->me_tot_time * 0.001);
     ...
     p_Vid->mfc_BL_Muxing_time        = timenorm(p_Vid->mfc_BL_Muxing_time);
     p_Vid->mfc_EL_Muxing_time        = timenorm(p_Vid->mfc_EL_Muxing_time);

    fprintf(stdout,"\n\n");
    fprintf(stdout,"|-----------|-------------|MFC 3D Encoder PROFLING-------------|------------|\n");
    fprintf(stdout,"|     -     | lencode     | lencode   | RPU        |BL Muxing   | EL Muxing  |\n");        
    fprintf(stdout,"|     -     |(totaltime)  |(jm only)  |   -        |     -      |     -      |\n");    
 ...
 
     {
        ...
        t1 = p_Vid->tot_time-p_Vid->mfc_rpu_time- p_Vid->mfc_BL_Muxing_time- p_Vid->mfc_EL_Muxing_time;
        t2 = p_Vid->tot_time;
    
        fprintf(stdout,"| %5s     | %8.2f    | %8.2f  | %8.2f   |  %8.2f  | %8.2f   |\n","t(s)",t2*0.001,t1*0.001,p_Vid->mfc_rpu_time*0.001,p_Vid->mfc_BL_Muxing_time*0.001,p_Vid->mfc_EL_Muxing_time*0.001);            
        fprintf(stdout,"| %5s     | %8.2f    | %8.2f  | %8.2f   |  %8.2f  | %8.2f   |\n","fps",p_Stats->frame_counter  * 1000.0 / t2,p_Stats->frame_counter  * 1000.0 / t1,0.0,0.0,0.0);        
        fprintf(stdout,"| %5s     | %8.2f    | %8.2f  | %8.2f   |  %8.2f  | %8.2f   |\n","ms/f",(float)t2/p_Stats->frame_counter ,(float)t1/p_Stats->frame_counter,0.0,0.0,0.0);        
        fprintf(stdout,"| %5s     | %8.2f    | %8.2f  | %8.2f   |  %8.2f  | %8.2f   |\n","%inc",0.0,0.0,(float)(p_Vid->mfc_rpu_time)*100/t1,(float)(p_Vid->mfc_BL_Muxing_time)*100/t1,(float)(p_Vid->mfc_EL_Muxing_time)*100/t1);    
     }     
        fprintf(stdout,"|-----------|-------------|-----------|------------|------------|------------|\n\n\n");
...
#if(MFC_ENC_3D_FCFR)
#if (MFC_DEPTH_ENC)
		if (p_Inp->ProfileIDC == MFC3D_HIGH && p_Vid->is_depth==0)
#else
        if (p_Inp->ProfileIDC == MFC3D_HIGH)
#endif
        {
            fprintf(stdout," Base_Y { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n", 
                snr_v[0]->average[0], csnr_y_v[0], sse_v[0]->average[0]/(float)impix);
            fprintf(stdout," Base_U { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
                snr_v[0]->average[1], csnr_u_v[0], sse_v[0]->average[1]/(float)impix_cr);
            fprintf(stdout," Base_V { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
                snr_v[0]->average[2], csnr_v_v[0], sse_v[0]->average[2]/(float)impix_cr);
            fprintf( stdout, "\n" );
            fprintf(stdout," Enh_Y  { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n", 
                snr_v[1]->average[0], csnr_y_v[1], sse_v[1]->average[0]/(float)impix);
            fprintf(stdout," Enh_U  { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
                snr_v[1]->average[1], csnr_u_v[1], sse_v[1]->average[1]/(float)impix_cr);
            fprintf(stdout," Enh_V  { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
...
#if (MFC_DEPTH_ENC)
  p_Vid->p_Stats->total_bits = total_bits;
#endif
#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->num_of_views == 2)
  {
...
#if MFCD_INTERLACE
  if((p_Inp->num_of_views == 2 )&&( (p_Vid->fld_flag)||(p_Vid->p_DualVid->fld_flag) ))
#else
    ...
    for ( view_id = 0; view_id < NUM_VIEWS; view_id++ )
    {
        ...
        if (p_Inp->ProfileIDC == MFC3D_HIGH)
        {
            if (view_id == 0)
            {
                if (p_Stats->frame_ctr[B_SLICE] != 0)
                {
                    fprintf(stdout, " Base layer Total-bits             : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", B %" FORMAT_OFF_T  " NVB %d) \n",
                        total_bits_v[view_id],  p_Stats->bit_counter_v[view_id][I_SLICE], p_Stats->bit_counter_v[view_id][P_SLICE], p_Stats->bit_counter_v[view_id][B_SLICE], p_Stats->bit_ctr_parametersets_v[view_id]);
                }
                else
                {
                    fprintf(stdout, " Base layer Total-bits             : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", NVB %d) \n",
                        total_bits_v[view_id], p_Stats->bit_counter_v[view_id][I_SLICE], p_Stats->bit_counter_v[view_id][P_SLICE], p_Stats->bit_ctr_parametersets_v[view_id]);
                }
            }
            else
            {
                if (p_Stats->frame_ctr[B_SLICE] != 0)
                {
                    fprintf(stdout, " Enh layer Total-bits              : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", B %" FORMAT_OFF_T  " NVB %d) \n",
                        total_bits_v[view_id],  p_Stats->bit_counter_v[view_id][I_SLICE], p_Stats->bit_counter_v[view_id][P_SLICE], p_Stats->bit_counter_v[view_id][B_SLICE], p_Stats->bit_ctr_parametersets_v[view_id]);
                }
                else
                {
                    fprintf(stdout, " Enh layer Total-bits              : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", NVB %d) \n",
                        total_bits_v[view_id], p_Stats->bit_counter_v[view_id][I_SLICE], p_Stats->bit_counter_v[view_id][P_SLICE], p_Stats->bit_ctr_parametersets_v[view_id]);
                }
            }
        }
  ...
  {
    report_layer_stats(p_Vid, p_Inp, p_Vid->p_layer_Stats[i], bit_use,i);
  }
#endif
...
#if (MFC_DEPTH_ENC)
void threeDV_information_init( VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats )
{
	int i;
	...
	case 3:
		...
		fprintf(stdout,  " Texture Input YUV file  %d        : %s \n", 1,p_TextInp->input_file1.fname);
		fprintf(stdout,  " Texture Input YUV file  %d        : %s \n", 2,p_TextInp->input_file2.fname);
	
	
		fprintf(stdout,  " Depth   Input YUV file  %d        : %s \n", 1,p_Inp->input_file1.fname);
		fprintf(stdout,  " Depth   Input YUV file  %d        : %s \n", 2,p_Inp->input_file2.fname);
	...
	if (p_TextVid->p_dec != -1)
		fprintf(stdout,  " Texture Output YUV file  %d        : %s \n", 1, p_TextInp->ReconFile);
	if (p_TextVid->p_dec2 != -1)
		fprintf(stdout,  " Texture Output YUV file  %d        : %s \n", 1, p_TextInp->ReconFile2);

	if (p_Vid->p_dec != -1)
		fprintf(stdout,  " Depth Output YUV file  %d        : %s \n", 1, p_Inp->ReconFile);
	if (p_Vid->p_dec2 != -1)
		fprintf(stdout,  " Depth Output YUV file  %d        : %s \n", 1, p_Inp->ReconFile2);



	fprintf(stdout,  " YUV Format                        : Texture (%s) Depth (%s)\n", &yuv_types[p_TextVid->yuv_format][0],&yuv_types[p_Vid->yuv_format][0]);//p_Vid->yuv_format==YUV422?"YUV 4:2:2":(p_Vid->yuv_format==YUV444)?"YUV 4:4:4":"YUV 4:2:0");
	fprintf(stdout,  " Frames to be encoded              : %d\n", p_Inp->no_frames);
	if (p_Inp->Verbose != 0)
	{
		fprintf(stdout,  " Freq. for encoded bitstream       : %3.2f\n", p_Inp->output.frame_rate);
		fprintf(stdout,  " Transform8x8Mode                  : Texture( %d ) Depth( %d )\n", p_TextInp->Transform8x8Mode,p_Inp->Transform8x8Mode);

		for (i=0; i<3; i++)
		{
			fprintf(stdout," ME Metric for Refinement Level %1d  : Texture( %s ) Depth( %s )\n", i, DistortionType[p_TextInp->MEErrorMetric[i]],DistortionType[p_Inp->MEErrorMetric[i]]);
		}
		fprintf(stdout,  " Mode Decision Metric              : Texture( %s ) Depth( %s )\n", DistortionType[p_TextInp->ModeDecisionMetric],DistortionType[p_Inp->ModeDecisionMetric]);


		fprintf(stdout,  " Texture Image format              : %dx%d (%dx%d)\n", p_TextInp->output.width[0], p_TextInp->output.height[0], p_TextVid->width,p_TextVid->height);
		fprintf(stdout,  " Depth   Image format              : %dx%d (%dx%d)\n", p_Inp->output.width[0], p_Inp->output.height[0], p_Vid->width,p_Vid->height);

		fprintf(stdout,  " Search range                      : %d\n", p_Inp->search_range[0]);

		fprintf(stdout,  " Total number of references        : %d\n", p_Inp->num_ref_frames);
		fprintf(stdout,  " References for P slices           : Texture( %d ) Depth( %d )\n", p_TextInp->P_List0_refs[0] ? p_TextInp->P_List0_refs[0] : p_TextInp->num_ref_frames,
			p_Inp->P_List0_refs[1] ? p_Inp->P_List0_refs[1] : p_Inp->num_ref_frames);
		fprintf(stdout,  " References for B slices (L0, L1)  : Texture( %d, %d ) Depth( %d, %d )\n", 
			p_TextInp->B_List0_refs[0] ? p_TextInp->B_List0_refs[0] : p_TextInp->num_ref_frames, 
			p_TextInp->B_List1_refs[0] ? p_TextInp->B_List1_refs[0] : p_TextInp->num_ref_frames,
			p_Inp->B_List0_refs[1] ? p_Inp->B_List0_refs[1] : p_Inp->num_ref_frames, 
			p_Inp->B_List1_refs[1] ? p_Inp->B_List1_refs[1] : p_Inp->num_ref_frames);

		// Sequence Type
		fprintf(stdout,  " Sequence type                     :");
		if (p_Stats->NumberBFrames > 0 && p_Inp->HierarchicalCoding)
		{
			fprintf(stdout, " Hierarchy Texture( QP: I %d, P %d, B %d ) Depth( QP: I %d, P %d, B %d )\n",
				p_TextInp->qp[I_SLICE], p_TextInp->qp[P_SLICE], p_TextInp->qp[B_SLICE],p_Inp->qp[I_SLICE], p_Inp->qp[P_SLICE], p_Inp->qp[B_SLICE]);
		}
		else if (p_Stats->NumberBFrames == 0 && (p_Inp->intra_period == 1 || p_Inp->idr_period == 1)) 
			fprintf(stdout, " IIII Texture( QP: I %d ) Depth( QP: I %d )\n", p_TextInp->qp[I_SLICE],p_Inp->qp[I_SLICE]);
		else if (p_Stats->NumberBFrames == 0 && p_Inp->sp_periodicity == 0) 
			fprintf(stdout, " IPPP Texture( QP: I %d, P %d ) Depth( QP: I %d, P %d )\n", p_TextInp->qp[I_SLICE], p_TextInp->qp[P_SLICE],p_Inp->qp[I_SLICE], p_Inp->qp[P_SLICE]);
		...
		fprintf(stdout,  " Texture Profile/Level IDC                 : (%d,%d)\n", p_TextInp->ProfileIDC, p_TextInp->LevelIDC);
		fprintf(stdout,  " Depth Profile/Level IDC                 : (%d,%d)\n", p_Inp->ProfileIDC, p_Inp->LevelIDC);

		if (p_TextInp->rdopt)
			fprintf(stdout," Texture RD-optimized mode decision: used\n");
		else
			fprintf(stdout," Texture RD-optimized mode decision: not used\n");

		if (p_Inp->rdopt)
			fprintf(stdout," Depth RD-optimized mode decision  : used\n");
		else
			fprintf(stdout," Depth RD-optimized mode decision  : not used\n");


		switch(p_TextInp->of_mode)
		{
		case PAR_OF_ANNEXB:
	...
	case 1:
		printf("------------------------------------------------------------------------------------\n");
		printf("Frame     View  TId Bit/pic    QP   SnrY    SnrU    SnrV    Time(ms) MET(ms) Frm/Fld  Anchor/Non-Anchor Ref  \n");
		printf("------------------------------------------------------------------------------------\n");

		break;
	case 2:
		if (p_TextInp->Distortion[SSIM] == 1)
		{
			printf("--------------------------------------------------------------------------------------------------------------------------------\n");
