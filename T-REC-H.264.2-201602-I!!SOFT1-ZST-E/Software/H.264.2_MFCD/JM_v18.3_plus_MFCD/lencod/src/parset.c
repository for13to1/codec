...
#if MFC_ENC_3D_FCFR
void write_sps_rpu_data(MFC_RPU_DATA *mfc_rpu_data,Bitstream *bitstream,int *len ,int frame_mbs_only, int profile_idc)
{
  if(MFC3D_HIGH == profile_idc)
  {    
    *len += write_u_v(6,"SPS:MFC mfc_format_idc"                            ,mfc_rpu_data->mfc_format_idc        ,bitstream);

    if(mfc_rpu_data->mfc_format_idc==0 || mfc_rpu_data->mfc_format_idc==1)
    {        
        *len += write_u_1  ("SPS: MFC default_grid_position_flag"        ,mfc_rpu_data->default_grid_position_flag    ,bitstream);
        if(!mfc_rpu_data->default_grid_position_flag)
        {
            *len += write_u_v(4,"SPS:MFC view0_grid_position_x"          ,mfc_rpu_data->view0_grid_position_x    ,bitstream);
            *len += write_u_v(4,"SPS:MFC view0_grid_position_y"          ,mfc_rpu_data->view0_grid_position_y    ,bitstream);
            *len += write_u_v(4,"SPS:MFC view1_grid_position_x"          ,mfc_rpu_data->view1_grid_position_x    ,bitstream);
            *len += write_u_v(4,"SPS:MFC view1_grid_position_y"          ,mfc_rpu_data->view1_grid_position_y    ,bitstream);
        }
    }
    
    *len += write_u_1  ("SPS: MFC rpu_filter_enabled_flag"               ,mfc_rpu_data->rpu_filter_enabled_flag               ,bitstream);
    if(!frame_mbs_only)
    {
        *len += write_u_1  ("SPS: MFC rpu_field_processing_flag"        ,mfc_rpu_data->rpu_field_processing_flag,bitstream);
    }
                                  ...
                                  )
{
  ...
  int frext_profile = ((identify_profile(p_Inp)==FREXT_HP) ||
    ...
    #if (MFC_ENC_3D_FCFR)
                      || (identify_profile(p_Inp)==MFC3D_HIGH)
    #endif
	#if (MFC_DEPTH_ENC)
	                 || (identify_profile(p_Inp)==ThreeDV_HIGH)
					 || (identify_profile(p_Inp)==MFC_DEPTH_HIGH)
...
#endif
{
  ...
  if(p_Vid->p_Inp->num_of_views==2)
  {
    ...
    {
#if (MFC_DEPTH_ENC)
	  assert(sps->profile_idc==MULTIVIEW_HIGH || sps->profile_idc==STEREO_HIGH   || sps->profile_idc==MFC3D_HIGH 
		  || sps->profile_idc==ThreeDV_HIGH || sps->profile_idc==MFC_DEPTH_HIGH);
#elif MFC_ENC_3D_FCFR
      assert(sps->profile_idc==MULTIVIEW_HIGH || sps->profile_idc==STEREO_HIGH   || sps->profile_idc==MFC3D_HIGH);
#else
         assert(sps->profile_idc==MULTIVIEW_HIGH || sps->profile_idc==STEREO_HIGH );
#endif
#if MFC_MVC_HACK
		 len+=write_u_v  (8, "SPS: profile_idc",                            128,                               bitstream);
#else
  ...
  if(Is_Subset==1)
  {
    ...
    // fixed to 2 views only
    {
      ...
      if ( p_Vid->p_Inp->MVCFlipViews )
      {
		...
		{
			len+=write_u_1 ("SPS: depth_view_present_flag",                     p_Inp->ViewPresentFlag[0],           bitstream);
			len+=write_u_1("SPS: texture_view_present_flag",                   p_Vid->p_DualInp->ViewPresentFlag[0], bitstream);
		}
#endif        
        ...
        if ( (sps->profile_idc==ThreeDV_HIGH) || (sps->profile_idc==MFC_DEPTH_HIGH))
		{
			len+=write_u_1 ("SPS: depth_view_present_flag",                     p_Inp->ViewPresentFlag[1],           bitstream);
			len+=write_u_1("SPS: texture_view_present_flag",                   p_Vid->p_DualInp->ViewPresentFlag[1], bitstream);
		}
#endif 

#if (MFC_DEPTH_ENC)
		if ( (sps->profile_idc==ThreeDV_HIGH || sps->profile_idc==MFC_DEPTH_HIGH ) && p_Inp->ViewPresentFlag[1]==0)
		{
		}
		else
#endif
	  ...
	  if (sps->profile_idc==ThreeDV_HIGH || sps->profile_idc==MFC_DEPTH_HIGH)
      {
        len+=write_u_1  ("SPS: applicable_op_depth_flag[ 0 ][ 0 ][ 0 ]",      0,           bitstream);
        len+=write_u_1  ("SPS: applicable_op_texture_flag[ 0 ][ 0 ][ 0 ]",      0,           bitstream);
		len+=write_ue_v ("SPS: applicable_op_num_texture_views_minus1[ 0 ][ 0 ]",  0,           bitstream);
	    len+=write_ue_v ("SPS: applicable_op_num_depth_views[ 0 ][ 0 ]",           0,           bitstream);
      }      
	  else
#endif
    ...
    /* Write RPU Data */
#if !MFC_MVC_HACK
    write_sps_rpu_data(&p_Vid->p_Inp->mfc_rpu_data,bitstream,&len,sps->frame_mbs_only_flag,sps->profile_idc);
#endif
    #endif
    //end of seq_parameter_set_mvc_extension( ) 
#if (MFC_DEPTH_ENC)
    if (sps->profile_idc==ThreeDV_HIGH||sps->profile_idc==MFC_DEPTH_HIGH)
      len+=write_u_1  ("SPS: 3dvc_vui_parameters_present_flag",                    sps->vui_parameters_present_flag,           bitstream);
    else
#endif
    ...
    if (sps->profile_idc==ThreeDV_HIGH || sps->profile_idc==MFC_DEPTH_HIGH)
    {
      len+=write_u_1  ("SPS: texture_vui_parameters_present_flag",                    0,           bitstream);
    }

	/*if((sps->profile_idc==ThreeDV_HIGH)||(sps->profile_idc==MFC_DEPTH_HIGH))
	{
		len+=write_u_1 ("SPS: additional_extension3_flag",                          0,           bitstream);
	}
	else*/
#endif
