 ...
 ************************************************************************
 */
void update_all_global_stats(InputParameters *p_Inp, VideoParameters *p_Vid, StatParameters *cur_stats,int i_layer_id)
{
    //StatParameters *gl_stats
    update_global_stats(p_Inp, p_Vid->p_Stats, cur_stats);

    #if DISPLAY_LAYERLEVEL_BITUSAGE
    update_global_stats(p_Inp, p_Vid->p_layer_Stats[i_layer_id], cur_stats);
    #endif
...
#if (MVC_EXTENSION_ENABLE)
  #if (MFC_ENC_3D_FCFR)
  if(p_Inp->num_of_views==2 && p_Inp->ProfileIDC == MFC3D_HIGH)
  {
      // For MFC Profile read left and right frames simulatneously
       if(p_Vid->view_id == 0)
       {
           file_read = read_one_frame (p_Vid, &p_Inp->input_file1, p_Vid->frm_no_in_file, p_Inp->infile_header, &p_Inp->source, &p_Inp->output, p_Vid->imgData0.frm_data);
            ...
            file_read = read_one_frame (p_Vid, &p_Inp->input_file2, p_Vid->frm_no_in_file, p_Inp->infile_header, &p_Inp->source, &p_Inp->output, p_Vid->imgData1.frm_data);
            if ( !file_read )
            {
                // end of file or stream found: trigger error handling
                get_number_of_frames (p_Inp, &p_Inp->input_file2);
            ...
            pad_borders (p_Inp->output, p_Vid->width, p_Vid->height, p_Vid->width_cr, p_Vid->height_cr, p_Vid->imgData1.frm_data);
...
void perform_encode_frame(VideoParameters *p_Vid)
{
  ...
  if(p_Vid->view_id!=1 || !p_Vid->sec_view_force_fld)
  {
    ...
    if ((p_Vid->type == B_SLICE || p_Vid->type == P_SLICE || p_Vid->type==I_SLICE) && p_Inp->RDPictureDecision && p_Vid->view_id == 0)
    {
      frame_picture_mp (p_Vid, p_Inp); 
    }
    else if ((p_Vid->type == B_SLICE || p_Vid->type == P_SLICE || p_Vid->type==I_SLICE) && p_Inp->RDPictureDecisionEL && p_Vid->view_id > 0)
    {
      frame_picture_mp (p_Vid, p_Inp); 
    }
#else
  ...
  if ((p_Inp->PicInterlace == ADAPTIVE_CODING) && (!is_MVC_profile(p_Vid->active_sps->profile_idc) || 
     (/*(p_Vid->active_sps->profile_idc == MFC3D_HIGH) &&*/ ((p_Vid->view_id == 0) || (p_Vid->view_id == 1 && p_Vid->sec_view_force_fld)))))
#else
...
#endif
  {
    ...
    if(p_Inp->num_of_views==1 || p_Vid->view_id==0)  // first view will make the decision
    {
	  ...
	  printf("frame_no = %d, field_coding = %d\n", p_Vid->frame_no,  p_Vid->fld_flag);
#endif
...
void free_slice_data(VideoParameters *p_Vid)
{
  ...
  if ((p_Vid->field_pic_ptr||p_Vid->p_DualVid->field_pic_ptr) && 
    (p_Vid->p_Inp->num_of_views==1 || ((VOIdex==1)&&(tempVOIdx==1)&&(p_Vid->curr_frm_idx==p_Vid->p_DualVid->curr_frm_idx)) ) ) 
  {
    if(p_Vid->is_depth)
    ...
    for (i = 0; i < 2; i++)
    {
      ...
      if (p_VidTmp->p_Inp->num_of_views==2 && p_VidTmp->field_pic2)
      {
    ...
    if (p_VidTmp->p_DualVid->field_pic_ptr==NULL)
    {
      ...
      {  
        ...
        if (p_VidTmp->p_DualInp->num_of_views==2 && p_VidTmp->p_DualVid->frame_pic2)
        {
...
void store_coded_picture(DecodedPictureBuffer *p_Dpb)
{
...
#endif
  {
    if (p_Vid->fld_flag)
    {      
      update_all_global_stats(p_Inp, p_Vid, &p_Vid->enc_field_picture[0]->stats,p_Dpb->layer_id);
      update_all_global_stats(p_Inp, p_Vid, &p_Vid->enc_field_picture[1]->stats,p_Dpb->layer_id);
      // store bottom field
      store_picture_in_dpb (p_Dpb, p_Vid->enc_field_picture[1], &p_Inp->output);
#if (MVC_EXTENSION_ENABLE)
      if ( p_Vid->proc_picture[0] && is_MVC_profile(profile_idc) )
      {
        store_proc_picture_in_dpb (p_Vid->p_Dpb_layer[1], p_Vid->proc_picture[0], &p_Inp->output);
    ...
    {
      update_all_global_stats(p_Inp, p_Vid, &p_Vid->enc_frame_picture[p_Vid->rd_pass]->stats,p_Dpb->layer_id);
  ...
  else
  {
    if (p_Vid->fld_flag)
    {
      ...
      store_picture_in_dpb(p_Dpb, p_Vid->enc_field_picture[1], &p_Inp->output);
#if (MVC_EXTENSION_ENABLE)
      if ( is_MVC_profile(profile_idc) && p_Vid->proc_picture[0])
      {
        store_proc_picture_in_dpb (p_Vid->p_Dpb_layer[1], p_Vid->proc_picture[0], &p_Inp->output);
        p_Vid->proc_picture[0] = NULL;
      }
    ...
    {      
      ...
      {
        update_all_global_stats(p_Inp, p_Vid, &p_Vid->enc_picture->stats,p_Dpb->layer_id);
...
#if (MFCD_INTERLACE)
  if( (p_Inp->num_of_views>1)&&( (p_Vid->fld_flag)||(p_Vid->p_DualVid->fld_flag) )&&
...
#if (MFCD_INTERLACE)
  if( (p_Vid->is_depth)&&(p_Inp->num_of_views>1) )
  {
...
//#endif
  ...
  for (slice=0; slice < pic->no_slices; slice++)
  {
    ...
    for (partition=0; partition < currSlice->max_part_nr; partition++)
    {
      ...
      {
#if (MFC_DEPTH_ENC)
        if(p_Vid->p_Inp->num_of_views==2 &&(!p_Vid->is_depth)&&p_Vid->view_id==0)
#else
 ...
 * \brief
 ************************************************************************
static void get_dual_picture(VideoParameters *p_Vid, StorablePicture *stored_pic )
{
	...
	if(p_DualDpb)
	{
		for(i=0;i<p_DualDpb->used_size;++i)
		{
			...
			if((p->view_id==p_Vid->view_id)&&(p->frame_num==p_Vid->frame_num)&&(p->poc==p_Vid->framepoc))
			{
				...
				stored_pic->dual_picture=p;
				if(p->dual_picture==NULL)
					p->dual_picture=stored_pic;
				break;
			}
		}
	}
...
static void prepare_3dv_coding(VideoParameters* p_Vid, StorablePicture*stored_pic)
{
	...
	/*if((p_Vid->is_depth==0)&&(p_Vid->view_id==0))
	{
		if(p_Vid->p_DualVid)
...
*/
  ...
  stored_pic->upsampling_params=p_Vid->low_res?p_Vid->p_output_upsample_params:NULL;
	...
	//get_dual_picture(p_Vid,stored_pic);
}
#endif
/*!
************************************************************************
...
#if (MVC_EXTENSION_ENABLE)
  if ( is_MVC_profile(p_Vid->active_sps->profile_idc) && p_Vid->proc_picture[0])
  {
    store_proc_picture_in_dpb (p_Vid->p_Dpb_layer[1], p_Vid->proc_picture[0], &p_Inp->output);
    p_Vid->proc_picture[0] = NULL;
  }
  ...
  if ( p_Vid->p_Inp->num_of_views == 2 )
  {
    ...
    if ( p_VidTempText->fld_flag )
    {
      ...
      if ( ((voidx==1)&&(voidx1==1)&&(p_Vid->curr_frm_idx==p_Vid->p_DualVid->curr_frm_idx)) )
      {
        ...
        NumOfViewsTexture=p_VidTempText->p_Inp->num_of_views;
        NumofViewsDepth=p_VidTempText->p_Inp->ThreeDVCoding?p_VidTempDepth->p_Inp->num_of_views:0;
        ...
        for(i=0;i<NumofViewsDepth+NumOfViewsTexture;++i)
        {
          ...
          if( voidx==0 )
            ...
            p_VidTmp->prev_cs.frm_no_in_file, p_VidTmp->prev_cs.pic_type, p_VidTmp->prev_cs.view_id,
            ...
            printf ("%05d(%3s)  %1d %8d   %2d %7.3f %7.3f %7.3f %9d %7d    %3s    %d\n",
            p_VidTmp->prev_cs1.frm_no_in_file, p_VidTmp->prev_cs1.pic_type, p_VidTmp->prev_cs1.view_id, 
...
#if (MFC_FPA_SEI)
#if (MFC_DEPTH_ENC)
  if ( ( p_Vid->view_id == 0 && p_Vid->is_depth == 0 && p_Vid->p_Inp->FPASEIPresentFlag) )
  {
#else
  if (p_Vid->view_id == 0 || (!p_Vid->p_Inp->FPASEIPresentFlag) )
  {
#endif
#endif
