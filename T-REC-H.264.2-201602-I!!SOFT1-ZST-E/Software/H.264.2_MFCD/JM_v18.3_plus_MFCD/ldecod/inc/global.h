...
typedef struct video_par
{
...
#if (MFC_DEPTH_DEC)
  struct decoded_picture_buffer *p_Dpb_layer[2][MAX_NUM_DPB_LAYERS];
  CodingParameters *p_EncodePar[2][MAX_NUM_DPB_LAYERS];
  LayerParameters *p_LayerPar[2][MAX_NUM_DPB_LAYERS];
#else
  ...
  int text_view_coding_order[MAX_VIEW_NUM];
  int depth_view_coding_order[MAX_VIEW_NUM];
  //int camera_order[MAX_CODEVIEW];

  //int ViewCompOrderDepthFlag[2*MAX_VIEW_NUM];
  //int ViewCompOrderVOIdx[2*MAX_VIEW_NUM];
...
#if (MFC_DEPTH_DEC)
  struct snr_par       *snr_3dv[2][MAX_VIEW_NUM];
  int  num_of_views;
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_DEC)
  int p_out_3dv[2][MAX_VIEW_NUM];     //!< file descriptor to output YUV file for 3DV
#else
  int p_out_mvc[MAX_VIEW_NUM];     //!< file descriptor to output YUV file for MVC
#endif
#endif
#if (MFC_DEPTH_DEC)
  int p_ref_3dv[2][MAX_VIEW_NUM] ;
#else
...
/******************* end deprecative variables; ***************************************/
  ...
  #if(MFC_DEC_3D_FCFR)  
  void *pv_mfc_rpu_data;  
  void *pv_mfc_decoder_layer_context;    
  MFC_RPU_DATA mfc_rpu_data;
  ImageData tempData4;

   #if(MFC_PROFILING)
   int64 mfc_rpu_time;   
   int64 mfc_reconstruction_time;   
   TIME_T mfc_start_time;
   TIME_T mfc_end_time;
   #endif
...
typedef struct inp_par
{
 ...
 int write_uv[2];                   //!< 0:texture 1:depth
  ...
  int  DeMuxMode;    
    #if (MFC_DEC_3D_FCFR)        
        int EnableDbgYUVFiles;
        FILE *fp_recon_baselayer;
        FILE *fp_pred_enhlayer;        
    #endif
...
#if (MFC_DEPTH_DEC)
extern int  init_global_buffers( VideoParameters *p_Vid, int layer_id, int is_depth );
#else
...
#if (MFC_DEPTH_DEC)
extern void free_layer_buffers( VideoParameters *p_Vid, int layer_id, int is_depth );
#else
...
#if (MFC_DEPTH_DEC)
extern void OpenOutputFiles(VideoParameters *p_Vid, int view0_id, int view1_id, int is_depth);
#else
...
#if (MFC_DEPTH_DEC)
		   || profile_idc == ThreeDV_HIGH || profile_idc == MFC_DEPTH_HIGH
#endif
		   );
}
...
#if (MFC_DEPTH_DEC)
static inline int is_MVCD_profile(unsigned int profile_idc)
{
  return (  (profile_idc == ThreeDV_HIGH) || (profile_idc == MFC_DEPTH_HIGH));
}
#endif
