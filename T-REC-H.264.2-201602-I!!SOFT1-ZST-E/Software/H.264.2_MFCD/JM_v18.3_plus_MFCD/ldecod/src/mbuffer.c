...
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb, int type)
{
 ...
 #if (MFC_DEPTH_DEC)
  int is_depth = active_sps->is_depth;
#endif
...
#if (MFC_DEPTH_DEC)
  if(!p_Vid->no_reference_picture[is_depth])
  {
    p_Vid->no_reference_picture[is_depth] = alloc_storable_picture (p_Vid, FRAME, p_Vid->width, p_Vid->height, p_Vid->width_cr, p_Vid->height_cr, 1);
    p_Vid->no_reference_picture[is_depth]->top_field    = p_Vid->no_reference_picture[is_depth];
    p_Vid->no_reference_picture[is_depth]->bottom_field = p_Vid->no_reference_picture[is_depth];
    p_Vid->no_reference_picture[is_depth]->frame        = p_Vid->no_reference_picture[is_depth];
  }
#else
...
#if (MFC_DEPTH_DEC)
  p_Vid->last_has_mmco_5[is_depth] = 0;
#else
...
#if (MFC_DEPTH_DEC)
  if(p_Vid->no_reference_picture[0])
  {
	  free_storable_picture(p_Vid->no_reference_picture[0]);
	  p_Vid->no_reference_picture[0] = NULL;
  }
  if(p_Vid->no_reference_picture[1])
  {
	  free_storable_picture(p_Vid->no_reference_picture[1]);
	  p_Vid->no_reference_picture[1] = NULL;
  }
#else
  if(p_Vid->no_reference_picture)
  {
...
StorablePicture* alloc_storable_picture(VideoParameters *p_Vid, PictureStructure structure, int size_x, int size_y, int size_x_cr, int size_y_cr, int is_output)
{
...
#if(MFC_DEC_3D_FCFR)
  get_mem2Dpel_pad (&(s->postY), size_y, size_x, p_Vid->iLumaPadY, p_Vid->iLumaPadX);
#if (!MFCD_FORCE_YUV_400)
  if (active_sps->chroma_format_idc != YUV400)
#endif
  {
    get_mem3Dpel_pad(&(s->postUV), 2, size_y_cr, size_x_cr, p_Vid->iChromaPadY, p_Vid->iChromaPadX);
  }

#if (MFC_DEPTH_DEC)
  if( /*(MFC3D_HIGH==p_Vid->active_sps->profile_idc || MFC_DEPTH_HIGH==p_Vid->active_sps->profile_idc) &&*/ p_Vid->p_Inp->export_views)
#else
  if(MFC3D_HIGH==p_Vid->SubsetSeqParSet[0].sps.profile_idc && p_Vid->p_Inp->export_views)
#endif
  {
    s->outY = s->postY;
    s->outUV[0] = s->postUV[0];
    s->outUV[1] = s->postUV[1];
  }
  ...
  {
    s->size_x_cr_ori  = (s->size_x_ori >> 1);
    s->size_y_cr_ori = (s->size_y_ori >> 1);
  }
  else if (active_sps->chroma_format_idc == YUV422)
  {
    s->size_x_cr_ori = (s->size_x_ori >> 1);
    s->size_y_cr_ori = s->size_y_ori;
  }
  else if (active_sps->chroma_format_idc== YUV444)
  {
    //YUV444
    s->size_x_cr_ori = s->size_x_ori;
    s->size_y_cr_ori = s->size_y_ori;
  }
#endif 
...
#if (MFC_DEPTH_DEC)
	s->top_field    = p_Vid->no_reference_picture[active_sps->is_depth];
	s->bottom_field = p_Vid->no_reference_picture[active_sps->is_depth];
	s->frame        = p_Vid->no_reference_picture[active_sps->is_depth];
#else
...
void free_storable_picture(StorablePicture* p)
{
  ...
  if (p)
  {
    ...
    if (p->postY)
    {
      free_mem2Dpel_pad(p->postY, p->iLumaPadY, p->iLumaPadX);
      p->postY=NULL;
    }

    if (p->postUV)
    {
      free_mem3Dpel_pad(p->postUV, 2, p->iChromaPadY, p->iChromaPadX);
      p->postUV=NULL;
    }
    #endif
...
int is_depth=currSlice->is_depth;
  ...
  {
#if (MFC_DEPTH_DEC)
    currSlice->listX[0][i] = p_Vid->no_reference_picture[is_depth];
#else
  ...
  {
#if (MFC_DEPTH_DEC)
    currSlice->listX[1][i] = p_Vid->no_reference_picture[is_depth];
#else
...
void init_mbaff_lists(VideoParameters *p_Vid, Slice *currSlice)
{
  ...
  for (i=2;i<6;i++)
  {
    ...
    {
#if (MFC_DEPTH_DEC)
	  currSlice->listX[i][j] = p_Vid->no_reference_picture[is_depth];
#else
...
#if (MFC_DEPTH_DEC)
  return currSlice->p_Vid->no_reference_picture[is_depth];
#else
...
static void adaptive_memory_management(DecodedPictureBuffer *p_Dpb, StorablePicture* p)
{
...
#if (MFC_DEPTH_DEC)
	p_Vid->last_has_mmco_5[p->is_depth] = 0;
#else
  p_Vid->last_has_mmco_5 = 0;
#endif
...
#if (MFC_DEPTH_DEC)
        p_Vid->last_has_mmco_5[p->is_depth] = 1;	
#else
...
#if (MFC_DEPTH_DEC)
  if ( p_Vid->last_has_mmco_5[p->is_depth] )
#else
...
#endif
  {
    ...
    {
#if (MFC_DEPTH_DEC)
      flush_dpb(p_Vid->p_Dpb_layer[0][0]);
      flush_dpb(p_Vid->p_Dpb_layer[0][1]);
	  flush_dpb(p_Vid->p_Dpb_layer[1][0]);
      flush_dpb(p_Vid->p_Dpb_layer[1][1]);
#else	
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_DEC)
        direct_output(p_Vid, p, p_Vid->p_out_3dv[is_depth][p_Dpb->layer_id]);
#else
  ...
  if (fs->is_used==3)
  {
    calculate_frame_no(p_Vid, p);
		...
		if (-1 != p_Vid->p_ref_3dv[p->is_depth][ voidx] && !p_Inp->silent)
			find_snr(p_Vid, fs->frame, &p_Vid->p_ref_3dv[p->is_depth][ voidx]);
#else
...
static int output_one_frame_from_dpb(DecodedPictureBuffer *p_Dpb)
{
  ...
  int poc, pos;
#if (MFC_DEPTH_DEC)
  int is_depth = p_Dpb->fs[0]->frame->is_depth;
#endif
  //diagnostics
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_DEC)
    write_lost_non_ref_pic(p_Dpb, poc, p_Vid->p_out_3dv[is_depth][p_Dpb->layer_id]);
#else
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_DEC)
  write_stored_frame(p_Vid, p_Dpb->fs[pos], p_Vid->p_out_3dv[is_depth][p_Dpb->layer_id]);
#else
...
#if (MFC_DEPTH_DEC)
    fs_top->p_out   = fs_btm->p_out   = fs->p_out;
#endif
#endif
...
#if MFCD_INTERLACE
  fs->frame->sps=fs->top_field->sps;
  fs->frame->is_depth=fs->top_field->is_depth;

  fs->frame->reduced_res=fs->top_field->reduced_res;
  fs->frame->upsampling_params=fs->top_field->upsampling_params;
#endif
                      ...
                      int       picType)
{
...
#if MFCD_FORCE_YUV_400
	if (img_in->format.yuv_format == YUV400)
	{
		is_yuv_400 = 1;
		img_in->format.yuv_format = YUV420;
	}
#endif
    reset_mfc_image_format(img_in,picType);
#if MFCD_FORCE_YUV_400
	if (is_yuv_400 == 1)
	{
	  img_in->format.yuv_format = YUV400;
	}
#endif
#if MFCD_FORCE_YUV_400
	is_yuv_400 = 0;
	...
	{
		is_yuv_400 = 1;
		p_Vid->tempData3.format.yuv_format = YUV420;
	}
#endif
    reset_mfc_image_format(&p_Vid->tempData3,picType);
#if MFCD_FORCE_YUV_400
	if (is_yuv_400 == 1)
	{
	  p_Vid->tempData3.format.yuv_format = YUV400;
	}
#endif
    status = processDecRpuWrapper( p_Vid->pv_mfc_decoder_layer_context,
                        img_in->pv_mfc_img_data,                     //recon BL                        
                        p_Vid->tempData3.pv_mfc_img_data            //pred_EL        
                        );

    if(0 != status)
    {
        error ("MFC Error :processDecRpuWrapper() Failed!!! ", status);
    }
  
    #if MFC_PROFILING
    gettime (&(p_Vid->mfc_end_time));              
    p_Vid->mfc_rpu_time += timediff(&(p_Vid->mfc_start_time), &(p_Vid->mfc_end_time));  
    #endif


}

void process_picture_in_dpb(VideoParameters *p_Vid, StorablePicture *p_pic)
{
  InputParameters *p_Inp = p_Vid->p_Inp;
   
  ImageData img_in;  
  imgpel *itemp1, *itemp2,*itemp3;
  ...
  {
    p_Vid->tempData3.format.auto_crop_bottom    = p_Inp->output.auto_crop_bottom;
    p_Vid->tempData3.format.auto_crop_right     = p_Inp->output.auto_crop_right;
    p_Vid->tempData3.format.auto_crop_bottom_cr = p_Inp->output.auto_crop_bottom_cr;
    p_Vid->tempData3.format.auto_crop_right_cr  = p_Inp->output.auto_crop_right_cr;
    p_Vid->tempData3.format.yuv_format          = (ColorFormat) p_Vid->active_sps->chroma_format_idc;
  }
#endif
  
  if(p_Vid->tempData4.frm_data[0] == NULL)
    init_img_data( p_Vid, &(p_Vid->tempData4), p_Vid->active_sps);  
  #if (MFCD_REDUCED_RES)
  else
  {
    p_Vid->tempData4.format.auto_crop_bottom    = p_Inp->output.auto_crop_bottom;
    p_Vid->tempData4.format.auto_crop_right     = p_Inp->output.auto_crop_right;
    p_Vid->tempData4.format.auto_crop_bottom_cr = p_Inp->output.auto_crop_bottom_cr;
    p_Vid->tempData4.format.auto_crop_right_cr  = p_Inp->output.auto_crop_right_cr;
    p_Vid->tempData4.format.yuv_format          = (ColorFormat) p_Vid->active_sps->chroma_format_idc;
  }
#endif

  img_in.pv_mfc_img_data =allocate_mfc_image_format();

  img_in.format            = p_Vid->tempData3.format;
  img_in.frm_stride[0] = p_pic->iLumaStride;
  img_in.frm_stride[1] = img_in.frm_stride[2] = p_pic->iChromaStride;
  ...
  if (p_pic->structure == FRAME)
  {
    ...
    if (p_Vid->mfc_rpu_data.rpu_field_processing_flag)
    {
      ...
      img_in.top_stride[0] = img_in.frm_stride[0] << 1;
      img_in.top_stride[1] = img_in.top_stride[2] = img_in.frm_stride[1] << 1;

      process_mfc_dec(p_Vid,
                      &img_in,
                      1); 
      ...
      itemp1 = img_in.bot_data[0][0];
      img_in.bot_data[0][0] += img_in.frm_stride[0];

      img_in.bot_data[1] = p_pic->imgUV[0] ;
      itemp2 = img_in.bot_data[1][0];
      img_in.bot_data[1][0] += img_in.frm_stride[1];

      img_in.bot_data[2] = p_pic->imgUV[1] ;
      itemp3 = img_in.bot_data[2][0];
      img_in.bot_data[2][0] += img_in.frm_stride[2];
      img_in.bot_stride[0] = img_in.frm_stride[0] << 1;
      img_in.bot_stride[1] = img_in.bot_stride[2] = img_in.frm_stride[1] << 1;
 
      process_mfc_dec(p_Vid,
                      &img_in,
                      2); 

      img_in.bot_data[0][0] = itemp1;
      img_in.bot_data[1][0] = itemp2;
      img_in.bot_data[2][0] = itemp3;
  ...
  else //If reference picture is a field, then frm_data will actually contain field data and therefore top/bottom stride is set accordingly.
  {
    if (p_pic->structure == TOP_FIELD)
    {
      ...
      img_in.top_data[1] = p_pic->imgUV[0];
      img_in.top_data[2] = p_pic->imgUV[1];
      img_in.top_stride[0] = img_in.frm_stride[0];
      img_in.top_stride[1] = img_in.top_stride[2] = img_in.frm_stride[1];
    ...
    else
    {
      ...
      img_in.bot_data[1] = p_pic->imgUV[0];
      img_in.bot_data[2] = p_pic->imgUV[1];
      img_in.bot_stride[0] = img_in.frm_stride[0];
      img_in.bot_stride[1] = img_in.bot_stride[2] =img_in.frm_stride[1];
  ...
  if(p_Inp->EnableDbgYUVFiles)
  {
        if(FRAME == p_pic->structure)
        {
        reset_mfc_image_format(&img_in,0);
        write_yuv_file_wrapper( p_Inp->fp_recon_baselayer,
                                img_in.pv_mfc_img_data);
        }
        else if(TOP_FIELD == p_pic->structure )
        {
        reset_mfc_image_format(&p_Vid->tempData4,1);
        copy_mfc_ImageData(p_Vid->tempData4.pv_mfc_img_data,img_in.pv_mfc_img_data);
        }
        else if(BOTTOM_FIELD == p_pic->structure )
        {
        reset_mfc_image_format(&p_Vid->tempData4,2);
        copy_mfc_ImageData(p_Vid->tempData4.pv_mfc_img_data,img_in.pv_mfc_img_data);

        reset_mfc_image_format(&p_Vid->tempData4,0);
        write_yuv_file_wrapper( p_Inp->fp_recon_baselayer,
                                p_Vid->tempData4.pv_mfc_img_data);
        }


        if((FRAME == p_pic->structure ) || (BOTTOM_FIELD == p_pic->structure ))
        {

            reset_mfc_image_format(&p_Vid->tempData3,0);    
            write_yuv_file_wrapper( p_Inp->fp_pred_enhlayer,
                                p_Vid->tempData3.pv_mfc_img_data);                
        }
  }


    free_mfc_format(img_in.pv_mfc_img_data);
}
#endif
int init_img_data(VideoParameters *p_Vid, ImageData *p_ImgData, seq_parameter_set_rbsp_t *sps)
{
...
#if (MFCD_REDUCED_RES)
  int low_res = 0; 
  int tmp_width, tmp_width_cr, tmp_height, tmp_height_cr; 
  if ( (p_Vid->width_ori > p_Vid->width) || (p_Vid->height_ori > p_Vid->height) )
  {
    low_res = 1;
    tmp_width = p_Vid->width;
    tmp_width_cr = p_Vid->width_cr;
    tmp_height = p_Vid->height;
    tmp_height_cr = p_Vid->height_cr;
    p_Vid->width = p_Vid->width_ori;
    p_Vid->width_cr = p_Vid->width_ori/2;
    p_Vid->height = p_Vid->height_ori;
    p_Vid->height_cr = p_Vid->height_ori/2;
  }
#endif
  ...
  #if(MFC_DEC_3D_FCFR)
   p_ImgData->pv_mfc_img_data        = allocate_mfc_image_format();     
 #endif

#if (MFCD_REDUCED_RES)
   if ( low_res )
   {
     p_Vid->width = tmp_width;
     p_Vid->width_cr = tmp_width_cr;
     p_Vid->height =  tmp_height;
     p_Vid->height_cr = tmp_height_cr;
   }
#endif
...
void free_img_data(VideoParameters *p_Vid, ImageData *p_ImgData)
{
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag)
  {
   ...
   #if(MFC_DEC_3D_FCFR)
   free_mfc_format(p_ImgData->pv_mfc_img_data);  
   #endif
...
#if (MFCD_REDUCED_RES)
  p_stored_pic->size_x_ori = p_pic->size_x;
  p_stored_pic->size_x_cr_ori = p_pic->size_x_cr_ori;
  p_stored_pic->size_y_ori = p_pic->size_y_cr_ori;
  p_stored_pic->size_y_cr_ori = p_pic->size_y_cr_ori;
#endif
