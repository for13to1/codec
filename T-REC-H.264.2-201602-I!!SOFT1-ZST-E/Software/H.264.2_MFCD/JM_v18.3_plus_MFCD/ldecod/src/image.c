...
#if (MFC_DEPTH_DEC)
  if ( (p_Vid->last_dec_layer_id != layer_id) || (p_Vid->last_dec_is_depth != is_depth) )
#else
...
#if (MFC_DEPTH_DEC)
	p_Vid->last_dec_is_depth = is_depth;
#endif
  }
}
...
#if (MFC_DEPTH_DEC)
  int is_depth = currSlice->is_depth;
  DecodedPictureBuffer *p_Dpb = p_Vid->p_Dpb_layer[is_depth][0];
#else
...
#if (MFCD_INTERLACE)
    p_Vid->structure = currSlice->structure;
#endif
...
#if MFC_DEC_3D_FCFR
static void init_mfc_picture(Slice *currSlice)
{
  ...
  {
    process_picture_in_dpb(p_Vid, p_pic);
    store_proc_picture_in_dpb (currSlice->p_Dpb, clone_storable_picture(p_Vid, p_pic));
  }
}
#endif
...
static void init_picture(VideoParameters *p_Vid, Slice *currSlice, InputParameters *p_Inp)
{
  ...
  {
#if (MFC_DEPTH_DEC)
    if((p_Vid->active_sps->profile_idc == MVC_HIGH) || (p_Vid->active_sps->profile_idc == STEREO_HIGH) || (p_Vid->active_sps->profile_idc == ThreeDV_HIGH) || (p_Vid->active_sps->profile_idc == MFC_DEPTH_HIGH))
#else
    ...
    #if(MFC_DEC_3D_FCFR)
#if (MFC_DEPTH_DEC)
	if(p_Vid->active_sps->profile_idc == MFC3D_HIGH)
#else
     if(p_Vid->profile_idc == MFC3D_HIGH)
#endif
      init_mfc_picture(currSlice);
    #endif
  }
#endif
...
static void init_picture_decoding(VideoParameters *p_Vid)
{
  ...
  {
   idr_memory_management(p_Vid->p_Dpb_layer[is_depth][pSlice->layer_id], p_Vid->dec_picture);
  }
  update_ref_list(p_Vid->p_Dpb_layer[is_depth][pSlice->view_id]);
  update_ltref_list(p_Vid->p_Dpb_layer[is_depth][pSlice->view_id]);
#else
  if((pSlice->layer_id>0) && (pSlice->svc_extension_flag == 0 && pSlice->NaluHeaderMVCExt.non_idr_flag == 0))
  {
  ...
  //set;
  if(p_Vid->dec_picture->structure==FRAME || p_Vid->dec_picture->structure==TOP_FIELD)
  {
    CodingParameters *p_EncPar = p_Vid->p_EncodePar[p_Vid->dec_picture->layer_id];
    p_EncPar->iInterViewMBs[0] = p_EncPar->iInterViewMBs[1] = p_EncPar->iInterViewMBs[2] = 0;
  }
  if(p_Vid->p_EncodePar[p_Vid->dpb_layer_id]->fpMBStat)
  {
    calculate_frame_no(p_Vid, p_Vid->dec_picture);
    fprintf(p_Vid->p_EncodePar[p_Vid->dpb_layer_id]->fpMBStat, "FrameNum=%d", p_Vid->frame_no); //p_Vid->dec_picture->structure==FRAME? (p_Vid->dec_picture->frame_poc/2): (p_Vid->dec_picture->top_poc/2));
  }
#endif
...
#if MFC_DEC_3D_FCFR_STAT 
  currSlice->iInterViewMBs[0] = currSlice->iInterViewMBs[1] = currSlice->iInterViewMBs[2] = 0;
#endif
}
...
#if (MFC_DEPTH_DEC)
	currSlice->p_Dpb = p_Vid->p_Dpb_layer[0][0]; //set default value;
#else
...
#if (MVC_EXTENSION_ENABLE)
#if (MFC_DEPTH_DEC)
    if(p_Inp->DecodeAllLayers == 1 && (nalu->nal_unit_type == NALU_TYPE_PREFIX || 
...
#if (MFC_DEPTH_DEC)
      currSlice->is_depth=(nalu->nal_unit_type!=NALU_TYPE_3DV_EXT)?0:1;
#endif
    }
#endif
...
process_nalu:
    ...
    case NALU_TYPE_3DV_EXT:
      ...
      currSlice->p_Dpb=p_Vid->p_Dpb_layer[currSlice->is_depth][0];
#endif
    ...
    if(currSlice->svc_extension_flag == 0)
      ...
      */
      ...
      {
	  #if (MFC_DEPTH_DEC)
	    currSlice->p_Dpb = p_Vid->p_Dpb_layer[currSlice->is_depth][currSlice->view_id];
	  #else
        currSlice->p_Dpb = p_Vid->p_Dpb_layer[currSlice->view_id];
	  #endif
      }
...
void dpb_combine_post_field_yuv(VideoParameters *p_Vid, FrameStore *fs)
{
  int i, j;
  ...
  {
    memcpy(fs->frame->postY[i*2],     fs->top_field->postY[i]   , fs->top_field->size_x * sizeof(imgpel));     // top field
    memcpy(fs->frame->postY[i*2 + 1], fs->bottom_field->postY[i], fs->bottom_field->size_x * sizeof(imgpel)); // bottom field
  }
  ...
  for (j = 0; j < 2; j++)
  {
    ...
    {
      memcpy(fs->frame->postUV[j][i*2],     fs->top_field->postUV[j][i],    fs->top_field->size_x_cr*sizeof(imgpel));
      memcpy(fs->frame->postUV[j][i*2 + 1], fs->bottom_field->postUV[j][i], fs->bottom_field->size_x_cr*sizeof(imgpel));
    }
  }
}

void set_jm_ImageData(ImageData *inImg,StorablePicture* p_pic )
  {
    ...
    inImg->frm_stride[0] = p_pic->iLumaStride;
    inImg->frm_stride[1] = inImg->frm_stride[2] = p_pic->iChromaStride;


    inImg->frm_data[0] = p_pic->imgY;
    inImg->frm_data[1] = p_pic->imgUV[0];
    inImg->frm_data[2] = p_pic->imgUV[1];
  

    inImg->format.width[0] = p_pic->size_x;
    inImg->format.width[1] = p_pic->size_x_cr;
    inImg->format.width[2] = p_pic->size_x_cr;

    inImg->format.height[0] = p_pic->size_y;
    inImg->format.height[1] = p_pic->size_y_cr;
    inImg->format.height[2] = p_pic->size_y_cr;

    inImg->format.auto_crop_right   = SubWidthC[p_pic->chroma_format_idc] * p_pic->frame_crop_right_offset;
    inImg->format.auto_crop_bottom  = SubWidthC[p_pic->chroma_format_idc] * p_pic->frame_crop_bottom_offset;

    if(p_pic->structure == FRAME)
    {
            inImg->format.auto_crop_bottom *= (2-p_pic->frame_mbs_only_flag);
    }


    inImg->format.auto_crop_right_cr = inImg->format.auto_crop_right >>1;
    inImg->format.auto_crop_bottom_cr = inImg->format.auto_crop_bottom >>1;

    reset_mfc_image_format(inImg,0);

  }

static void export_left_right_views(VideoParameters *p_Vid, PictureStructure structure , int poc)
{  
  ...
  //FrameStore *BL_proc_fs = NULL;
  FrameStore *BL_frame = NULL;
  FrameStore *EL_frame = NULL;
  StorablePicture* BL_picture = NULL;
  StorablePicture* EL_picture = NULL;
  int field_reconstruction;

  ImageData img_in_bl,img_in_el,img_out_view0,img_out_view1;

  imgpel **temp1;
  imgpel ***temp2;
  
  ImageData *EL_used_in_recon;

#if (MFC_DEPTH_DEC)
  int is_depth = 0; 
#endif
  
  EL_used_in_recon = &p_Vid->tempData3;            
  ...
  if (structure == FRAME)
  {
    //Find BL reconstructed picture
#if (MFC_DEPTH_DEC)
	p_Dpb = p_Vid->p_Dpb_layer[is_depth][0];
#else
    p_Dpb = p_Vid->p_Dpb_layer[0];
#endif
    for (i = 0; i < p_Dpb->used_size; i++)
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->frame;
      if ((picture->view_id == 0) && (picture->frame_poc == poc))
      {
          BL_frame = fs;
          BL_picture = picture;
          break;        
...
#if (MFC_DEPTH_DEC)
	p_Dpb = p_Vid->p_Dpb_layer[is_depth][1];
#else
    p_Dpb = p_Vid->p_Dpb_layer[1];
#endif
    for (i = 0; i < p_Dpb->used_size; i++)
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->frame;
      if ((picture->view_id == 1) && (picture->frame_poc == poc))
      {
        EL_frame = fs;
        EL_picture = picture;
        break;
  ...
  else if (structure == TOP_FIELD)
  {
    ...
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->top_field;
      if ((picture->view_id == 0) && (picture->top_poc == poc))
      {
          BL_frame = fs;
          BL_picture = picture;
          break;
    ...
    for (i = 0; i < p_Dpb->used_size; i++)
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->top_field;
       if ((picture->view_id == 1) && (picture->top_poc == poc))
      {
        EL_frame = fs;
        EL_picture = picture;
      }
  ...
  else //bottom field
  {
    ...
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->bottom_field;
      if ((picture->view_id == 0) && (picture->bottom_poc == poc))
      {
          BL_frame = fs;
          BL_picture = picture;
          break;
    ...
    for (i = 0; i < p_Dpb->used_size; i++)
    {
      FrameStore *fs = p_Dpb->fs[i];
      StorablePicture *picture = fs->bottom_field;
       if ((picture->view_id == 1) && (picture->bottom_poc == poc))
      {
        EL_frame = fs;
        EL_picture = picture;
        break;
  ...
  #if MFC_PROFILING
  gettime (&(p_Vid->mfc_start_time));              
  #endif



  img_in_bl.pv_mfc_img_data =allocate_mfc_image_format();  
  set_jm_ImageData(&img_in_bl,BL_picture );

  img_in_el.pv_mfc_img_data =allocate_mfc_image_format();  
  set_jm_ImageData(&img_in_el,EL_picture );  

  temp1 = BL_picture->imgY;
  temp2 = BL_picture->imgUV;
  BL_picture->imgY = BL_picture->postY;
  BL_picture->imgUV = BL_picture->postUV;
  img_out_view0.pv_mfc_img_data =allocate_mfc_image_format();  
  set_jm_ImageData(&img_out_view0,BL_picture);
  BL_picture->imgY = temp1;
  BL_picture->imgUV = temp2;

  temp1 = EL_picture->imgY;
  temp2 = EL_picture->imgUV;
  EL_picture->imgY = EL_picture->postY;
  EL_picture->imgUV = EL_picture->postUV;
  img_out_view1.pv_mfc_img_data =allocate_mfc_image_format();  
  set_jm_ImageData(&img_out_view1,EL_picture);
  EL_picture->imgY = temp1;
  EL_picture->imgUV = temp2;

  reset_mfc_image_format(EL_used_in_recon,BL_picture->structure);


  field_reconstruction = (FRAME == structure && p_Vid->mfc_rpu_data.rpu_field_processing_flag);


   process3DDeMuxWrapper(p_Vid->pv_mfc_decoder_layer_context,
                        img_out_view0.pv_mfc_img_data,
                        img_out_view1.pv_mfc_img_data,
                        img_in_bl.pv_mfc_img_data,                        //recon BL
                        img_in_el.pv_mfc_img_data,                        //recon EL                                                
                        EL_used_in_recon->pv_mfc_img_data,                  //Carrier EL
                        p_Vid->p_Inp->DeMuxMode,
  ...
  #if MFC_PROFILING
  gettime (&(p_Vid->mfc_end_time));              
  p_Vid->mfc_reconstruction_time += timediff(&(p_Vid->mfc_start_time), &(p_Vid->mfc_end_time));    
  #endif
 
   if(BOTTOM_FIELD == BL_picture->structure)
   {
       dpb_combine_post_field_yuv(p_Vid,BL_frame);
       dpb_combine_post_field_yuv(p_Vid,EL_frame);
   }

    free_mfc_format(img_in_bl.pv_mfc_img_data);
    free_mfc_format(img_in_el.pv_mfc_img_data);
    free_mfc_format(img_out_view0.pv_mfc_img_data);
    free_mfc_format(img_out_view1.pv_mfc_img_data);
...
#if (MFC_DEPTH_DEC)
static void export_copy_views(VideoParameters *p_Vid, PictureStructure structure , int poc)
{  
  ...
  //field_reconstruction = structure;//(FRAME == structure && p_Vid->mfc_rpu_data.rpu_field_processing_flag);
  field_reconstruction = (structure == FRAME )? 0 : 1;

   process3DCopyWrapper(p_Vid->pv_mfc_decoder_layer_context,
                        img_out_view0.pv_mfc_img_data,
                        img_out_view1.pv_mfc_img_data,
                        img_in_bl.pv_mfc_img_data,                        //recon BL
                        img_in_el.pv_mfc_img_data,                        //recon EL                                                
...
void exit_picture(VideoParameters *p_Vid, StorablePicture **dec_picture)
{
  ...
  frame recfr;
#endif
  int frame_poc, slice_type, refpic, qp, pic_num, chroma_format_idc, is_idr,top_poc,bottom_poc;
...
#if MVC_EXTENSION_ENABLE
#if (MFC_DEPTH_DEC)
  store_picture_in_dpb(p_Vid->p_Dpb_layer[is_depth][(*dec_picture)->view_id], *dec_picture);
#else
  ...
  store_picture_in_dpb(p_Vid->p_Dpb_layer[0], *dec_picture);
    ...
    #if(MFC_DEC_3D_FCFR)
    {
        if(p_Inp->export_views)
        {
            int profile_idc = (p_Vid->active_subset_sps && p_Vid->active_subset_sps->sps.Valid)?p_Vid->active_subset_sps->sps.profile_idc : p_Vid->active_sps->profile_idc;
            if (profile_idc==MFC3D_HIGH && ((*dec_picture)->view_id == 1))
            {
                export_left_right_views(p_Vid, structure,(structure == FRAME) ? frame_poc : ((structure == TOP_FIELD) ? top_poc : bottom_poc));
            }
#if (MFC_DEPTH_DEC)
			else if(profile_idc==MFC_DEPTH_HIGH && (*dec_picture)->view_id == 1)
			{
				export_copy_views(p_Vid, structure,(structure == FRAME) ? frame_poc : ((structure == TOP_FIELD) ? top_poc : bottom_poc));
			}
#endif
        }
     }
...
#if MFC_DEC_3D_FCFR_STAT
void collect_interview(VideoParameters *p_Vid)
{
  int j;
  if(p_Vid->dpb_layer_id>0)
  {
    CodingParameters *pEncPar = p_Vid->p_EncodePar[p_Vid->dpb_layer_id];
    int iSliceType = iClip3(0, 2, p_Vid->dec_picture->slice_type);
    for(j=0; j<p_Vid->iSliceNumOfCurrPic; j++)
    {
      pEncPar->iInterViewMBs[0] += p_Vid->ppSliceList[j]->iInterViewMBs[0];
      pEncPar->iInterViewMBs[1] += p_Vid->ppSliceList[j]->iInterViewMBs[1];
      pEncPar->iInterViewMBs[2] += p_Vid->ppSliceList[j]->iInterViewMBs[2];
    }
    if(p_Vid->dec_picture->structure == FRAME || p_Vid->dec_picture->structure == BOTTOM_FIELD)
    {
      pEncPar->iTotalInterViewMBs[iSliceType][0] += pEncPar->iInterViewMBs[0];
      pEncPar->iTotalInterViewMBs[iSliceType][1] += pEncPar->iInterViewMBs[1];
      pEncPar->iTotalInterViewMBs[iSliceType][2] += pEncPar->iInterViewMBs[2];
    }
    if(p_Vid->dec_picture->structure == FRAME || p_Vid->dec_picture->structure == TOP_FIELD)
      pEncPar->iFrmsNum[iSliceType]++;
  }
}
#endif
