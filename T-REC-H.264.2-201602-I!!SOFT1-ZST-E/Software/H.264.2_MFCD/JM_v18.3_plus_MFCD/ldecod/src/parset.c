...
#if MFC_DEC_3D_FCFR
void read_sps_rpu_data(MFC_RPU_DATA *p_mfc_rpudata,Bitstream *bitstream ,int *len,int frame_mbs_only, int profile_idc)
{

   if(MFC3D_HIGH == profile_idc)
   {
        p_mfc_rpudata->mfc_format_idc    = read_u_v(6, "SPS:MFC mfc_format_idc", bitstream, &p_Dec->UsedBits);
    
        if(p_mfc_rpudata->mfc_format_idc==0 || p_mfc_rpudata->mfc_format_idc==1)
        {            
            p_mfc_rpudata->default_grid_position_flag = read_u_1  ("SPS: MFC default_grid_position_flag"    ,bitstream, &p_Dec->UsedBits);
            if(!p_mfc_rpudata->default_grid_position_flag)
            {
                p_mfc_rpudata->view0_grid_position_x= read_u_v(4,"SPS:MFC view0_grid_position_x"            ,bitstream, &p_Dec->UsedBits);
                p_mfc_rpudata->view0_grid_position_y= read_u_v(4,"SPS:MFC view0_grid_position_y"            ,bitstream, &p_Dec->UsedBits);
                p_mfc_rpudata->view1_grid_position_x= read_u_v(4,"SPS:MFC view1_grid_position_x"            ,bitstream, &p_Dec->UsedBits);
                p_mfc_rpudata->view1_grid_position_y= read_u_v(4,"SPS:MFC view1_grid_position_y"            ,bitstream, &p_Dec->UsedBits);
            }

        }
        
        p_mfc_rpudata->rpu_filter_enabled_flag    = read_u_1  ("SPS: MFC rpu_filter_enabled_flag"            ,bitstream, &p_Dec->UsedBits);

        if(!frame_mbs_only)
        {
            p_mfc_rpudata->rpu_field_processing_flag= read_u_1  ("SPS: MFC rpu_field_processing_flag"        ,bitstream,&p_Dec->UsedBits);        
        }
    }
...
int InterpretSPS (VideoParameters *p_Vid, DataPartition *p, seq_parameter_set_rbsp_t *sps)
{
  ...
  if ((sps->profile_idc!=BASELINE       ) &&
      ...
      #if (MFC_DEC_3D_FCFR)
      && (sps->profile_idc!=MFC3D_HIGH)
    #endif
#if (MFC_DEPTH_DEC)
      && (sps->profile_idc!=ThreeDV_HIGH)
	  && (sps->profile_idc!=MFC_DEPTH_HIGH)
#endif	
#endif
      )
  {
...
#if (MFC_DEPTH_DEC)
  else if((subset_sps->sps.profile_idc==ThreeDV_HIGH)||(subset_sps->sps.profile_idc==MFC_DEPTH_HIGH))
  {
	  subset_sps->bit_equal_to_one = read_u_1("bit_equal_to_one"											, s, &p_Dec->UsedBits);
	  ...
	  subset_sps->threedvc_vui_parameters_present_flag = read_u_1("3dvc_vui_parameters_present_flag"			, s, &p_Dec->UsedBits);
	  if(subset_sps->threedvc_vui_parameters_present_flag)
		  mvc_vui_parameters_extension(&(subset_sps->MVCVUIParams),s);
	  subset_sps->texture_vui_parameters_present_flag = read_u_1("texture_vui_parameters_present_flag"			, s, &p_Dec->UsedBits);
...
#if MFCD_INTERLACE
    if ( (sps->is_depth==0) && (sps->profile_idc!=ThreeDV_HIGH) && (sps->profile_idc!=MFC_DEPTH_HIGH))
...
void ProcessSubsetSPS (VideoParameters *p_Vid, NALU_t *nalu)
{
  ...
  {
#if (MFC_DEPTH_DEC)
	OpenOutputFiles(p_Vid, subset_sps->view_id[0], subset_sps->view_id[1], 0);
	OpenOutputFiles(p_Vid, subset_sps->view_id[0], subset_sps->view_id[1], 1);
#else
...
void reset_format_info(seq_parameter_set_rbsp_t *sps, VideoParameters *p_Vid, FrameFormat *source, FrameFormat *output)
{
  ...
  int crop_left, crop_right,crop_left_uv, crop_right_uv;
  int crop_top, crop_bottom,crop_top_uv,crop_bottom_uv;
  ...
  output->width[0]  = p_Vid->width - crop_left - crop_right;
  output->height[0] = p_Vid->height - crop_top - crop_bottom;

  if (sps->frame_cropping_flag)
  {
    crop_left_uv   = sps->frame_crop_left_offset;
    crop_right_uv  = sps->frame_crop_right_offset;
    crop_top_uv    = ( 2 - sps->frame_mbs_only_flag ) *  sps->frame_crop_top_offset;
    crop_bottom_uv = ( 2 - sps->frame_mbs_only_flag ) *  sps->frame_crop_bottom_offset;
  }
  else
  {
    crop_left_uv = crop_right_uv = crop_top_uv = crop_bottom_uv = 0;
  }
  #else
   // cropping for chroma
  if (sps->frame_cropping_flag)
  ...
  {
    #if MFC_JM_CODE_CHANGE
    source->width[1]  = p_Vid->width_cr - crop_left_uv - crop_right_uv;
    source->width[2]  = source->width[1];
    source->height[1] = p_Vid->height_cr - crop_top_uv - crop_bottom_uv;
    source->height[2] = source->height[1];

    output->width[1]  = p_Vid->width_cr - crop_left_uv - crop_right_uv;
    output->width[2]  = output->width[1];
    output->height[1] = p_Vid->height_cr - crop_top_uv - crop_bottom_uv;
...
static void setup_layer_info(VideoParameters *p_Vid, seq_parameter_set_rbsp_t *sps, LayerParameters *p_Lps)
{
...
#if (MFC_DEPTH_DEC)
  int is_depth = sps->is_depth; 
#endif 
  p_Lps->p_Vid = p_Vid;

#if (MFC_DEPTH_DEC)
  p_Lps->p_Cps = p_Vid->p_EncodePar[is_depth][layer_id];
  p_Lps->p_SPS = sps;
  p_Lps->p_Dpb = p_Vid->p_Dpb_layer[is_depth][layer_id];
#else
  ...
  if(p_Vid->p_Inp->iMBStat==2)
  {
    CodingParameters *pCodingParam = p_Vid->p_EncodePar[layer_id];
    if(!pCodingParam->fpMBStat)
    {
      char strFN[256], strFullPath[256];
      char *pCh = strrchr(p_Vid->p_Inp->infile, '\\');
      if(!pCh)
        pCh = p_Vid->p_Inp->infile;
      strcpy(strFN, pCh);
      pCh = strrchr(strFN, '.');
      if(pCh)
        *pCh = '\0';
      sprintf(strFullPath, "%s_mbstat_layer%d.txt", strFN, layer_id);
      pCodingParam->fpMBStat = fopen(strFullPath, "w");
    }
  }
#endif
}
...
#if (MFC_DEPTH_DEC)
static void set_coding_par(seq_parameter_set_rbsp_t *sps, CodingParameters *cps, int is_depth)
#else
...
#endif
{
  ...
  cps->FrameSizeInMbs = cps->PicWidthInMbs * cps->FrameHeightInMbs;
    ...
    if (is_depth==0)
    {
      cps->text_width=cps->PicWidthInMbs*MB_BLOCK_SIZE;
      cps->text_height=cps->FrameHeightInMbs*MB_BLOCK_SIZE;
      if (sps->chroma_format_idc == YUV420)
      {
        cps->text_width_cr  = (cps->text_width  >> 1);
        cps->text_height_cr = (cps->text_height >> 1);
      }
      else if (sps->chroma_format_idc == YUV422)
      {
        cps->text_width_cr  = (cps->text_width >> 1);
        cps->text_height_cr = cps->text_height;
      }
      else if (sps->chroma_format_idc== YUV444)
      {
        //YUV444
        cps->text_width_cr = cps->text_width;
        cps->text_height_cr = cps->text_height;
      }
      
#if MFCD_INTERLACE
      if ( (sps->profile_idc!=ThreeDV_HIGH) && (sps->profile_idc!=MFC_DEPTH_HIGH) )
        cps->is_texture_frame = sps->frame_mbs_only_flag;
#endif
    }
    else
    {
      ...
      cps->depth_width  = cps->PicWidthInMbs * MB_BLOCK_SIZE ;
      cps->depth_height = cps->FrameHeightInMbs * MB_BLOCK_SIZE ;

      

      if (sps->chroma_format_idc == YUV420)
      {
        cps->depth_width_cr  = (cps->depth_width  >> 1);
        cps->depth_height_cr = (cps->depth_height >> 1);
      }
      else if (sps->chroma_format_idc == YUV422)
      {
        cps->depth_width_cr  = (cps->depth_width >> 1);
        cps->depth_height_cr = cps->depth_height;
      }
      else if (sps->chroma_format_idc== YUV444)
      {
        //YUV444
        cps->depth_width_cr = cps->depth_width;
        cps->depth_height_cr = cps->depth_height;
      }
...
#endif
#if (MFC_DEPTH_DEC)
  if (p_Vid->active_sps != sps || sps->profile_idc == ThreeDV_HIGH || sps->profile_idc == MFC_DEPTH_HIGH)
  {
	...
	if(p_Vid->dpb_layer_id==0 && is_BL_profile(sps->profile_idc) && !p_Vid->p_Dpb_layer[is_depth][0]->init_done)
    {
      set_coding_par(sps, p_Vid->p_EncodePar[is_depth][0], is_depth);
      setup_layer_info( p_Vid, sps, p_Vid->p_LayerPar[is_depth][0]);
     
      if (is_depth == 1)
      {
        set_coding_par(sps, p_Vid->p_EncodePar[is_depth][1], is_depth);
        setup_layer_info( p_Vid, sps, p_Vid->p_LayerPar[is_depth][1]);
      }
    }
    else if(p_Vid->dpb_layer_id==1 && is_EL_profile(sps->profile_idc) && !p_Vid->p_Dpb_layer[is_depth][1]->init_done)
    {
      set_coding_par(sps, p_Vid->p_EncodePar[is_depth][1], is_depth);
      setup_layer_info(p_Vid, sps, p_Vid->p_LayerPar[is_depth][1]);
    }
...
//to be removed in future;
    set_global_coding_par(p_Vid, p_Vid->p_EncodePar[is_depth][p_Vid->dpb_layer_id]);
//end;
	...
	if (/*p_Vid->last_pic_width_in_mbs_minus1 != p_Vid->active_sps->pic_width_in_mbs_minus1
		...
		|| */(p_Vid->last_profile_idc != p_Vid->active_sps->profile_idc && is_BL_profile(p_Vid->active_sps->profile_idc) && !p_Vid->p_Dpb_layer[is_depth][0]->init_done /*&& is_BL_profile(p_Vid->last_profile_idc)*/))
	{
		//init_frext(p_Vid);
		init_global_buffers(p_Vid, 0, is_depth);

		if (!p_Vid->no_output_of_prior_pics_flag)
		{
			flush_dpb(p_Vid->p_Dpb_layer[is_depth][0]);
			flush_dpb(p_Vid->p_Dpb_layer[is_depth][1]);
		}
		init_dpb(p_Vid, p_Vid->p_Dpb_layer[is_depth][0], 1);
	}
		...
		|| ( is_MVCD_profile(p_Vid->active_sps->profile_idc) && p_Vid->dpb_layer_id==1) ) 
		&& (!p_Vid->p_Dpb_layer[is_depth][1]->init_done))
	{
		assert(p_Vid->p_Dpb_layer[is_depth][0]->init_done);
		//init_frext(p_Vid);
#if (!MFC_DEPTH_DEC)
		if(p_Vid->p_Dpb_layer[is_depth][0]->init_done)
		{
			free_dpb(p_Vid->p_Dpb_layer[is_depth][0]);
			init_dpb(p_Vid, p_Vid->p_Dpb_layer[is_depth][0], 1);
		}
#endif
		init_global_buffers(p_Vid, 1, is_depth);
...
#if MVC_EXTENSION_ENABLE
		init_dpb(p_Vid, p_Vid->p_Dpb_layer[is_depth][1], 2);
#endif
  ...
  if (!is_depth)
  {
    ...
    p_Vid->height_ori= (p_Vid->active_sps->pic_height_in_map_units_minus1+1)*(2-p_Vid->active_sps->frame_mbs_only_flag)*MB_BLOCK_SIZE;
    p_Vid->width_ori = (p_Vid->active_sps->pic_width_in_mbs_minus1+1)*MB_BLOCK_SIZE;
  }
#endif
#if MFCD_INTERLACE
    if ( (p_Vid->active_sps->is_depth==0) && (p_Vid->active_sps->profile_idc!=ThreeDV_HIGH) && (p_Vid->active_sps->profile_idc!=MFC_DEPTH_HIGH) )
      p_Vid->is_texture_frame = sps->frame_mbs_only_flag;
#endif
#else
	//init_frext(p_Vid);
...
void seq_parameter_set_mvc_extension(subset_seq_parameter_set_rbsp_t *subset_sps, Bitstream *s)
{
...
#if (MFC_DEPTH_DEC)
    if (subset_sps->sps.profile_idc==ThreeDV_HIGH || subset_sps->sps.profile_idc==MFC_DEPTH_HIGH)
		{
			subset_sps->depth_view_present_flag[i] = read_u_1 ("depth_view_present_flag"					, s, &p_Dec->UsedBits);
			subset_sps->num_coded_depth_view += subset_sps->depth_view_present_flag[i];
			subset_sps->texture_view_present_flag[i] = read_u_1 ("texture_view_present_flag"					, s, &p_Dec->UsedBits);
		}
#endif
  }
  ...
  {
#if (MFC_DEPTH_DEC)
    if ( (subset_sps->sps.profile_idc==ThreeDV_HIGH ||subset_sps->sps.profile_idc==MFC_DEPTH_HIGH) && subset_sps->depth_view_present_flag[i]==0) continue;
#endif
      ...
      for(j=0; j<=subset_sps->num_applicable_ops_minus1[i]; j++)
      {
        ...
        if(subset_sps->applicable_op_num_target_views_minus1[i][j]>=0)
        {
          ...
          for(k = 0; k <= subset_sps->applicable_op_num_target_views_minus1[i][j]; k++)
          {
            ...
            if (subset_sps->sps.profile_idc==MFC_DEPTH_HIGH || subset_sps->sps.profile_idc==ThreeDV_HIGH)
            {
              subset_sps->applicable_op_depth_flag[i][j][k] = read_u_1("applicable_op_depth_flag"				, s, &p_Dec->UsedBits);
              subset_sps->applicable_op_texture_flag[i][j][k] = read_u_1("applicable_op_texture_flag"				, s, &p_Dec->UsedBits);
            }
          }
#else
          if ((subset_sps->applicable_op_target_view_id[i][j] = (int*) calloc(1+subset_sps->applicable_op_num_target_views_minus1[i][j], sizeof(int))) == NULL)
		...
		if (subset_sps->sps.profile_idc==ThreeDV_HIGH || subset_sps->sps.profile_idc==MFC_DEPTH_HIGH)
		{
			subset_sps->applicable_op_num_texture_views_minus1[i][j] = read_ue_v("applicable_op_num_texture_views_minus1"					, s, &p_Dec->UsedBits);
			subset_sps->applicable_op_num_depth_views[i][j] = read_ue_v("applicable_op_num_depth_views"					, s, &p_Dec->UsedBits);
		}
		else
#endif
  ...
  #if MFC_DEC_3D_FCFR              
    read_sps_rpu_data(&p_Dec->p_Vid->mfc_rpu_data,s,&p_Dec->UsedBits,subset_sps->sps.frame_mbs_only_flag,subset_sps->sps.profile_idc);
    
    if(set_mfc_rpu_data(&p_Dec->p_Vid->mfc_rpu_data, p_Dec->p_Vid->pv_mfc_rpu_data) !=0)
    {
    error("\nMFC HIGH PROFILE : set_mfc_rpu_data() Failed",400);
    }

    initialize_mfc_decoder_wrapper(p_Dec->p_Vid->pv_mfc_decoder_layer_context,p_Dec->p_Vid->pv_mfc_rpu_data);
	...
	if(subset_sps && subset_sps->num_views_minus1>=0)
	{
...
#if (MFC_DEPTH_DEC)
		free_pointer(subset_sps->depth_view_present_flag);
		free_pointer(subset_sps->texture_view_present_flag);
#endif
		...
		if(subset_sps->num_level_values_signalled_minus1 >= 0)
		{
			...
			for(i=0; i<=subset_sps->num_level_values_signalled_minus1; i++)
			{
...
#if (MFC_DEPTH_DEC)
					free_pointer(subset_sps->applicable_op_depth_flag[i][j]);
					free_pointer(subset_sps->applicable_op_texture_flag[i][j]);
#endif
				}
				free_pointer(subset_sps->applicable_op_target_view_id[i]);
#if (MFC_DEPTH_DEC)
				free_pointer(subset_sps->applicable_op_depth_flag[i]);
				free_pointer(subset_sps->applicable_op_texture_flag[i]);
#endif
				...
				if (subset_sps->sps.profile_idc==ThreeDV_HIGH || subset_sps->sps.profile_idc==MFC_DEPTH_HIGH)
				{
					free_pointer(subset_sps->applicable_op_num_texture_views_minus1[i]);
					free_pointer(subset_sps->applicable_op_num_depth_views[i]);
				}
				else
#else
...
#if (MFC_DEPTH_DEC)
				free_pointer(subset_sps->applicable_op_depth_flag);
				free_pointer(subset_sps->applicable_op_texture_flag);
#endif
				...
				if (subset_sps->sps.profile_idc==ThreeDV_HIGH || subset_sps->sps.profile_idc==MFC_DEPTH_HIGH)
				{
					free_pointer(subset_sps->applicable_op_num_texture_views_minus1);
					free_pointer(subset_sps->applicable_op_num_depth_views);
				}
				else
#endif
