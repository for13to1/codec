...
void write_out_picture(StorablePicture *p, FrameFormat *output, int p_out)
{
  ...
  if(p->is_depth && (p->NonlinearDepthNum>0))
  {
  ...
  else
  {
    ...
    img2buf (imgX_linear_Y, buf, p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
