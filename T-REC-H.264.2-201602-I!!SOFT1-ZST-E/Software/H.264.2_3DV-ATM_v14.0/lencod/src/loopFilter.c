...
static void DeblockMb(VideoParameters *p_Vid, imgpel **imgY, imgpel ***imgUV, int MbQAddr)
{
  ...
  for (edge = 0; edge < 4 ; ++edge )
  {
    ...
    if (MbQ->cbp == 0)
    {
      ...
      {
#if EXT3D
        if (((MbQ->mb_type == PSKIP || MbQ->mb_type == PVSP_SKIP) && currSlice->slice_type == P_SLICE) || (MbQ->mb_type == 1) || (MbQ->mb_type == 2))
          continue;
        else if ((edge & 0x01) && ((MbQ->mb_type == 3) || ((edge & 0x01) && (MbQ->mb_type == BSKIP_DIRECT || MbQ->mb_type == BVSP_DIRECT) && currSlice->slice_type == B_SLICE && active_sps->direct_8x8_inference_flag)))
          continue;
#else
  ...
  for( edge = 0; edge < 4 ; ++edge )
  {
    ...
    if (MbQ->cbp == 0)
    {
      ...
      {
#if EXT3D
        if ((((MbQ->mb_type == PSKIP || MbQ->mb_type == PVSP_SKIP) && currSlice->slice_type == P_SLICE) || (MbQ->mb_type == P16x16) || (MbQ->mb_type == P8x16)))
          continue;
        else if ((edge & 0x01) && (( MbQ->mb_type == P16x8) || (currSlice->slice_type == B_SLICE && (MbQ->mb_type == BSKIP_DIRECT || MbQ->mb_type == BVSP_DIRECT) && active_sps->direct_8x8_inference_flag)))
          continue;
#else
