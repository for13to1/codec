...
void ProfileCheck(InputParameters *p_Inp)
{
  if((p_Inp->ProfileIDC != BASELINE ) &&
   ...
   (p_Inp->ProfileIDC != ThreeDV_HIGH)     &&  // 3dv profile
   (p_Inp->ProfileIDC !=ThreeDV_EXTEND_HIGH)&&
...
void LevelCheck(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if(p_Vid->is_depth)
  {
    if((p_Inp->ProfileIDC!=ThreeDV_HIGH)&&(p_Inp->ProfileIDC!=ThreeDV_EXTEND_HIGH))
    {
      snprintf(errortext,ET_SIZE,"The ProfileIDc for depth shall be set to 138 or 139");
      error(errortext,500);
    }
  }
  else
  {
    if((0==p_Inp->ThreeDVCoding)||(0==p_Inp->CompatibilityCategory))
    {
      if((p_Inp->ProfileIDC==ThreeDV_HIGH)||(p_Inp->ProfileIDC==ThreeDV_EXTEND_HIGH) )
      {
        snprintf(errortext,ET_SIZE,"The profileIDC cannot be larger than 128 for Multi-view compatible");
        error(errortext,500);
      }
    }
  }
...
#if EXT3D
  if (mode==SI4MB || mode==I4MB || mode==I16MB || mode==I8MB || mode==IPCM)
#else
  if (mode > P8x8)
#endif
    return invalid_mode;
...
#if EXT3D
    if ( ( currSlice->weighted_prediction == 1 || currSlice->depth_range_wp ) && (mode < P8x8 || mode == BVSP_DIRECT) )
#else
