...
#if EXT3D 
  // @DT: safe checking added, compared to JM implemntation
  if(p_me_ffast)
  {
    for (list=0; list<2; list++)
    {
    ...
    for (list=0; list<2; list++)
    {
      ...
      if(p_me_ffast->search_center_padded[list])
        free (p_me_ffast->search_center_padded[list]);
      if(p_me_ffast->search_center[list])
    ...
    if(p_me_ffast->search_setup_done)
    ...
    if(p_me_ffast->pos_00)
      free (p_me_ffast->pos_00);
    if(p_me_ffast->max_search_range)
...
#if EXT3D
  int     apply_weights = ( ((p_Vid->active_pps->weighted_pred_flag||p_Vid->p_Inp->DepthRangeBasedWP) && (currSlice->slice_type == P_SLICE || (currSlice->slice_type == SP_SLICE))) ||
    ((p_Vid->active_pps->weighted_bipred_idc||p_Vid->p_Inp->DepthRangeBasedWP) && (currSlice->slice_type == B_SLICE))) && p_Inp->UseWeightedReferenceME;
#else
...
#if EXT3D
  check_neighbors(currMB,block,p_Vid->enc_picture->mv_info,list,ref);
