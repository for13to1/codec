...
#if EXT3D 
  memset (currSlice->std_mvd, 0, BLOCK_CONTEXT * sizeof(short));
#endif
...
#if EXT3D 
  currMB->is_alc_mode    = 0;    // turn off ALC mode
  currSlice->is_alc_mode = 0;    // clear ALC selection flag
#endif
}
...
************************
*\brief
*input: position of one MB
*output: disparity vector from depth view 
***********************
*/
void getDVFromBaseDepth( Macroblock **currMB, StorablePicture* ref_img_pic,int* disparity_table, short disparity )
{
  VideoParameters* p_Vid= (*currMB)->p_Vid;
  imgpel** depth_map=ref_img_pic->imgY;
  imgpel maxDepth = 0;

  int i = 0, blocki = 0, j = 0, blockj = 0;

  int disparity_mul4;
 
  int subx, suby;
  int part_mode;
  int part_size_x[4]={16,16, 8, 8};
  int part_size_y[4]={16, 8,16, 8};

  for(j=0; j<4; j++)
    for(i=0; i< 16/BLOCK_VSP_T; i++)
      memset( (*currMB)->iDoNBDV[j][i], 0, (MB_BLOCK_SIZE/BLOCK_VSP_T)*sizeof(short) );

  for (part_mode=0; part_mode<4; part_mode++)
  {
    for (j = (*currMB)->pix_y; j < ((*currMB)->pix_y + MB_BLOCK_SIZE); j+=part_size_y[part_mode])
    {
      for (i = (*currMB)->pix_x; i < ((*currMB)->pix_x + MB_BLOCK_SIZE); i+=part_size_x[part_mode])
      {
        maxDepth = 0;
        for (blockj = 0; blockj < part_size_y[part_mode]; blockj+=(part_size_y[part_mode]-1))
        {
          for (blocki = 0; blocki < part_size_x[part_mode]; blocki+=(part_size_x[part_mode]-1))
          {
            int curr_view_id=ref_img_pic->view_id;
            int depthPosX=iClip3(p_Vid->DepthCropLeftCoord,p_Vid->DepthCropRightCoord, ( i + blocki + ( disparity>>2 ) +p_Vid->grid_pos_x[curr_view_id])*p_Vid->depth_hor_mult>>p_Vid->depth_hor_rsh);
            int depthPosY=iClip3(p_Vid->DepthCropTopCoord, p_Vid->DepthCropBottomCoord,( j + blockj   +p_Vid->grid_pos_y[curr_view_id] )*p_Vid->depth_ver_mult>>p_Vid->depth_ver_rsh);

            if (maxDepth < depth_map[depthPosY][depthPosX])
              maxDepth = depth_map[depthPosY][depthPosX];
          }
        }
        disparity_mul4=disparity_table[maxDepth];

        for( suby=0; suby<(part_size_y[part_mode]/BLOCK_VSP_T); suby++)
        {
          for( subx=0; subx<(part_size_x[part_mode]/BLOCK_VSP_T); subx++)
          {
            (*currMB)->iDoNBDV[part_mode][(j%MB_BLOCK_SIZE)/BLOCK_VSP_T + suby][(i%MB_BLOCK_SIZE)/BLOCK_VSP_T + subx] = (short)disparity_mul4;
          }
        }
      } 
    } 
...
void getDVFromBaseDepth_MB( Macroblock **currMB, StorablePicture* ref_img_pic,int* disparity_table, short disparity)
{
  VideoParameters* p_Vid= (*currMB)->p_Vid;
  imgpel** depth_map=ref_img_pic->imgY;
  imgpel maxDepth = 0;
  ...
  for (j = (*currMB)->pix_y; j < ((*currMB)->pix_y + MB_BLOCK_SIZE); j += (MB_BLOCK_SIZE-1))
  {
    for (i = (*currMB)->pix_x; i < ((*currMB)->pix_x + MB_BLOCK_SIZE); i += (MB_BLOCK_SIZE-1))
    {
      int curr_view_id=ref_img_pic->view_id;
      int depthPosX=iClip3(p_Vid->DepthCropLeftCoord,p_Vid->DepthCropRightCoord, ( i + (disparity>>2) +p_Vid->grid_pos_x[curr_view_id])*p_Vid->depth_hor_mult>>p_Vid->depth_hor_rsh);
      int depthPosY=iClip3(p_Vid->DepthCropTopCoord,p_Vid->DepthCropBottomCoord,(j+p_Vid->grid_pos_y[curr_view_id])*p_Vid->depth_ver_mult>>p_Vid->depth_ver_rsh);
  ...
  disparity_mul4=disparity_table[maxDepth];
  (*currMB)->iNBDV = (short)disparity_mul4;
}
#endif
...
#if EXT3D
  (*currMB)->base_view_id  = (*currMB)->p_Inp->ViewCodingOrder[0];

  if( !currSlice->is_depth && p_Vid->isTextureFirst && currSlice->hasInterviewRef && (p_Vid->p_Inp->VSP_Enable || currSlice->depth_based_mvp_flag) )
  {
    short disparity;
    (*currMB)->iDDV = mb_addr ? p_Vid->mb_data[mb_addr-1].iDDV : 0;
    get_nbdv( (*currMB), &disparity );
    (*currMB)->iNBDV = disparity;
    if( !currSlice->is_depth && GetVOIdx(p_Inp,p_Vid->view_id) && p_Vid->p_Inp->VSP_Enable )
    {
      StorablePicture* ref_depth_pic = p_Vid->left_refer_depth        ? p_Vid->left_refer_depth        : p_Vid->right_refer_depth;
      int*           disparity_table = p_Vid->forward_disparity_table ? p_Vid->forward_disparity_table : p_Vid->backward_disparity_table;
      getDVFromBaseDepth_MB( currMB, ref_depth_pic, disparity_table, disparity        );
      getDVFromBaseDepth   ( currMB, ref_depth_pic, disparity_table, (*currMB)->iNBDV );
    }
  }
#endif
}
...
#if EXT3D
  if( p_Vid->isTextureFirst && currSlice->hasInterviewRef && !(IS_INTRA(currMB)) )
  {
    currMB->iDDV = currMB->iNBDV;
  }
#endif
...
************************************************************************
* \brief
*    the same as reset_block, except alc 4x4 blocks prrocessing:
*    prediction is performed for each 4x4 block again when alc mode is enabled and DCT4x4 is used
************************************************************************
*/
int reset_blockEx(Macroblock* currMB, int *cbp, int64 *cbp_blk, int block8x8, int  p_dir, int  list_mode[2], char  *ref_idx,  short bipred_me)
{
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  //int i, j;
  ...
  if (0 == currMB->is_alc_mode || (1 == currMB->is_alc_mode && list_mode[0] == SMB8x8))
    return reset_block(currMB, cbp, cbp_blk, block8x8);
  else
  {
    (*cbp)     &=  (63 - cbp_mask);
    (*cbp_blk) &= ~(51 << (4 * block8x8 - 2 * (block8x8 & 0x01)));
    ...
    {    
      for (block_y=mb_y; block_y<mb_y+8; block_y+=4)
      {
        ...
        {
          if(currMB->mb_type==PVSP_SKIP)
          {
            int lmode[2]={10,10};
            char ridx[2];
            ridx[0]=(char)currSlice->idxVspRef[0];
            ridx[1]=(char)currSlice->idxVspRef[1];
            luma_prediction (currMB, block_x, block_y, 4, 4, p_dir, lmode, ridx, bipred_me); // @DT: Bi-VSP support
          }
          else
          {
            luma_prediction (currMB, block_x, block_y, 4, 4, p_dir, list_mode, ref_idx, bipred_me);
          }
          copy_image_data_4x4(&p_Vid->enc_picture->imgY[currMB->pix_y + block_y], &currSlice->mb_pred[0][block_y], currMB->pix_x + block_x, block_x);
        }
      }
    }
    else
                           ...
                           char  *ref_idx)     //!< reference pictures for each list
{
  ...
  VideoParameters *p_Vid = currSlice->p_Vid;  
#if EXT3D
  int    block_y, block_x, pic_pix_x, nonzero = 0, cbp_blk_mask; // pic_pix_y
#else
  ...
  {
#if EXT3D 
    coeff_cost  = reset_blockEx(currMB, cbp, cbp_blk, block8x8, p_dir, list_mode, ref_idx, bipred_me);
#else
    coeff_cost  = reset_block(currMB, cbp, cbp_blk, block8x8);
#endif
  }
...
void luma_residual_coding (Macroblock *currMB)
{
...
#if EXT3D
  int is_skip = currSlice->slice_type == P_SLICE && (currMB->mb_type == PSKIP || currMB->mb_type == PVSP_SKIP);
#else
  ...
  if (is_skip)
  {
    ...
    //===== luma prediction ======
#if EXT3D
      if (1 == currSlice->alc_slice_flag && 1 == currMB->is_alc_mode)
      {
        if( currSlice->bIVRefExist && list_ref_idx[0] == currSlice->idxInterviewRef[0])
        {
          // here we use is_alc_mode to switch ALC mode in Skip, then skip is in VSP.
      ...
      else
      {
        if(currMB->mb_type == PVSP_SKIP)
        {
          ...
          for(j=0;j<16;j+=BLOCK_VSP_T)
          {
            ...
            {
              luma_prediction (currMB, i, j, BLOCK_VSP_T, BLOCK_VSP_T, p_dir, lmode, ridx, currMB->b8x8[0].bipred); // @DT: Bi-VSP support
            }
          }
        }
        else
...
#if EXT3D
    else if(1 == currMB->is_alc_mode && currMB->mb_type == P16x8)
    {
      block8x8 = 0;
      currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
      luma_prediction (currMB, 0, 0, 16, 8, p_dir, list_mode, list_ref_idx, currMB->b8x8[block8x8].bipred); 
      compute_residue (&p_Vid->pCurImg[currMB->opix_y], &currSlice->mb_pred[0][0], &currSlice->mb_ores[0][0], 0, currMB->pix_x, MB_BLOCK_SIZE, BLOCK_SIZE_8x8);
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      block8x8 = 1;
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      ...
      block8x8 = 2;
      currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
      luma_prediction (currMB, 0, 8, 16, 8, p_dir, list_mode, list_ref_idx, currMB->b8x8[block8x8].bipred); 
      compute_residue (&p_Vid->pCurImg[currMB->opix_y + BLOCK_SIZE_8x8], &currSlice->mb_pred[0][BLOCK_SIZE_8x8], &currSlice->mb_ores[0][BLOCK_SIZE_8x8], 0, currMB->pix_x, MB_BLOCK_SIZE, BLOCK_SIZE_8x8);
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      block8x8 = 3;
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
    }
    else if(1 == currMB->is_alc_mode && currMB->mb_type == P8x16)
    {
      block8x8 = 0;
      currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
      luma_prediction (currMB, 0, 0, 8, 16, p_dir, list_mode, list_ref_idx, currMB->b8x8[block8x8].bipred); 
      compute_residue (&p_Vid->pCurImg[currMB->opix_y], &currSlice->mb_pred[0][0], &currSlice->mb_ores[0][0], 0, currMB->pix_x, BLOCK_SIZE_8x8, MB_BLOCK_SIZE);
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      block8x8 = 2;
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      ...
      block8x8 = 1;
      currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
      luma_prediction (currMB, 8, 0, 8, 16, p_dir, list_mode, list_ref_idx, currMB->b8x8[block8x8].bipred); 
      compute_residue (&p_Vid->pCurImg[currMB->opix_y], &currSlice->mb_pred[0][0], &currSlice->mb_ores[0][0], BLOCK_SIZE_8x8, currMB->pix_x + BLOCK_SIZE_8x8, BLOCK_SIZE_8x8, MB_BLOCK_SIZE);
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
      block8x8 = 3;
      sum_cnt_nonz += luma_residual_coding_16x16 (currMB, block8x8, list_mode);
    }
    else if(1 == currMB->is_alc_mode && currMB->mb_type == P8x8 && currMB->luma_transform_size_8x8_flag == 1)
    {
      for (block8x8 = 0; block8x8 < 4; ++block8x8)
      {
          currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
...
void chroma_residual_coding (Macroblock *currMB)
{
  ...
  short p_dir;
#if EXT3D
  int   skipped = ((currMB->mb_type == 0 || currMB->mb_type == PVSP_SKIP) && (currSlice->slice_type == P_SLICE));
#else
  ...
  for (uv = 0; uv < 2; ++uv)
  {
    ...
    // Intra modes
#if EXT3D
    if (currMB->mb_type > P8x8 && currMB->mb_type != PVSP_SKIP)
#else
    if (currMB->mb_type > P8x8)
#endif
    {
...
#if EXT3D
          if(currMB->mb_type==PVSP_SKIP)
            chroma_prediction_4x4 (currMB, uv, block_x, block_y, p_dir, 10, 10, (short)currSlice->idxVspRef[0], (short)currSlice->idxVspRef[1], bipred_me); // @DT: Bi-VSP support
          else
#endif
          chroma_prediction_4x4 (currMB, uv, block_x, block_y, p_dir, list_mode[0], list_mode[1], (short)list_ref_idx[0], (short)list_ref_idx[1], bipred_me);
        }
      }
    }
...
#if EXT3D
    else if (currMB->mb_type==PVSP_SKIP && currSlice->slice_type == P_SLICE) return 0;
#endif
...
#if EXT3D
    else if (mbtype==BVSP_DIRECT)          return 0;
#endif
...
int writeMBLayerBSlice (Macroblock *currMB, int rdopt, int *coeff_rate)
{
  ...
  int             no_bits    = 0;
#if EXT3D
  int             skip       = (currMB->mb_type != 0 && currMB->mb_type != BVSP_DIRECT) ? 0 : !currMB->cbp;  
#else
  ...
  if (currSlice->mb_aff_frame_flag)
  {
    ...
    {
#if EXT3D
      if ((prevMB->mb_type != 0 && prevMB->mb_type != BVSP_DIRECT) ? 0: !prevMB->cbp)
#else
...
#if EXT3D
      prevMbSkipped = p_Vid->mb_data[prev_mb_nr].skip_flag || p_Vid->mb_data[prev_mb_nr].vsskip_flag;
#else
  ...
  if (currSlice->symbol_mode == CABAC)
  {
...
#if EXT3D
    if(checkNeigborVSSkip(currMB)) 
    {
      //========= view synthesis skip flag -> conventional skip flag =========
      if (currSlice->bVspRefExist) 
      {
        if (currMB->mb_type == BVSP_DIRECT && !currMB->cbp) se.value1 = 1; // view synthesis skip
        else se.value1 = 0;
        se.value2  = currMB->cbp;
        se.type    = SE_MBTYPE;
        TRACE_SE (se.tracestring, "mb_vsskip_flag");
        currSlice->writeMB_VSSkip(currMB, &se, dataPart);
        no_bits         += se.len;
      }

      if(currMB->mb_type != BVSP_DIRECT || currMB->cbp)
      {
#endif
    ...
    no_bits         += se.len;
      ...
      //========= conventional skip flag -> view synthesis skip flag =========    
      se.value1  = mb_type;
      se.value2  = currMB->cbp;
      se.type    = SE_MBTYPE;
      ...
      //========= write mb_skip_flag (CABAC) =========    
      if (!currMB->skip_flag && currSlice->bVspRefExist) 
      {
        if (currMB->mb_type == BVSP_DIRECT && !currMB->cbp) se.value1 = 1;  // view synthesis skip
        else se.value1 = 0;
        se.value2  = currMB->cbp;
    ...
    //========= write mb_type (CABAC) =========
#if EXT3D
    if ( (currMB->mb_type != 0 && currMB->mb_type != BVSP_DIRECT) || ((currMB->cbp != 0 || currSlice->cmp_cbp[1] !=0 || currSlice->cmp_cbp[2]!=0 )))
#else
...
#endif
    {
      ...
      no_bits         += se.len;
#if EXT3D
      //========= write mb_skip_type_flag (CABAC) =========
      if ((currMB->mb_type == 0 || currMB->mb_type == BVSP_DIRECT) && currSlice->bVspRefExist) // direct mode
      {
        if (currMB->mb_type == 0) se.value1 = 0; // conventional direct
        else if (currMB->mb_type == BVSP_DIRECT)  se.value1 = 1; // view synthesis direct
        se.value2  = currMB->cbp;
        se.type    = SE_MBTYPE;
        TRACE_SE (se.tracestring, "mb_direct_type_flag");
        currSlice->writeMB_DirectModeType(currMB, &se, dataPart);
        no_bits         += se.len;
      }
#endif
      }
...
#if EXT3D
  currMB->NoMbPartLessThan8x8Flag = ((currMB->mb_type == 0 || currMB->mb_type == BVSP_DIRECT) && !(p_Vid->active_sps->direct_8x8_inference_flag))? FALSE : TRUE;
#else
  ...
  //----- motion information -----
#if EXT3D
  if (currMB->mb_type !=0 && currMB->mb_type != BVSP_DIRECT && currMB->mb_type != P8x8 && (!IS_INTRA(currMB)))
#else
...
int writeMBLayerPSlice (Macroblock *currMB, int rdopt, int *coeff_rate)
{
  ...
  int             no_bits    = 0;
#if EXT3D
  int             skip       = (currMB->mb_type == 0 || currMB->mb_type == PVSP_SKIP) ? 1 : 0;  
#else
...
#if EXT3D 
  void (*writeStdRefFrame) (SyntaxElement *se, DataPartition *dP) = NULL;
  // substitution of writeRefFrame: ref idx doesn't coded wnen alc is enabled 
  if (1 == currMB->is_alc_mode)
  {
    writeStdRefFrame = currSlice->writeRefFrame[0];
    currSlice->writeRefFrame[0] = writeSE_Dummy;
  }
#endif
  ...
  if (currSlice->symbol_mode == CABAC)
  {
    ...
    if(checkNeigborVSSkip(currMB)) 
    {
      ...
      if (currSlice->bVspRefExist)
      {
        if (currMB->mb_type == PVSP_SKIP) se.value1 = 1; // view synthesis skip
        else se.value1 = 0;
        se.value2  = currMB->cbp;
        se.type    = SE_MBTYPE;
        TRACE_SE (se.tracestring, "mb_vsskip_flag");
...
#if EXT3D
    if (1 == currSlice->alc_slice_flag && currMB->mb_type == 0)
    {
      se.value1 = currMB->is_alc_mode;
      se.value2  = 0;
      se.type    = SE_MBTYPE;
      TRACE_SE (se.tracestring, "ALCskip_flag");
      write_ALCskip_flag_CABAC(currMB, &se, dataPart);
      no_bits         += se.len;
    }
#endif
...
#endif
    {
      ...
      no_bits         += se.len;
#if EXT3D 
      if (1 == currSlice->alc_slice_flag  && currMB->mb_type < 4)
      {
        se.value1  = currMB->is_alc_mode;
        se.value2  = 0;
        se.type    = SE_MBTYPE;

        TRACE_SE (se.tracestring, "ALC_flag");
        write_ALC_flag_CABAC(currMB, &se, dataPart);
        no_bits    += se.len;
      }
#endif
    }
  ...
  //----- motion information -----
#if EXT3D
  if (currMB->mb_type !=0 && currMB->mb_type != PVSP_SKIP && currMB->mb_type != P8x8 && (!IS_INTRA(currMB)))
#else
  if (currMB->mb_type !=0 && currMB->mb_type != P8x8 && (!IS_INTRA(currMB)))
#endif
  {
...
#if EXT3D 
  // returns saved standard value of writeRefFrame
  if (1 == currMB->is_alc_mode)
    currSlice->writeRefFrame[0] = writeStdRefFrame;
#endif
...
void write_macroblock (Macroblock* currMB, int eos_bit)
{
  ...
  //--- write non-slice termination symbol if the macroblock is not the first one in its slice ---
#if EXT3D
  if (currSlice->symbol_mode==CABAC && currMB->mbAddrX != currSlice->start_mb_nr && eos_bit && 0 == currSlice->series_7skip_len)
  {
...
#if EXT3D
  if (!((currMB->mb_type !=0 && currMB->mb_type != BVSP_DIRECT) || ((currSlice->slice_type==B_SLICE) && currMB->cbp != 0) ))
#else
...
#if EXT3D
  if( currSlice->Harmonize_VSP_IVP && (currSlice->listX[list_idx][ref]->view_id != currSlice->view_id) )
  {
    int ref_len = se.len;
    se.value1 = currSlice->p_Vid->enc_picture->mv_info[currMB->block_y + j][currMB->block_x + i].isVSPRef[list_idx];
#if TRACE
    if (!list_idx)
      snprintf(se.tracestring, TRACESTRING_SIZE, "bvsp_flag_l%d = %d", list_idx, se.value1);
    else
      snprintf(se.tracestring, TRACESTRING_SIZE, "bvsp_flag_l%d = %d", list_idx, se.value1);
#endif
    currSlice->writeMB_Ref_VSP_flag(currMB,list_idx,&se, dataPart);
  ...
  for (j = j0; j < j1; j += step_v)
  {
    ...
    for (i = i0; i < i1; i += step_h)
    {
...
#if EXT3D
      if(currSlice->Harmonize_VSP_IVP&&(currSlice->listX[list_idx][refindex]->view_id!=currSlice->view_id))
      {
        if( motion[currMB->block_y + j][currMB->block_x + i].isVSPRef[list_idx] )
        {
          continue;
        }
      }
      ...
      if(currMB->p_Slice->depth_based_mvp_flag)
        check_neighbors(currMB,block,p_Vid->enc_picture->mv_info,list_idx,refindex);
...
#if EXT3D
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0 && currMB->mb_type != BVSP_DIRECT)
#else
...
int write_pslice_motion_info_to_NAL (Macroblock* currMB)
{
  ...
  //=== If multiple ref. frames, write reference frame for the MB ===
#if EXT3D
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0 && currMB->mb_type != PVSP_SKIP)
#else
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0)
#endif
  {
...
int writeCBPandDquant (Macroblock* currMB)
{
  ...
  DataPartition*  dataPart;  
#if EXT3D 
  int IsSMB4x8_SMB8x4_SMB4_4 = 0;
  if (P8x8 == currMB->mb_type)
  {
    if (currMB->b8x8[0].mode > 4 && currMB->b8x8[0].mode < 8) 
      IsSMB4x8_SMB8x4_SMB4_4 = 1;
    else
      if (currMB->b8x8[1].mode > 4 && currMB->b8x8[1].mode < 8)
        IsSMB4x8_SMB8x4_SMB4_4 = 1;
      else
        if (currMB->b8x8[2].mode > 4 && currMB->b8x8[2].mode < 8)
          IsSMB4x8_SMB8x4_SMB4_4 = 1;
        else
          if (currMB->b8x8[3].mode > 4 && currMB->b8x8[3].mode < 8)
            IsSMB4x8_SMB8x4_SMB4_4 = 1;
  }
#endif
...
#if EXT3D 
    // SMRC: Transform Flag is needed if CBP == 0 in ALC mode
    if (need_transform_size_flag || (currMB->is_alc_mode && 0 == (currMB->cbp&15) && currSlice->p_Inp->Transform8x8Mode && 0 == IsSMB4x8_SMB8x4_SMB4_4) )            
#else
...
#if EXT3D
void get_nbdv( Macroblock* currMB, short* disparity )
{
  assert( !currMB->p_Slice->is_depth );
  
  if( get_nbdv_temporal( currMB, disparity ) ) return;

  if( get_nbdv_spatial ( currMB, disparity ) ) return; 

  *disparity = currMB->iDDV;
