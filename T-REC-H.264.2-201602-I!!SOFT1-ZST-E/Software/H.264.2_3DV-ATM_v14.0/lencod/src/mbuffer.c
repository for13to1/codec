...
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb)
{
  ...
  {
    p_Dpb->size = (p_Dpb->size)*(p_Vid->p_Inp->NumOfViews) + 2;    // The largest interview reference number is 2
  }
#else
#if (MVC_EXTENSION_ENABLE)
...
#if EXT3D
  p_Dpb->fs_listinterview0=calloc(MAX_VIEWREFERENCE,sizeof(FrameStore*));
  if(NULL==p_Dpb->fs_listinterview0)
    no_mem_exit("init_pb:p_Dpb->fs_listinterview0");
  p_Dpb->fs_listinterview1=calloc(MAX_VIEWREFERENCE,sizeof(FrameStore*));
  if(NULL==p_Dpb->fs_listinterview1)
    no_mem_exit("init_bp:p_Dpb->fs_listinterview1");
  for(i=0;i<MAX_VIEWREFERENCE;++i)
  {
    p_Dpb->fs_listinterview0[i]=NULL;
    p_Dpb->fs_listinterview1[i]=NULL;
  }
#endif
...
void free_dpb(DecodedPictureBuffer *p_Dpb)
{
  ...
  p_Dpb->last_output_poc = INT_MIN;
#if EXT3D
  if(p_Dpb->fs_listinterview0)
    free(p_Dpb->fs_listinterview0);
  if(p_Dpb->fs_listinterview1)
    free(p_Dpb->fs_listinterview1);
  p_Dpb->last_output_view_id = -1;
  p_Dpb->listinterviewidx0=0;
  p_Dpb->listinterviewidx1=0;
  p_Dpb->last_output_view_id=0;
#else
#if (MVC_EXTENSION_ENABLE)
  p_Dpb->last_output_view_id = 0;
...
#if EXT3D
  if((p_Vid->nal_reference_idc != NALU_PRIORITY_DISPOSABLE)||(p_Vid->p_curr_frm_struct->inter_view_flag))
#else
#if (MVC_EXTENSION_ENABLE)  
...
#if EXT3D
  s->size_x_ori=p_Vid->width_ori;
  s->size_x_cr_ori=p_Vid->width_cr_ori;
  s->size_y_ori=p_Vid->height_ori;
  s->size_y_cr_ori=p_Vid->height_cr_ori;
#endif
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
      if (p_Dpb->fs_ref[i]->is_used==3 && (p_Inp->NumOfViews == 1 || p_Dpb->fs_ref[i]->view_id == p_Vid->view_id))
#else
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
      if (p_Dpb->fs_ltref[i]->is_used==3 && (p_Inp->NumOfViews==1 || p_Dpb->fs_ref[i]->view_id == p_Vid->view_id))
#else
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
      if (p_Dpb->fs_ref[i]->is_reference && (p_Inp->NumOfViews == 1 || p_Dpb->fs_ref[i]->view_id == p_Vid->view_id))
#else
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
      if (p_Inp->NumOfViews == 1 || p_Dpb->fs_ref[i]->view_id == p_Vid->view_id)
#else
...
{
#if EXT3D
  currSlice->p_Vid->p_Dpb->listinterviewidx0=0;
  currSlice->p_Vid->p_Dpb->listinterviewidx1=0;
...
void init_lists_p_slice(Slice *currSlice)
{
  ...
  {
#if EXT3D
    if((p_Vid->anchor_pic_flag[0]==0)||(p_Inp->InterPredictionAtAnchorOff==1) || (p_Inp->GradualViewRefresh==1 && (currSlice->framepoc % (4*p_Inp->intra_period) != 0)))
    {
      //!   Non-base view anchor picture just have interview  prediction
      for(i=0;i<p_Dpb->ref_frames_in_buffer;i++)
      {
        if((p_Dpb->fs_ref[i]->is_used==3)&&(p_Dpb->fs_ref[i]->view_id==p_Vid->view_id)
          /*&&(p_Dpb->fs_ref[i]->temporal_id<=p_Vid->temporal_id)*/)
        {
          //!For AVC ,view_id is aways equal to zero
          if ((p_Dpb->fs_ref[i]->frame->used_for_reference)&&(!p_Dpb->fs_ref[i]->frame->is_long_term))
          {
      ...
      if((!currSlice->is_depth)&&(!voidx)&&p_Vid->p_DualVid)
      {
        if(currSlice->listXsize[0])
        {
          p_Vid->p_DualVid->forward_frm_num=currSlice->listX[0][0]->frame_num;
        }
        else
        {
          p_Vid->p_DualVid->forward_frm_num=-1;
        }
      }
      ...
      for (i=0; i<p_Dpb->ltref_frames_in_buffer; i++)
      {
        if (p_Dpb->fs_ltref[i]->is_used==3 && (p_Dpb->fs_ref[i]->view_id==p_Vid->view_id)
          /*&&(p_Dpb->fs_ref[i]->temporal_id<=p_Vid->temporal_id)*/)
        {
          if (p_Dpb->fs_ltref[i]->frame->is_long_term)
          {
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
      if (p_Inp->NumOfViews == 1 || p_Dpb->fs_ltref[i]->view_id == p_Vid->view_id)
#else
  ...
  {
    // MVC interview pictures list for field coding
    assert(currSlice->structure != FRAME);
    if((p_Inp->NumOfViews>1)&&voidx&&(currSlice->structure!=FRAME))
...
void init_lists_b_slice(Slice *currSlice)
{
...

  {
    ...
    {
#if EXT3D
      if((p_Vid->anchor_pic_flag[0]==0)||(p_Vid->anchor_pic_flag[0]&&(p_Inp->InterPredictionAtAnchorOff==1)))
      {
        for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++)
        {
          if (p_Dpb->fs_ref[i]->is_used==3 && ( p_Dpb->fs_ref[i]->view_id==p_Vid->view_id)
        ...
        if((!currSlice->is_depth)&&(!voidx)&&p_Vid->p_DualVid)
        {
          ...
          if(currSlice->listXsize[1])
          {
            p_Vid->p_DualVid->backward_frm_num=currSlice->listX[1][0]->frame_num;
          }
          else
          {
            p_Vid->p_DualVid->backward_frm_num=-1;
          }
        }
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
        if (p_Dpb->fs_ref[i]->is_used && (p_Inp->NumOfViews==1 || p_Dpb->fs_ref[i]->view_id==p_Vid->view_id))
#else
    ...
    if (currSlice->structure == FRAME)
    {
      if (p_Dpb->fs_ref[i]->is_reference == 3)
        ...
        if ((!p_Dpb->fs_ref[i]->frame->is_long_term)&&(p_Dpb->fs_ref[i]->frame->pic_num == picNum)&&
          (p_Dpb->fs_ref[i]->frame->view_id==currSlice->view_id))
#else
        if ((!p_Dpb->fs_ref[i]->frame->is_long_term)&&(p_Dpb->fs_ref[i]->frame->pic_num == picNum))
#endif
...
void reorder_ref_pic_list(Slice *currSlice, int cur_list)
{
  ...
  // that's a definition
#if EXT3D
  currSlice->listXsize[cur_list] = (char)imin(currSlice->listXsize[cur_list],currSlice->num_ref_idx_active[cur_list]);
#else
...
#if EXT3D
int view_id_in_ref_view_list_idx(int view_id, int *ref_view_id, int num_ref_views)
{
  int i;
               ...
               int anchor_pic_flag)
{
  ...
  int CurrViewOrderIdx=GetVOIdx(p_Inp,curr_view_id) ;


  assert(CurrViewOrderIdx>=0);
  if(anchor_pic_flag)
  {
    num_ref_views = list_idx==0? p_Inp->NumOfFwdAnchorRefs[CurrViewOrderIdx] : p_Inp->NumOfBwdAnchorRefs[CurrViewOrderIdx];
    ref_view_id = list_idx==0? p_Inp->FwdAnchorRefs[CurrViewOrderIdx]:p_Inp->BwdAnchorRefs[CurrViewOrderIdx];
  }
  else
  {
    num_ref_views = list_idx==0? p_Inp->NumOfFwdNoAnchorRefs[CurrViewOrderIdx] : p_Inp->NumOfBwdNoAnchorRefs[CurrViewOrderIdx];
    ref_view_id = list_idx==0? p_Inp->FwdNoAnchorRefs[CurrViewOrderIdx]:p_Inp->BwdAnchorRefs[CurrViewOrderIdx];
  }
  ...
  for(i=0; i<p_Dpb->used_size; i++)
  {
    ...
    if(pic_avail && p_Dpb->fs[i]->inter_view_flag[fld_idx])
    {
      if(poc == currPOC)
      {
        pos=view_id_in_ref_view_list_idx(p_Dpb->fs[i]->view_id, ref_view_id, num_ref_views);
        if(pos==-1)
          continue;
        else
        {
  ...
  assert(*listXsize==num_ref_views);
...
static void mm_unmark_short_term_for_reference(DecodedPictureBuffer *p_Dpb, StorablePicture *p, int difference_of_pic_nums_minus1)
{
  ...
  if(p_Inp->NumOfViews>1)
  {
    ...
    for(i=0;i<p_Dpb->used_size;++i)
    {
      if(p->structure==FRAME)
      {
        if ((p_Dpb->fs[i]->is_reference==3) && (p_Dpb->fs[i]->is_long_term==0))
        {
          if (p_Dpb->fs[i]->frame->pic_num == picNumX)
          {
            unmark_for_reference(p_Dpb->fs[i]);
            return;
          }
...
void store_picture_in_dpb(DecodedPictureBuffer *p_Dpb, StorablePicture* p, FrameFormat *output)
{
  ...
  if(p_Vid->p_Inp->NumOfViews>1)
  {
    if(p->structure==FRAME)
    {
      ...
      p->temporal_layer=p_Vid->temporal_id;
#if ITRI_INTERLACE
    }
    else
    {
...
#if EXT3D
  if((p_Vid->currentPicture->idr_flag)&&(voidx==0))
#else
  ...
  while (p_Dpb->used_size == p_Dpb->size)
  {
...
#if EXT3D
    // non-reference frames(inter prediction && interview prediction) may be output directly
    if((!p->used_for_reference)&&(p->view_id==p_Vid->view_id)&&(!p_Vid->inter_view_flag[0]))
#else
#if (MVC_EXTENSION_ENABLE)
...
#endif
    {
      ...
      if ((-1==pos) || (p->poc < poc))
      {
        direct_output(p_Vid, p, output, p_Vid->p_decMVC[voidx]);
        return;
      }
#else
#if (MVC_EXTENSION_ENABLE)
...
#if EXT3D
    direct_output_paff(p_Vid, p, output, p_Vid->p_decMVC[p->view_id]);
#else
#if (MVC_EXTENSION_ENABLE)
    if(p_Vid->p_Inp->num_of_views==2)
    {
...
static void insert_picture_in_dpb(VideoParameters *p_Vid, FrameStore* fs, StorablePicture* p)
{
...
#if EXT3D
  else if(p_Vid->p_Inp->NumOfViews>1 && p->inter_view_flag[0])
  {
    if( (p_Vid->p_Inp->separate_colour_plane_flag != 0) )
    {
      UnifiedOneForthPix_JV(p_Vid, 0, p);
  ...
  fs->temporal_id=p->temporal_layer;
#else
#if (MVC_EXTENSION_ENABLE)
  fs->view_id = p->view_id;
#endif
...
#if EXT3D
  voidx=GetVOIdx(p_Inp,p_Dpb->fs[pos]->view_id);
  write_stored_frame(p_Vid,p_Dpb->fs[pos],output,p_Vid->p_decMVC[voidx]);
#else
#if (MVC_EXTENSION_ENABLE)
  if(p_Inp->num_of_views==2)
  {
...
#if EXT3D
#if ITRI_INTERLACE
  fs->frame->force_yuv400 = fs->top_field->force_yuv400;
  fs->frame->is_depth = p_Vid->is_depth;
  fs->frame->view_id = fs->top_field->view_id;
  if (p_Vid->is_depth)
  {
    fs->frame->NonlinearDepthNum = p_Vid->NonlinearDepthNum;
    memcpy(fs->frame->NonlinearDepthPoints, p_Vid->NonlinearDepthPoints, sizeof(p_Vid->NonlinearDepthPoints));
  } else {
    fs->frame->NonlinearDepthNum = 0;    
    fs->frame->NonlinearDepthPoints[0]=0;
    fs->frame->NonlinearDepthPoints[1]=0;
  }

  fs->frame->reduced_res    =p_Vid->low_res;
  fs->frame->upsampling_params=p_Vid->low_res?p_Vid->p_output_upsample_params:NULL;
#endif
  fs->frame->PostDilation = p_Vid->p_Inp->PostDilation;
#endif
}
...
#if EXT3D
void init_interview_list(Slice *currSlice)
{
  VideoParameters* p_Vid=currSlice->p_Vid;
  InputParameters* p_Inp=p_Vid->p_Inp;
  ...
  if((p_Inp->NumOfViews>1)&&(voidx>0))
  {
    ...
    if(p_Vid->anchor_pic_flag[0])
    {
      ...
      if(p_Inp->NumOfFwdAnchorRefs[voidx])
        append_interview_list(p_Dpb,p_Vid->structure,0,p_Dpb->fs_listinterview0,&(p_Dpb->listinterviewidx0),p_Vid->framepoc,p_Vid->view_id,p_Vid->anchor_pic_flag[0]);
      if(p_Inp->NumOfBwdAnchorRefs[voidx])
        append_interview_list(p_Dpb,p_Vid->structure,1,p_Dpb->fs_listinterview1,&(p_Dpb->listinterviewidx1),p_Vid->framepoc,p_Vid->view_id,p_Vid->anchor_pic_flag[0]);
    }
    else
    {
      for(i=0;i<MAX_VIEWREFERENCE;++i)
      ...
      if(p_Inp->NumOfFwdNoAnchorRefs[voidx])
        append_interview_list(p_Dpb,p_Vid->structure,0,p_Dpb->fs_listinterview0,&(p_Dpb->listinterviewidx0),p_Vid->framepoc,p_Vid->view_id,p_Vid->anchor_pic_flag[0]);
      if(p_Inp->NumOfBwdNoAnchorRefs[voidx])
        append_interview_list(p_Dpb,p_Vid->structure,1,p_Dpb->fs_listinterview1,&(p_Dpb->listinterviewidx1),p_Vid->framepoc,p_Vid->view_id,p_Vid->anchor_pic_flag[0]);
    }
  }
 ...
 ************************************************************************
 * \brief
 *    Allocate memory for buffering of interview reference picture reordering commands
 ************************************************************************
 */
void alloc_interview_ref_pic_list_reordering_buffer(Slice*currSlice)
{
  //VideoParameters*p_Vid=currSlice->p_Vid;
  //assert(p_Vid->view_id>0);
  if(currSlice->slice_type!=I_SLICE && currSlice->slice_type != SI_SLICE)
  {
    if((currSlice->reordering_of_interview_num_idc[LIST_0]=calloc(MAX_VIEWREFERENCE+1,sizeof(int)))==NULL)
      no_mem_exit("alloc_ref_pic_list_reordering_buffer:reordering_of_interview_num_idc[LIST_0]");
    if ((currSlice->abs_diff_view_idx_minus1[LIST_0] = calloc(MAX_VIEWREFERENCE+1, sizeof(int)))==NULL)
      no_mem_exit("alloc_ref_pic_list_reordering_buffer:abs_diff_view_idx_minus1[LIST_0] ");
    currSlice->reordering_interview_flg[0]=0;
  }
  else
  {
    currSlice->reordering_of_interview_num_idc[LIST_0]=NULL;
    currSlice->abs_diff_view_idx_minus1[LIST_0]=NULL;
    currSlice->reordering_interview_flg[0]=0;
  }
  if(currSlice->slice_type==B_SLICE)
  {
    if((currSlice->reordering_of_interview_num_idc[LIST_1]=calloc(MAX_VIEWREFERENCE+1,sizeof(int)))==NULL)
      no_mem_exit("alloc_ref_pic_list_reordering_buffer:reordering_of_interview_num_idc[LIST_1]");
    if ((currSlice->abs_diff_view_idx_minus1[LIST_1] = calloc(MAX_VIEWREFERENCE+1, sizeof(int)))==NULL) 
      no_mem_exit("alloc_ref_pic_list_reordering_buffer: abs_diff_view_idx_minus1[LIST_1]");
    currSlice->reordering_interview_flg[1]=0;
  }
  else
  {
    currSlice->reordering_of_interview_num_idc[LIST_1]=0;
    currSlice->abs_diff_view_idx_minus1[LIST_1]=0;
    currSlice->reordering_interview_flg[1]=0;
  }
}
#endif
...
************************************************************************
* \brief
*    Free memory for buffering of interview reference picture reordering commands
************************************************************************
*/
void free_interview_ref_pic_list_reordering_buffer(Slice*currSlice)
{
  if(currSlice->reordering_of_interview_num_idc[LIST_0])
    free(currSlice->reordering_of_interview_num_idc[LIST_0]);
  ...
  if(currSlice->reordering_of_interview_num_idc[LIST_1])
    free(currSlice->reordering_of_interview_num_idc[LIST_1]);
  if(currSlice->abs_diff_view_idx_minus1[LIST_1])
    free(currSlice->abs_diff_view_idx_minus1[LIST_1]);
  currSlice->reordering_of_interview_num_idc[LIST_1]=NULL;
...
************************************************************************
*/
void check_neighbors(Macroblock* currMB,PixelPos* block,PicMotionParams**mv_info,int list,int ref)
{
