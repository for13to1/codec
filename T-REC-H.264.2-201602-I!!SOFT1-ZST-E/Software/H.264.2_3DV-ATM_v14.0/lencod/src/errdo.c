...
{
  UNREFERENCED_PARAMETER(concealment_type);
...
*/
#if EXT3D
StorablePicture* find_nearest_ref_picture(DecodedPictureBuffer *p_Dpb, int poc,int view_id)
#else
  ...
  for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++)
  {
    if (p_Dpb->fs_ref[i]->is_used==3)
    {
      ...
      if ((p_Dpb->fs_ref[i]->frame->used_for_reference)
        &&p_Dpb->fs_ref[i]->frame->view_id==view_id&&(!p_Dpb->fs_ref[i]->frame->is_long_term))
#else
