...
{
#if EXT3D // @DT: ALC related
  int     j; //, k;
  imgpel *ref_line = UMVLine4X (list, pic_pix_y, pic_pix_x);
  if ( IsTMFilled() )
  {
    PiLC_TMPelCorHist_1DArray(ref_line, mpred, p_Vid->padded_size_x, block_size_x, block_size_y, block_size_x);
    ALC_TurnOFF();
  }  
  else
  {
      // conventional prediction w/o ALC
    for (j = 0; j < block_size_y; j++) 
    {
                     ...
                     )
{
...
#if EXT3D 
  int pic_rpix_y, pic_rpix_x; // coordinates of reference block (in QPEL)
  StorablePicture *pref;
#endif

#if EXT3D
  int  apply_weights = ( (currSlice->weighted_prediction == 1)||(currSlice->depth_range_wp) || (currSlice->weighted_prediction == 2 && p_dir == 2) );
#else
  ...
  {
    pic_rpix_x = pic_opix_x + curr_mv->mv_x; pic_rpix_y = pic_opix_y + curr_mv->mv_y;
    pref = currSlice->listX[LIST_0 + currMB->list_offset][(short) ref_idx[0]];
 
    if (((pic_rpix_x - (4 * MAX_TEMPLATE_SIZE) >=0) && (pic_rpix_y - (4 * MAX_TEMPLATE_SIZE) >=0)) &&
        (pic_opix_x - 4 * MAX_TEMPLATE_SIZE >= 0 && pic_opix_y - 4 * MAX_TEMPLATE_SIZE >= 0) &&
       (((pic_rpix_y + block_size_y*4) <= pref->size_y * 4) && (pic_rpix_x + block_size_x * 4) <= pref->size_x * 4)
       )
    {
      //======Get Templates into internal buffers
      ALC_GetTM(  pref->p_curr_img_sub[pic_rpix_y & 0x03][pic_rpix_x & 0x03], (pic_rpix_y >> 2), (pic_rpix_x >> 2), 
                  p_Vid->enc_picture->imgY, (pic_opix_y >> 2), (pic_opix_x >> 2), block_size_y, block_size_x);
    }
  }
  ...
  if (apply_weights)
  {
    ...
    {
#if EXT3D
      // alc replaces weighted prediction
      if( currMB->is_alc_mode)
        MCPrediction(&mb_pred[block_y], l0_pred, block_size_y, block_x, block_size_x);
      else
        MCWeightedPrediction(&mb_pred[block_y], l0_pred, block_size_y, block_x, block_size_x, p_Vid->max_imgpel_value,
...
#if EXT3D
   int  apply_weights = (( currSlice->weighted_prediction != 0 )||currSlice->depth_range_wp);
#else
