...
distblk errdo_distortion_estimation_multihyp(Macroblock *currMB, int block, int block_size, short mode, short pdir, distblk min_rdcost)
{
  ...
  UNREFERENCED_PARAMETER(block_size);
...
#if EXT3D
  refPic = find_nearest_ref_picture(p_Vid->p_Dpb, enc_pic->poc,enc_pic->view_id); //Used for concealment if actual reference pic is not known.
#else
