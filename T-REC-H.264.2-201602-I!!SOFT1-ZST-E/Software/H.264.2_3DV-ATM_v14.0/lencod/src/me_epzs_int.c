                             ...
                             )
{
  ...
  if(mv_block->isVSPRef[list])
  {
    ...
    currMB->GetInterviewMVPredictionMax(currMB, &cand,
      (mv_block->block_x)<<2,(mv_block->block_y)<<2,mv_block->blocksize_x,mv_block->blocksize_y,list==LIST_0?p_Vid->forward_disparity_table:p_Vid->backward_disparity_table);
