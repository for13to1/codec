...
#if EXT3D
    case 2:
      p_Vid->EstimateWPPSlice=EstimateWPPSliceAlg3;
      p_Vid->EstimateWPBSlice=EstimateWPBSliceAlg3;
      p_Vid->TestWPPSlice=NULL;
      p_Vid->TestWPBSlice=NULL;
#endif
      break;
    }
  }
...
static int ComputeBlockSum(imgpel **CurrentImage, int height_in_blk, int width_in_blk, int blk_size_y, int blk_size_x, int cur_blk)
{
  ...
  UNREFERENCED_PARAMETER(height_in_blk);
  ...
  if (select_method == 0) //! implicit mode
  {
...
#if EXT3D
    active_refs[0] = currSlice->listXsize[0];
    active_refs[1] = currSlice->listXsize[1];
#else
    ...
    for (clist=0; clist<2 + list_offset; clist++)
    {
      for (index = 0; index < active_refs[clist]; index++)
      {
        ...
        {
#if EXT3D
          if (weight[clist][index][comp] != default_weight[comp])
#else
