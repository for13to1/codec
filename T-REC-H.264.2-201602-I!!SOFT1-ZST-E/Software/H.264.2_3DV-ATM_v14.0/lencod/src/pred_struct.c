...
#if EXT3D
    p_seq_struct->num_frames_mvc           = (p_seq_struct->num_frames)*(p_Inp->NumOfViews) ; // support for multi-view
...
***********************************************************************
* \brief
*    Initialize the coding order for joint texture-depth coding. 
* \param p_Vid
*    pointer to the VideoParameters structure
* \param p_Inp
*    pointer to the InputParameters structure
...
void init_joint_coding_order(VideoParameters*p_Vid,InputParameters*p_Inp)
{
  ...
  //char*p=NULL;
  char item[128]={0x00};//!<hard code,but I think this is enough.
  
  
  //VideoParameters* p_Vid1=p_Vid->p_Vid1;

  int Num=p_Inp->NumOfViews;

  if(!p_Vid->is_depth)
  {
    //!<we set the coding order according to the specification in  texture config file
    char* ThreeDVCodingOrder=p_Inp->ThreeDVCodingOrder;
    int length=(int)strlen(ThreeDVCodingOrder);

    memcpy(p_Vid->ThreeDVCodingOrder,p_Inp->ViewCodingOrder,sizeof(p_Inp->ViewCodingOrder[0])*Num);
    ...
    if(length)
    {
      for(i=0;i<length;)
      {
        switch(ThreeDVCodingOrder[i])
        {
        case 'T'  :
        case 't'  :
          i++;
          k=0;
          while(isdigit(ThreeDVCodingOrder[i+k]))
            k++;
          assert(k);
          memset(item,0x00,10);
          memcpy(item,ThreeDVCodingOrder+i,k);

          p_Inp->ViewPresentFlag[atoi(item)]=1;

          p_Vid->ThreeDVCodingOrder[j++]=p_Inp->ViewCodingOrder[atoi(item)];
          i+=k;
          break;
        case 'D'  :
        case  'd'  :
          ...
          p_Vid->p_DualInp->ViewPresentFlag[atoi(item)]=1;

          p_Vid->ThreeDVCodingOrder[j++]=-(p_Vid->p_DualInp->ViewCodingOrder[atoi(item)]+1);  //!Here,we use  opposite number to indicate depth 
          i+=k;
          break;
        default:
          snprintf(errortext,ET_SIZE,"The format of joint coding order is invalid.\n");
          error(errortext,200);
        }
      }
    }
    ...
    {
      //!we first set default coding order: Texture First ,then Depth
      InputParameters* p_Inp1=p_Vid->p_DualInp;
      int Num1=p_Inp1->NumOfViews;
      j=0;
      memcpy(p_Vid->ThreeDVCodingOrder,p_Inp1->ViewCodingOrder,Num*sizeof(int)) ;
      memcpy(&(p_Vid->ThreeDVCodingOrder[Num]),p_Inp->ViewCodingOrder,Num1*sizeof(int));

      for (i=0;i<Num;i++)
      {
        p_Inp->ViewPresentFlag[i]=1;
      }
      for (i=0;i<Num1;i++)
      {
        p_Vid->p_DualInp->ViewPresentFlag[i]=1;
      }
    }

    p_Inp->NumOfCodingView=0;
    for (i=0;i<p_Inp->NumOfViews;i++)
    {
      if (p_Inp->ViewPresentFlag[i]==1) p_Inp->NumOfCodingView++;
    }

    k=0;
    for (i=0;i<p_Vid->p_DualInp->NumOfViews;i++)
    {
      if (p_Vid->p_DualInp->ViewPresentFlag[i]==1) 
        p_Vid->p_DualVid->CodingDepthView[k++] = i;
    }

    p_Vid->p_DualInp->NumOfCodingView=0;
    for (i=0;i<p_Vid->p_DualInp->NumOfViews;i++)
    {
      if (p_Vid->p_DualInp->ViewPresentFlag[i]==1) p_Vid->p_DualInp->NumOfCodingView++;
    }

    memcpy(p_Vid->p_DualVid->ThreeDVCodingOrder,p_Vid->ThreeDVCodingOrder,sizeof(p_Vid->ThreeDVCodingOrder));
  }
}
#endif
/*!
    ...
    case 3: // explicit structure
      {
       ...
       // encode without delay
        ...
        for ( idx = 1; idx < length; idx++ )
        {
          ...
          p_dst->random_access = 0;
#if EXT3D
      p_dst->temporal_layer=  p_Vid->gop_structure[idx - 1].temporal_layer;
#else
          p_dst->temporal_layer = 0;
#endif
...
void populate_frm_struct( VideoParameters *p_Vid, InputParameters *p_Inp, SeqStructure *p_seq_struct, int num_to_populate, int init_frames_to_code )
{
  ...
  if (((init_frames_to_code - p_seq_struct->pop_start_frame) < p_Inp->intra_period)
    || num_to_populate <= p_Inp->intra_period )
  {
    p_Vid->ShortGOP = 1;
  } 
#endif
...
#if EXT3D
  int weighted_pred = (slice_type == P_SLICE) ?((p_Inp->WeightedPrediction)||(p_Inp->DepthRangeBasedWP)) : 
    ((slice_type == B_SLICE) ? ((p_Inp->WeightedBiprediction)||(p_Inp->DepthRangeBasedWP)) : 0);
#else
...
#if EXT3D
  int ViewIdx=0;
  int NumOfViews=p_Inp->NumOfViews;
#endif
...
#if EXT3D
    for(ViewIdx=0;ViewIdx<NumOfViews;++ViewIdx)
    {
      p_curr_frm_mvc=p_frm_mvc+((idx*NumOfViews+ViewIdx)%p_seq_struct->num_frames_mvc);
      copy_frame_mvc(p_Inp,p_curr_frm,p_curr_frm_mvc,p_seq_struct->max_num_slices,p_Inp->ViewCodingOrder[ViewIdx]);
    }
#else
    // view 0
...
static void copy_frame_mvc( InputParameters *p_Inp, FrameUnitStruct *p_src, FrameUnitStruct *p_dst, int num_slices, int view_id )
{
  // Frame Unit Structure
#if EXT3D
  int j=0,ViewOrderIdx=0,OrderIdx=0;
  ViewOrderIdx  =GetVOIdx(p_Inp,view_id);

  p_dst->simulcast_type=p_dst->type  = p_src->type;
#else
  ...
  p_dst->temporal_layer=p_src->temporal_layer;
  //Wenyi >>
  //!<The value of idr_flag shall be the same for all VCL NAL units of an access unit
  p_dst->idr_flag=p_src->idr_flag; 
  p_dst->nal_ref_idc=ViewOrderIdx?(p_src->idr_flag ? NALU_PRIORITY_HIGH : p_src->nal_ref_idc) : p_src->nal_ref_idc;
#else
...
#if EXT3D
  p_dst->anchor_pic_flag=0;
  if((p_Inp->NumOfViews>1)&&((p_src->idr_flag)||((p_Inp->intra_period)&&(p_dst->frame_no%(p_Inp->intra_period)==0))))
    p_dst->anchor_pic_flag=1;
  p_dst->inter_view_flag=0;
  assert( ViewOrderIdx>=0);

  OrderIdx=ViewOrderIdx;
  if(OrderIdx+1<p_Inp->NumOfViews)
  {
    do
    {
      OrderIdx++;
      if(p_dst->anchor_pic_flag)
      {
        //anchor frame
        for(j=0;j<p_Inp->NumOfFwdAnchorRefs[OrderIdx];++j)
        {
          if(p_Inp->FwdAnchorRefs[OrderIdx][j]==view_id)
          {
            p_dst->inter_view_flag=1;
            goto EndLoop;
          }
        }
        for(j=0;j<p_Inp->NumOfBwdAnchorRefs[OrderIdx];++j)
        {
          if(p_Inp->BwdAnchorRefs[OrderIdx][j]==view_id)
          {
            p_dst->inter_view_flag=1;
            goto EndLoop;
          }
      ...
      {
        //non-anchor frame
        for(j=0;j<p_Inp->NumOfFwdNoAnchorRefs[OrderIdx];++j)
        {
          if(p_Inp->FwdNoAnchorRefs[OrderIdx][j]==view_id)
          {
            p_dst->inter_view_flag=1;
            goto EndLoop;
          }
        }
        for(j=0;j<p_Inp->NumOfBwdNoAnchorRefs[OrderIdx];++j)
        {
          if(p_Inp->BwdNoAnchorRefs[OrderIdx][j]==view_id)
          {
            p_dst->inter_view_flag=1;
            goto EndLoop;
          }
    ...
    }  while(OrderIdx<p_Inp->NumOfViews)  ;
  }
...
EndLoop:
  if(p_dst->anchor_pic_flag)
  {
    if(ViewOrderIdx)
    {
      if((p_Inp->NumOfFwdAnchorRefs[ViewOrderIdx]>0)&&(p_Inp->NumOfBwdAnchorRefs[ViewOrderIdx]>0))
        p_dst->type=B_SLICE;
      else  if((p_Inp->NumOfFwdAnchorRefs[ViewOrderIdx]>0)&&(p_Inp->NumOfBwdAnchorRefs[ViewOrderIdx]==0))
        p_dst->type=(p_dst->type ==B_SLICE)?B_SLICE:P_SLICE;
      else if((p_Inp->NumOfFwdAnchorRefs[ViewOrderIdx]==0)&&(p_Inp->NumOfBwdAnchorRefs[ViewOrderIdx]>0))
      {
        if(p_dst->type!=B_SLICE)
          assert(0)  ;
      }
    }
  }
  else
  {
    if(ViewOrderIdx)
    {
      if((p_Inp->NumOfFwdNoAnchorRefs[ViewOrderIdx]>0)&&(p_Inp->NumOfBwdNoAnchorRefs[ViewOrderIdx]>0))
        p_dst->type=B_SLICE;
      else  if((p_Inp->NumOfFwdNoAnchorRefs[ViewOrderIdx]>0)&&(p_Inp->NumOfFwdNoAnchorRefs[ViewOrderIdx]==0))
        p_dst->type=(p_dst->type ==B_SLICE)?B_SLICE:P_SLICE;
      else if((p_Inp->NumOfFwdNoAnchorRefs[ViewOrderIdx]==0)&&(p_Inp->NumOfFwdNoAnchorRefs[ViewOrderIdx]>0))
      {
        if(p_dst->type!=B_SLICE)
