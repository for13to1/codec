...
#if EXT3D 
  short cur_best_mode = currMB->best_mode;
...
#if EXT3D 
  // set possible coding modes for ALC
  if (currMB->is_alc_mode)
  {
      init_alc_enc_mb_params(currMB, &enc_mb);
  }
#endif
  ...
  if (!intra)
  {
    ...
    if (enc_mb.valid[0])
    {
...
#if EXT3D
      {
        ...
        {
        if(currMB->is_alc_mode)
          FindSkipModeMotionVector_4ALC(currMB);
        else
          FindSkipModeMotionVector_MVP (currMB);
        }
          else
...
#if EXT3D
    if (enc_mb.valid[PVSP_SKIP])
    {
      if (bslice)
        FindVSPDirectModeMotionVector(currMB, b8x8info);
      else
        FindVSPSkipModeMotionVector(currMB, b8x8info);
    }
#endif
      ...
      if (enc_mb.valid[mode])
      {
        ...
        if (currSlice->P444_joined)
        {
          if (p_Inp->SkipIntraInInterSlices && !intra && mode >= I16MB 
            && cur_best_mode <=3 && currMB->best_cbp == 0 && currSlice->cmp_cbp[1] == 0 && currSlice->cmp_cbp[2] == 0 && (currMB->min_rdcost < weighted_cost(enc_mb.lambda_mdfp,5)))
            continue;
        }
        else
        {
          if (p_Inp->SkipIntraInInterSlices)
          {
            if (!intra && mode >= I4MB)
            {
              if (cur_best_mode <=3 && currMB->best_cbp == 0 && (currMB->min_rdcost < weighted_cost(enc_mb.lambda_mdfp, 5)))
              {
                continue;
              }
              else if (cur_best_mode == 0 && (currMB->min_rdcost < weighted_cost(enc_mb.lambda_mdfp,6)))
              {
                continue;
              }
            }
