...
int mode_decision_for_I16x16_MB_RDO (Macroblock* currMB, int lambda)
{
  ...
  for (k = 0;k < 4; k++)
  {
    ...
    if ( p_Inp->VSD && p_Vid->is_depth )
    {
      ...
      distortionD = compute_SSE16x16_thres(&p_Vid->pCurImg[currMB->opix_y], &p_Vid->enc_picture->p_curr_img[currMB->pix_y], currMB->pix_x, currMB->pix_x, min_rdcost);
      ...
      {
        distortionVSD = compute_VSD_FlexDepth_UN(p_Vid,&p_Vid->pCurImg[currMB->opix_y], &p_Vid->enc_picture->p_curr_img[currMB->pix_y], &img_Texture[tex_pic_y], currMB->pix_x, currMB->pix_x, p_Inp->dispCoeff, p_Inp->OriginalWidth, 16,1,min_rdcost,grid_posx);
      }else
      {
