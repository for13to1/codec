...
#if EXT3D
    // @DT: Please ask ITRI double check this section..
...
#if EXT3D
    if(p_Vid->is_depth && (p_Vid->NonlinearDepthNum>0))
    {
      get_mem2Dpel(&imgX_linear_Y,  p_Vid->height, p_Vid->width);
      nonlinear_depth_process_buf2d_imgpel( imgX_linear_Y, p_Vid->imgSRC.data[0], p_Vid->width, p_Vid->height, p_Vid->NonlinearDepthNum, p_Vid->NonlinearDepthPoints, 0 );  
      p_Vid->imgSRC.data[0] = imgX_linear_Y;
    }
#endif
