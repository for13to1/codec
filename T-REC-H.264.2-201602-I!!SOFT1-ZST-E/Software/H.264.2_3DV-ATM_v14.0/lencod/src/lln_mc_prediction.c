                ...
                PicMotionParams **mv_info, int i, int j, int block_size_x, int block_size_y, short bipred_me)
{
  ...
  int apply_weights = ( ((p_Vid->active_pps->weighted_pred_flag||p_Vid->p_Inp->DepthRangeBasedWP)  && (currSlice->slice_type== P_SLICE || currSlice->slice_type == SP_SLICE)) ||
    ((p_Vid->active_pps->weighted_bipred_idc||p_Vid->p_Inp->DepthRangeBasedWP) && (currSlice->slice_type== B_SLICE)));
#else
                            ...
                            PicMotionParams **mv_info, int i, int j, int block_size_x, int block_size_y)
{
  ...
  UNREFERENCED_PARAMETER(l0_mode);
  UNREFERENCED_PARAMETER(l1_mode);
