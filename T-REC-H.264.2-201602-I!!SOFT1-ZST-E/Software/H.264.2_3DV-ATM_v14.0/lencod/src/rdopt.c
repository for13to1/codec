...
{
#if EXT3D
  memcpy(&currSlice->all_mv [LIST_0][0][0][0][0], &rdopt->all_mv [LIST_0][0][0][0][0], 352 * currSlice->max_num_references * sizeof(MotionVector));  
#else
...
#if EXT3D 
  free_mem4Dmv(p_RDO->std_all_mv8x8);

  free_mem4Dshort(p_RDO->isVSPRef8x8);
  free_mem4Dshort(p_RDO->std_isVSPRef8x8);
#endif
...
#if EXT3D 
  get_mem4Dmv(&p_RDO->std_all_mv8x8, 2, 2, 4, 4);
  p_RDO->all_mv8x8_size = 64 * sizeof(MotionVector);

  get_mem4Dshort(&p_RDO->isVSPRef8x8,2,2,4,4);
  get_mem4Dshort(&p_RDO->std_isVSPRef8x8,2,2,4,4);
  p_RDO-> std_isVSPRef8x8_size = 64*sizeof(short);
#endif
...
#if EXT3D
  if ( currMB->p_Inp->VSD && p_Vid->is_depth )
  {
    ...
    int tex_pic_y =  (pic_pix_y <<p_Vid->depth_ver_rsh)/p_Vid->depth_ver_mult-grid_posy;

    if ( p_Vid->p_dual_picture )
      img_Texture = p_Vid->p_dual_picture->imgY;
    else
      img_Texture = p_Vid->p_DualVid->imgData0.frm_data[0];

    distortionD = compute_SSE4x4(&p_Vid->pCurImg[pic_opix_y], &p_Vid->enc_picture->imgY[pic_pix_y], pic_pix_x, pic_pix_x);

    if (pic_pix_x>=p_Vid->DepthCropLeftCoord && (pic_pix_x+3)<=p_Vid->DepthCropRightCoord && pic_pix_y>=p_Vid->DepthCropTopCoord && (pic_pix_y +3)<=p_Vid->DepthCropBottomCoord )
    {
      distortionVSD = compute_VSD_FlexDepth_UN(p_Vid, &p_Vid->pCurImg[pic_opix_y], &p_Vid->enc_picture->imgY[pic_pix_y], &img_Texture[tex_pic_y], pic_pix_x, pic_pix_x, currMB->p_Inp->dispCoeff, currMB->p_Inp->OriginalWidth, 4,0,0,grid_posx);
    }else
    {
      distortionVSD=distortionD;
    }

    distortion += ( currMB->p_Inp->vsdWeight * distortionVSD + currMB->p_Inp->dWeight * distortionD ) / ( currMB->p_Inp->vsdWeight + currMB->p_Inp->dWeight );
  }
  else
#endif
                                       ...
                                       distblk     min_rdcost)
{
  ...
  UNREFERENCED_PARAMETER(min_rdcost);
                             ...
                             distblk  min_rdcost)     
{
  ...
  {
#if EXT3D
    if(pdir==2 && ((active_pps->weighted_bipred_idc == 1)||(currSlice->depth_range_wp)))
#else
    if (pdir == 2 && active_pps->weighted_bipred_idc == 1)
#endif
    {
#if EXT3D
      int weight_sum = ((active_pps->weighted_bipred_idc == 1)||(currSlice->depth_range_wp))? 
        currSlice->wbp_weight[0][l0_ref][l1_ref][0] + currSlice->wbp_weight[1][l0_ref][l1_ref][0] : 0;
#else
  ...
  {    
    ...
    if ( p_Inp->VSD && p_Vid->is_depth )
    {
      ...
      int grid_posy=p_Vid->grid_pos_y[cur_view_id];
      int grid_posx=p_Vid->grid_pos_x[cur_view_id];
      int tex_pic_y = ((currMB->pix_y+pay) <<p_Vid->depth_ver_rsh)/p_Vid->depth_ver_mult-grid_posy;
      ...
      distortionD = compute_SSE8x8(&p_Vid->pCurImg[currMB->opix_y + pay], &p_Vid->enc_picture->imgY[currMB->pix_y + pay], currMB->pix_x + pax, currMB->pix_x + pax);

      if ((currMB->pix_x + pax)>=p_Vid->DepthCropLeftCoord && (currMB->pix_x + pax+7)<=p_Vid->DepthCropRightCoord && (currMB->pix_y + pay)>=p_Vid->DepthCropTopCoord && (currMB->pix_y + pay +7)<=p_Vid->DepthCropBottomCoord )
      {
        distortionVSD = compute_VSD_FlexDepth_UN( p_Vid,&p_Vid->pCurImg[currMB->opix_y + pay], &p_Vid->enc_picture->imgY[currMB->pix_y + pay], &img_Texture[tex_pic_y], currMB->pix_x + pax, currMB->pix_x + pax, currMB->p_Inp->dispCoeff, p_Inp->OriginalWidth, 8,0,0,grid_posx);
      }else
      {
        distortionVSD=distortionD;
      }
...
void set_modes_and_refs_for_blocks_p_slice(Macroblock *currMB, short mode)
{
  ...
  //--- reference frame arrays ---
#if EXT3D
  if (mode==0 || mode==I4MB || mode==I16MB || mode==I8MB || mode == IPCM || mode == SI4MB|| mode == PVSP_SKIP)
#else
    ...
    if (!mode) // Skip
    {
      for (j = currMB->block_y; j < currMB->block_y + 4; j++)
...
#if EXT3D
        {
          ...
          {         
            motion[j][i].ref_idx[LIST_0] = (char)(currMB->best_ref[0]>=0?currMB->best_ref[0]:0);          
          }
          else
          {
  ...
  else
  {
...
#if EXT3D
          motion[j][i].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0+currMB->list_offset][curref][mode][j-currMB->block_y][i-currMB->block_x];
#endif
        }
      }
    }
    ...
    else
    {
      ...
      for (j = 0; j < 4; j++)
      {
        ...
        k = 2*(j >> 1);
#if EXT3D
        b8x8mode = b8x8info->best[mode][k].mode;
#endif
        curref = b8x8info->best[mode][k++].ref[LIST_0];
#if EXT3D
        motion[block_y][block_x].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0 + currMB->list_offset][curref][b8x8mode][j][0];
#endif
...
#if EXT3D
        motion[block_y][block_x].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0 + currMB->list_offset][curref][b8x8mode][j][1];
#endif
        motion[block_y][block_x].ref_pic [LIST_0] = currSlice->listX[LIST_0 + currMB->list_offset][curref];
        motion[block_y][block_x++].ref_idx[LIST_0] = (char) curref;
#if EXT3D
        ...
        curref = b8x8info->best[mode][k].ref[LIST_0];
#if EXT3D
        motion[block_y][block_x].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0 + currMB->list_offset][curref][b8x8mode][j][2];
#endif
        motion[block_y][block_x].ref_pic [LIST_0] = currSlice->listX[LIST_0 + currMB->list_offset][curref];
        motion[block_y][block_x++].ref_idx[LIST_0] = (char) curref;
#if EXT3D
        motion[block_y][block_x].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0 + currMB->list_offset][curref][b8x8mode][j][3];
#endif
        motion[block_y][block_x].ref_pic [LIST_0] = currSlice->listX[LIST_0 + currMB->list_offset][curref];
        motion[block_y][block_x].ref_idx[LIST_0] = (char) curref;
      }
...
#if EXT3D
              motion[block_y][block_x].isVSPRef[clist] = (curref>=0)?currSlice->isVSPRef[clist][(int)curref][mode][2 * (block8x8 >> 1) + (block4x4 >> 1)][2 * (block8x8 & 0x01) + (block4x4 & 0x01)]:0;
#endif
        }
      }
    }
...
#if EXT3D
          motion[block_y][block_x].isVSPRef[LIST_0] = (motion[block_y][block_x].ref_idx[LIST_0]>=0)?
              currSlice->isVSPRef[LIST_0][(int)(motion[block_y][block_x].ref_idx[LIST_0])][(int)(b8x8info->best[mode][k].mode)][j][i]:0;
          motion[block_y][block_x].isVSPRef[LIST_1] = (motion[block_y][block_x].ref_idx[LIST_1]>=0)?
              currSlice->isVSPRef[LIST_1][(int)(motion[block_y][block_x].ref_idx[LIST_1])][(int)(b8x8info->best[mode][k].mode)][j][i]:0;
#endif
      }        
    }        
  }
...
void set_coeff_and_recon_8x8_p_slice (Macroblock* currMB)
{
...
#if EXT3D 
    /*
    code below could corrupt context for ALC if forced residuals setting to zero is applied. 
    That is why currently this code is not allowed for ALC. 
    */
    //restore reconstruction
    if (p_RDO->tr8x8->cnt_nonz_8x8 <= _LUMA_8x8_COEFF_COST_ &&
      ((currMB->qp_scaled[0])!=0 || p_Vid->lossless_qpprime_flag==0) &&
      (currSlice->slice_type != SP_SLICE) && !currMB->is_alc_mode)// modif ES added last condition (we probably never go there so is the next modification useful ? check)
#else
  ...
  else
  {
    ...
    That is why currently this code is not allowed for ALC. 
    */
    if (((p_Inp->disthres && p_RDO->tr4x4->cnt_nonz_8x8 <= 0) || (p_RDO->tr4x4->cnt_nonz_8x8 <= _LUMA_MB_COEFF_COST_)) && currSlice->slice_type != SP_SLICE &&
      ((currMB->qp_scaled[0])!=0 || p_Vid->lossless_qpprime_flag==0) && !currMB->is_alc_mode)
#else
...
void store_macroblock_parameters (Macroblock *currMB, int mode)
{
...
#if EXT3D 
  currMB->best_mode_changed = 1;
  currSlice->is_alc_mode = currMB->is_alc_mode;
#endif
  ...
  //--- coeff, cbp, kac ---
#if EXT3D
  if ((mode != 0 && mode != PVSP_SKIP) || currSlice->slice_type == B_SLICE)
#else
...
#if EXT3D 
  currMB->is_alc_mode = currSlice->is_alc_mode;
#endif
...
#if EXT3D 
    if (currMB->is_alc_mode && 0 == (currMB->cbp & 15) && p_Inp->Transform8x8Mode)
      currMB->luma_transform_size_8x8_flag = currMB->temp_transform_size_8x8_flag;
#endif
  }
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      k = 2*(j >> 1)+(i >> 1);
          ...
          if(currMB->mb_type==PVSP_SKIP)
          {
            ...
            motion[block_y][block_x].ref_pic [LIST_0] = NULL;
            motion[block_y][block_x].mv      [LIST_0] = zero_mv;
            motion[block_y][block_x].isVSPRef[LIST_0] = 0;
          }
          else
          {
#endif
      ...
      {
#if EXT3D
        motion[block_y][block_x].isVSPRef[LIST_0] = currSlice->isVSPRef[LIST_0][(short)p_RDO->l0_refframe[j][i]][(short) currMB->b8x8[k].mode][j][i];
#endif
        if (currMB->b8x8[k].bipred && (currMB->b8x8[k].pdir == 2) && is_bipred_enabled(p_Vid, currMB->mb_type))
        {
          motion[block_y][block_x].ref_idx [LIST_0] = 0;
          ...
          motion[block_y][block_x].mv      [LIST_0] = currSlice->bipred_mv[currMB->b8x8[k].bipred - 1][LIST_0][0][(short) currMB->b8x8[k].mode][j][i]; 
#if EXT3D
          if (motion[block_y][block_x].mv[LIST_0].mv_x != currSlice->bipred_mv_implicit[currMB->b8x8[k].bipred - 1][LIST_0][0][(short) currMB->b8x8[k].mode][j][i].mv_x ||
              motion[block_y][block_x].mv[LIST_0].mv_y != currSlice->bipred_mv_implicit[currMB->b8x8[k].bipred - 1][LIST_0][0][(short) currMB->b8x8[k].mode][j][i].mv_y)
            motion[block_y][block_x].isVSPRef[LIST_0] = 0;
#endif
        }
        else
      ...
      if ((currMB->b8x8[k].pdir == 0) || IS_INTRA(currMB))
      {
        ...
        motion[block_y][block_x].mv      [LIST_1] = zero_mv;
#if EXT3D
              motion[block_y][block_x].isVSPRef[LIST_1] = 0;
#endif
            }
#if EXT3D
        }
  ...
  if (currSlice->slice_type == B_SLICE)
  {
    for (j=0; j<4; j++)
    {
      ...
      for (i=0; i<4; i++)
      {
        ...
        {
#if EXT3D
          motion[block_y][block_x].isVSPRef[LIST_1] = currSlice->isVSPRef[LIST_1][(short)p_RDO->l1_refframe[j][i]][(short) currMB->b8x8[k].mode][j][i];;
#endif
          if (currMB->b8x8[k].bipred && (currMB->b8x8[k].pdir == 2) && is_bipred_enabled(p_Vid, currMB->mb_type))
          {
            motion[block_y][block_x].ref_idx [LIST_1] = 0;
            motion[block_y][block_x].ref_pic [LIST_1] = currSlice->listX[LIST_1 + currMB->list_offset][0];
            motion[block_y][block_x].mv      [LIST_1] = currSlice->bipred_mv[currMB->b8x8[k].bipred - 1][LIST_1][0][(short) currMB->b8x8[k].mode][j][i]; 
            if (motion[block_y][block_x].mv[LIST_1].mv_x != currSlice->bipred_mv_implicit[currMB->b8x8[k].bipred - 1][LIST_1][0][(short) currMB->b8x8[k].mode][j][i].mv_x ||
                motion[block_y][block_x].mv[LIST_1].mv_y != currSlice->bipred_mv_implicit[currMB->b8x8[k].bipred - 1][LIST_1][0][(short) currMB->b8x8[k].mode][j][i].mv_y)
              motion[block_y][block_x].isVSPRef[LIST_1] = 0;
          }
          else
          {
...
#if EXT3D
  short *****isVSPRef = currSlice->isVSPRef;
  short **lc_l0_isVSPRef8x8 = p_RDO->isVSPRef8x8[dir][LIST_0];
  short **lc_l1_isVSPRef8x8 = p_RDO->isVSPRef8x8[dir][LIST_1];
#endif
...
#if EXT3D
      memcpy(&lc_l0_isVSPRef8x8[j0][i0], &isVSPRef[LIST_0][l0_ref][mode][j0][i0],  2 * sizeof(short));
      memcpy(&lc_l0_isVSPRef8x8[j1][i0], &isVSPRef[LIST_0][l0_ref][mode][j1][i0],  2 * sizeof(short));
#endif
    }
  }
  else
...
#if EXT3D
      memcpy(&lc_l1_isVSPRef8x8[j0][i0], &isVSPRef[LIST_1][l1_ref][mode][j0][i0],  2 * sizeof(short));
      memcpy(&lc_l1_isVSPRef8x8[j1][i0], &isVSPRef[LIST_1][l1_ref][mode][j1][i0],  2 * sizeof(short));
#endif
    }
...
#if EXT3D
      memcpy(&isVSPRef[LIST_0][l0_ref][4][j0][i0],&lc_l0_isVSPRef8x8[j0][i0],2*sizeof(short));
      memcpy(&isVSPRef[LIST_0][l0_ref][4][j1][i0],&lc_l0_isVSPRef8x8[j1][i0],2*sizeof(short));
#endif
    }
  }
  else
...
#if EXT3D
      memcpy(&isVSPRef[LIST_0][l0_ref][mode][j0][i0],&lc_l0_isVSPRef8x8[j0][i0],2*sizeof(short));
      memcpy(&isVSPRef[LIST_0][l0_ref][mode][j1][i0],&lc_l0_isVSPRef8x8[j1][i0],2*sizeof(short));
#endif
    }
    else if (pdir8==1) // list1
    {
...
#if EXT3D
      memcpy(&isVSPRef[LIST_1][l1_ref][mode][j0][i0],&lc_l1_isVSPRef8x8[j0][i0],2*sizeof(short));
      memcpy(&isVSPRef[LIST_1][l1_ref][mode][j1][i0],&lc_l1_isVSPRef8x8[j1][i0],2*sizeof(short));
#endif
    }
    else if (pdir8==2) // bipred
    {
...
void StoreNewMotionVectorsBlock8x8(Slice *currSlice, int dir, int block8x8, Info8x8 *B8x8Info)
{
...
#if EXT3D
  short ****l0_isVSPRef = currSlice->isVSPRef[LIST_0];
  short ****l1_isVSPRef = currSlice->isVSPRef[LIST_1];
  short **lc_l0_isVSPRef8x8 = &p_RDO->isVSPRef8x8[dir][LIST_0][j0];
  short **lc_l1_isVSPRef8x8 = &p_RDO->isVSPRef8x8[dir][LIST_1][j0];
#endif
...
#if EXT3D
    memset(&lc_l0_isVSPRef8x8[0][i0], 0, 2 * sizeof(short));
    memset(&lc_l0_isVSPRef8x8[1][i0], 0, 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[0][i0], 0, 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[1][i0], 0, 2 * sizeof(short));
#endif
    return;
  }
  ...
  if (currSlice->slice_type != B_SLICE)
  {
    ...
    memset(&lc_l1_mv8x8[1][i0], 0, 2 * sizeof(MotionVector));
#if EXT3D
    memcpy(&lc_l0_isVSPRef8x8[0][i0], &l0_isVSPRef[l0_ref][mode][j0][i0], 2 * sizeof(short));
    memcpy(&lc_l0_isVSPRef8x8[1][i0], &l0_isVSPRef[l0_ref][mode][j1][i0], 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[0][i0], 0, 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[1][i0], 0, 2 * sizeof(short));
#endif
    return;
...
#if EXT3D
      memcpy(&lc_l1_isVSPRef8x8[0][i0], &l1_isVSPRef[l1_ref][mode][j0][i0], 2 * sizeof(short));
      memcpy(&lc_l1_isVSPRef8x8[1][i0], &l1_isVSPRef[l1_ref][mode][j1][i0], 2 * sizeof(short));
#endif
    }
    else
    {
...
void store_8x8_motion_vectors_p_slice(Slice *currSlice, int dir, int block8x8, Info8x8 *B8x8Info)
{
  ...
  else
  {
...
#if EXT3D
    short ** l0_isVSPRef = currSlice->isVSPRef[LIST_0][(short) B8x8Info->ref[LIST_0]][(short)B8x8Info->mode];
#endif
    ...
    memset(&lc_l1_mv8x8[1][i0], 0, 2 * sizeof(MotionVector));
#if EXT3D
    memcpy(&lc_l0_isVSPRef8x8[0][i0], &l0_isVSPRef[j0][i0], 2 * sizeof(short));
    memcpy(&lc_l0_isVSPRef8x8[1][i0], &l0_isVSPRef[j1][i0], 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[0][i0], 0, 2 * sizeof(short));
    memset(&lc_l1_isVSPRef8x8[1][i0], 0, 2 * sizeof(short));
#endif
  } 
...
distblk compute_satd4x4_cost(VideoParameters *p_Vid, imgpel **cur_img, imgpel **prd_img, int pic_opix_x, distblk min_cost)
{
  ...
  UNREFERENCED_PARAMETER(min_cost);
