...
void SetMotionVectorsMBPSlice (Macroblock* currMB)
{
  ...
  {
#if EXT3D
    memcpy(&rdopt->all_mv[LIST_0][0][0][0][0], &all_mv[0][0][0][0], p_Vid->max_num_references * 11 * MB_BLOCK_PARTITIONS * sizeof(MotionVector));
#else
  ...
  if (currMB->mb_type == PSKIP) // Skip mode
  {
    ...
    if(currSlice->depth_based_mvp_flag)
      ...
      l0_ref=0;
    CopyMVBlock16(mv_info, all_mv[l0_ref][PSKIP], LIST_0, currMB->block_x, currMB->block_y, 0, 4);
#else
    CopyMVBlock16(mv_info, all_mv[0][PSKIP], LIST_0, currMB->block_x, currMB->block_y, 0, 4);
#endif
  }
#if EXT3D
  else if (currMB->mb_type == PVSP_SKIP)
  {
    // Set zero MVs, it should be OK to copy from all_mv
    ResetMVBlock16(mv_info, LIST_0, currMB->block_x, currMB->block_y, 0, 4);
  }
#endif
...
void SetMotionVectorsMBBSlice (Macroblock* currMB)
{
  ...
  {
#if EXT3D
    memcpy(&currSlice->rddata->all_mv[LIST_0][0][0][0][0], &currSlice->all_mv[LIST_0][0][0][0][0], 2 * p_Vid->max_num_references * 11 * MB_BLOCK_PARTITIONS * sizeof(MotionVector));
#else
...
#if EXT3D
  else if(currMB->mb_type == BVSP_DIRECT)
  {
    ResetMVBlock16(motion, LIST_0, currMB->block_x, currMB->block_y, 0, 4);
    ResetMVBlock16(motion, LIST_1, currMB->block_x, currMB->block_y, 0, 4);
  }
#endif
  else // Intra modes
  {
