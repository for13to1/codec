...
#if EXT3D
  for (i=0;i<30;i++)
    name[i] = p_Inp->InputFile[0].fname[i + imax(0,(int) (strlen(p_Inp->InputFile[0].fname)- 30))]; // write last part of path, max 30 chars
#else
  for (i=0;i<30;i++)
...
#if EXT3D
  if (p_Vid->curr_frm_idx == 0 && p_Vid->frame_num == 0 && !voidx)
#else
...
#if EXT3D
  for (i=0; i < 40; i++)
    name[i] = p_Inp->InputFile[0].fname[i + imax(0, ((int) strlen(p_Inp->InputFile[0].fname)) - 40)]; // write last part of path, max 40 chars
#else
  for (i=0; i < 40; i++)
...
void report( VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats)
{
  ...
  int64 total_bits;
#if EXT3D
  int64 total_bits_v[MAX_CODEVIEW];
#else
#if (MVC_EXTENSION_ENABLE)
  ...
  if (p_Inp->Verbose != 0)
  {
...
#if EXT3D
    DistMetric *snr_v[MAX_CODEVIEW] = {NULL};
    DistMetric *sse_v[MAX_CODEVIEW] = {NULL};
    float csnr_y_v[MAX_CODEVIEW] = {0.0};
    float csnr_u_v[MAX_CODEVIEW] = {0.0};
    float csnr_v_v[MAX_CODEVIEW] = {0.0};
    int  i=0;
    ...
    if(p_Inp->NumOfViews>1)
    {
      ...
      {
        snr_v[i]=&p_Dist->metric_v[i][PSNR];
        sse_v[i]=&p_Dist->metric_v[i][SSE];
        csnr_y_v[i] = psnr(p_Vid->max_imgpel_value_comp_sq[0], impix   , sse_v[i]->average[0]);
        csnr_u_v[i] = psnr(p_Vid->max_imgpel_value_comp_sq[1], impix_cr, sse_v[i]->average[1]);
        csnr_v_v[i] = psnr(p_Vid->max_imgpel_value_comp_sq[2], impix_cr, sse_v[i]->average[2]);
      }
    }
#else
#if (MVC_EXTENSION_ENABLE)
    ...
    if(p_Inp->NumOfViews>1)
    {
      ...
      {
        fprintf( stdout, "\n" );
        fprintf(stdout," View%2d_Y { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n", 
          p_Inp->ViewCodingOrder[i],snr_v[i]->average[0], csnr_y_v[i], sse_v[i]->average[0]/(float)impix);
        fprintf(stdout," View%2d_U { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
          p_Inp->ViewCodingOrder[i],snr_v[i]->average[1], csnr_u_v[i], sse_v[i]->average[1]/(float)impix_cr);
        fprintf(stdout," View%2d_V { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, %9.5f }\n",
          p_Inp->ViewCodingOrder[i],snr_v[i]->average[2], csnr_v_v[i], sse_v[i]->average[2]/(float)impix_cr);
      }
    }
#else
#if (MVC_EXTENSION_ENABLE)
...
#else
  if((p_Inp->NumOfViews>1)&&(p_Vid->fld_flag))
#endif
  {
    int64 all_view_bits_v=0;
    int64 all_view_bit_counter_v[NUM_SLICE_TYPES]={0};
    int64 all_view_bit_ctr_parametersets_v=0;
    for(i=0;i<p_Inp->NumOfViews;++i)
    {
      all_view_bits_v+=total_bits_v[i];
      all_view_bit_counter_v[I_SLICE]+=p_Stats->bit_counter_v[i][I_SLICE] ;
      all_view_bit_counter_v[P_SLICE]+=p_Stats->bit_counter_v[i][P_SLICE] ;
      if(p_Stats->frame_ctr[B_SLICE] != 0)
        all_view_bit_counter_v[B_SLICE]+=p_Stats->bit_counter_v[i][B_SLICE] ;
      all_view_bit_ctr_parametersets_v+=p_Stats->bit_ctr_parametersets_v[i]    ;
    }
#if ITRI_INTERLACE
    total_bits = all_view_bits_v; // Original total_bits is lack of NVB for interlace coding mode
#endif
    if (p_Stats->frame_ctr[B_SLICE] != 0)
    {

      fprintf(stdout, " Total bits                        : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", B %" FORMAT_OFF_T  " NVB %" FORMAT_OFF_T  ") \n",
        all_view_bits_v, 
        all_view_bit_counter_v[I_SLICE],
        all_view_bit_counter_v[P_SLICE], 
        all_view_bit_counter_v[B_SLICE], 
        all_view_bit_ctr_parametersets_v);
    }
    else
    {
      fprintf(stdout, " Total bits                        : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", NVB %" FORMAT_OFF_T  ") \n",
        all_view_bits_v, 
        all_view_bit_counter_v[I_SLICE],
        all_view_bit_counter_v[P_SLICE], 
        all_view_bit_ctr_parametersets_v);
  ...
  if (p_Inp->NumOfViews >1)
  {
    ...
    for ( voidx = 0; voidx < p_Inp->NumOfViews; voidx++ )
    {
      if (p_Stats->frame_ctr[B_SLICE] != 0)
      {
        fprintf(stdout, " View %1d Total-bits                 : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", B %" FORMAT_OFF_T  " NVB %d) \n",
          p_Inp->ViewCodingOrder[voidx], total_bits_v[voidx],  p_Stats->bit_counter_v[voidx][I_SLICE], p_Stats->bit_counter_v[voidx][P_SLICE], p_Stats->bit_counter_v[voidx][B_SLICE], p_Stats->bit_ctr_parametersets_v[voidx]);
      }
      else
      {
        fprintf(stdout, " View %1d Total-bits                 : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", NVB %d) \n",
          p_Inp->ViewCodingOrder[voidx], total_bits_v[voidx], p_Stats->bit_counter_v[voidx][I_SLICE], p_Stats->bit_counter_v[voidx][P_SLICE], p_Stats->bit_ctr_parametersets_v[voidx]);
      }
    }
  }
  ...
  if (p_Inp->NumOfViews >1)
  {
    p_Stats->bitrate= ((float) total_bits * (float) p_Inp->output.frame_rate) / ((float) ((p_Stats->frame_counter)/(p_Inp->NumOfViews)));
  }
  else
#else
#if (MVC_EXTENSION_ENABLE)
...
#if EXT3D
  p_Stats->total_bits=total_bits;
  if(p_Inp->NumOfViews>1)
  {
    for(i=0;i<p_Inp->NumOfViews;++i)
    {
      p_Stats->bitrate_v[i]=((float) total_bits_v[i] * (float) p_Inp->output.frame_rate) / ((float) (p_Stats->frame_counter /(p_Inp->NumOfViews)));
      fprintf(stdout, " View %d BR (kbit/s)  @ %2.2f Hz    : %5.2f\n",p_Inp->ViewCodingOrder[i], p_Inp->output.frame_rate, p_Stats->bitrate_v[i] / 1000.0);
    }
  }
#else
#if (MVC_EXTENSION_ENABLE)
...
void threeDV_information_init( VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats )
{
  ...
  for(i=0;i<p_TextInp->NumOfViews;++i)
  {
    fprintf(stdout,  " Texture Input YUV file  %d        : %s \n", i+1,p_TextInp->InputFile[i].fname);
  }
  for(i=0;i<p_Inp->NumOfViews;++i)
  {
    fprintf(stdout,  " Depth   Input YUV file  %d        : %s \n", i+1,p_Inp->InputFile[i].fname);
  }
  ...
  if (p_Inp->Verbose != 0)
  {
    ...
    fprintf(stdout,  " References for P slices           : Texture( %d ) Depth( %d )\n", p_TextInp->P_List0_refs ? p_TextInp->P_List0_refs : p_TextInp->num_ref_frames,
      p_Inp->P_List0_refs ? p_Inp->P_List0_refs : p_Inp->num_ref_frames);
    fprintf(stdout,  " References for B slices (L0, L1)  : Texture( %d, %d ) Depth( %d, %d )\n", 
      p_TextInp->B_List0_refs ? p_TextInp->B_List0_refs : p_TextInp->num_ref_frames, 
      p_TextInp->B_List1_refs ? p_TextInp->B_List1_refs : p_TextInp->num_ref_frames,
      p_Inp->B_List0_refs ? p_Inp->B_List0_refs : p_Inp->num_ref_frames, 
      p_Inp->B_List1_refs ? p_Inp->B_List1_refs : p_Inp->num_ref_frames);
...
void information_init ( VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats)
{
...
#if EXT3D
  for(i=0;i<p_Inp->NumOfViews;++i)
    fprintf(stdout,  " Input YUV file  %d                  : %s \n", i+1,p_Inp->InputFile[i].fname);
#else
  ...
  {
    if(p_Vid->p_decMVC[i]!=-1)
      fprintf(stdout,  " Output YUV file  %d                : %s \n", i+1, p_Inp->ThreeDVReconFile[i]);
  }
#else
  if (p_Vid->p_dec != -1)
