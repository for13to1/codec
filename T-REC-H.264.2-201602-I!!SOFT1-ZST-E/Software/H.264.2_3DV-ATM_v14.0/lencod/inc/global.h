...
typedef struct macroblock
{
  ...
  int                 is_alc_mode;        //!< stores is_alc_mode of the best mb
  int                 best_mode_changed;  //!< true when best mb coding mode is changed by encode_one_macroblock_high()
...
#if EXT3D
  void (*GetInterviewMVPrediction)(struct macroblock *currMB,MotionVector*pmv,int block_y,int block_x,int blockshape_y,int blockshape_x,int * disparity_table);

  void (*GetInterviewMVPredictionMax)(struct macroblock *currMB,MotionVector*pmv,int block_y,int block_x,int blockshape_y,int blockshape_x,int * disparity_table);

  void (*GetMVPredictor) (struct macroblock *currMB, PixelPos *block, MotionVector *pmv, short ref_frame, 
                          struct pic_motion_params **mv_info, int list, int mb_x, int mb_y, int blockshape_x, int blockshape_y,int depth_based_mvp);
...
#if EXT3D 
/*! Temporary variables stores copies of common values when alc mode is checked.
    The common values will be changed wnen alc mode is checked.*/
  struct epzs_params *p_EPZS_std;
  struct search_window searchRange_std;
  SearchType SearchMode_std;
  distblk (*computeStdUniPred[3]) (struct storable_picture *ref1, struct me_block *, distblk , MotionVector *);
  distblk (*IntPelME_std) (struct macroblock *currMB, MotionVector *, struct me_block *mv_block, distblk, int);
  distblk (*SubPelME_std) (struct macroblock *currMB, MotionVector *pred_mv, struct me_block *mv_block, distblk min_mcost, int* lambda_factor);
  int   DisableIntraInInter_std;
#endif 
...
typedef struct slice
{
  ...
  int                 *reordering_of_interview_num_idc[2];
  int                  reordering_interview_flg[2];
...
#if EXT3D
  MotionVector *****all_mv_backup;
  short        *****isVSPRef;
  int               Harmonize_VSP_IVP;
  int               mv_backup_size;

  MotionVector ******bipred_mv_implicit;
  MotionVector ******bipred_mv_backup;
  int                bi_mv_backup_size;

  int               is_alc_mode;      //!< enables alc for mb
  ...
  int               check_alc_skip;      //!< true wnen it requires to check aLCskip mode only

/*! Temporary variables stores copies of common values when alc mode is checked.
    The common values will be changed wnen alc mode is checked.*/
  short             std_mvd[2][BLOCK_MULTIPLE][BLOCK_MULTIPLE][2];
  MotionVector *****all_std_mv;

  short        *****all_std_VSP;

  int               all_mv_size;

  byte              std_luma_transform_size_8x8_flag;
  byte              std_temp_transform_size_8x8_flag;
  byte              std_valid_8x8;
  byte              std_valid_4x4;
  Info8x8           std_part_4x4[4];
  Info8x8           std_part_8x8[4];

  int               std_best_ref[2];
  MotionVector      std_best_mvp[2];
#endif
...
#if EXT3D 
    struct epzs_params    *p_alcEPZS; //!< structure is used when epzs search is used for alc mb
#endif
...
#if EXT3D
  void (*writeMB_VSSkip)        (Macroblock *currMB, SyntaxElement *se, DataPartition *dP);
  void (*writeMB_DirectModeType)(Macroblock *currMB, SyntaxElement *se, DataPartition *dP);

  void (*writeMB_Ref_VSP_flag)  (Macroblock *currMB, int list, SyntaxElement *se, DataPartition *dP);
#endif
...
#if EXT3D
  int        frame_ctr_v[MAX_CODEVIEW];                //!< number of coded frames for each view
  DistMetric metric_v[MAX_CODEVIEW][TOTAL_DIST_TYPES]; //!< Distortion metrics for each view
#else
...
typedef struct video_par
{
...
#if EXT3D
  int                       is_depth;   //!<depth or not  
  struct  video_par*        p_DualVid; //!<the video parameters for depth(texture:1) or texture(texture:0)
  InputParameters*          p_DualInp;  //!<the input parameter for depth or texture (texture:1) or texture(texture:0)
  struct storable_picture*  p_dual_picture;//!<corresponding depth(texture) for texture(depth)
#endif
  ...
  signed int                ThisPOC1;       //!<current picture POC for depth or texture
  unsigned int              frame_num1;   //!<frame_num for depth or texture of this frame

  int                       ThreeDVCodingOrder[MAX_CODEVIEWPLUSDEPTH];

  int                       vs_ok[MAX_CODEVIEW];
  int                       encoder_ok[MAX_CODEVIEW];
  ...
  double                ****d_disparity_lut; //!< disparity look up table [source_view_idx][target_view_idx][0:quarter-pel, 1:half-pel][depth_value]
  int                   ****i_disparity_lut; //!< disparity look up table [source_view_idx][target_view_idx][0:quarter-pel, 1:half-pel][depth_value]
  ...
  int                       SliceHeaderPred;
  int                       dec_view_flag[MAX_CODEVIEW][2];
#if FIX_SLICE_HEAD_PRED
  int                       dec_view_slice_type[MAX_CODEVIEW][2];
#endif
  int                       PredSliceHeaderSrc;
  int                       PredRefListsSrc;
  int                       PredWeightTableSrc;
  int                       PredDecRefPicMarkingSrc;
  ...
  int                       low_res;
  int                       mixed_res;                      //!<the resolution of depth is difference from texture
  int                       width_ori;
  int                       height_ori;
  int                       width_cr_ori;
  int                       height_cr_ori;

  ResizeParameters         *p_output_upsample_params;             //!<upsampling from depth/texture's encoding resolution to original resolution
  ResizeParameters         *p_switch_upsample_params;            //!<upsampling from depth's resolution to texture's resolution 
#endif
  ...
  int idr_refresh;
#if EXT3D
  int p_decMVC[MAX_CODEVIEW];   //!<For MVC support
  int p_dec;                    //!<For AVC
#else
  ...
  int NonBaseViewForeFld;      ////!< if view 0 is coded as FLD then force also view to be coded as FLD as well
  int last_mmco_frm_num;
  int LastIDRFrameNum;
  ...
  //!<reconstructed camera info 
  double rec_focal_length_x[MAX_CODEVIEW];
  double rec_focal_length_y[MAX_CODEVIEW];
  double rec_principal_point_x[MAX_CODEVIEW];
  double rec_principal_point_y[MAX_CODEVIEW];
  double rec_translation[MAX_CODEVIEW];


  double rec_depth_near[MAX_CODEVIEW];
  double rec_depth_far[MAX_CODEVIEW];

  int  forward_frm_num;
  int  backward_frm_num;

  //Needed for reference display info sei
  MultiviewAcquisitionInfoSEI* seiAcquisitionInfo;

  depth_parameter_set_rbsp_t  *DepParSet[MAXDPS];
  ThreeDVAcquisitionInfo      *ThreeDV_acquisition_info;

  int TexVoidxDelta[MAX_CODEVIEW];
  ...
  PictureStructure temp0_structure;
  ...
  PictureStructure temp1_structure;  
...
#if EXT3D
  int isTextureFirst; // @DT: p_Vid->isTextureFirst: can be equal to 1 for texture first coding or depth first coding. Had better to rename it.
  ...
  int depth_width_padded;
  int depth_height_padded;
  int text_width_padded;
  int text_height_padded;
  ...
  int CodingDepthView[MAX_CODEVIEW];
...
#if EXT3D
  InputParameters   *p_InpText;          //!< Input Parameters
  VideoParameters   *p_VidText;          //!< Image Parameters
  InputParameters   *p_InpDepth;         //!< Input Parameters
  VideoParameters   *p_VidDepth;         //!< Image Parameters
  int                num_of_sps;
  int                num_of_pps;
...
#if EXT3D
extern void free_encoder_memory        (VideoParameters *p_Vid, InputParameters *p_Inp, VideoParameters*p_VidDepth, InputParameters*p_InpDepth);
#else
