...
#if EXT3D
  SEI_MULTIVIEW_VIEW_POSITION, // @DT: Moved into 3D, double check
  SEI_DISPLAY_ORIENTATION,     // @DT: Moved into 3D, double check

  SEI_3DVC_SCALABLE_NESTING, 
  SEI_3DVC_VIEW_SCALABILITY,

  SEI_3DV_DEPTH_REPRESENTATION_INFO, // @DT: Moved into 3D, double check

  SEI_3DV_REFERENCE_DISPLAY_INFO,
#endif
...
typedef struct
{
  Boolean operation_point_flag;
  Boolean all_view_components_in_au_flag;
  int num_view_components_minus1;
  int sei_view_id[MAX_CODEVIEW];
  Boolean sei_view_applicability_flag[MAX_CODEVIEW];
  Boolean sei_op_texture_only_flag;
  int num_view_components_op_minus1;
  int sei_op_view_id[MAX_CODEVIEW];
  int sei_op_temporal_id;

  Bitstream *data;
  int payloadSize;  
} threedvc_scalable_nesting;

typedef struct
{
  int     num_views_minus1;
  Boolean intrinsic_param_flag;
  Boolean extrinsic_param_flag;
  Boolean intrinsic_params_equal;

  MultiviewAcquisitionInfoSEI*       multiview_acquisition_update_info_sei;
...
typedef struct
{
  Boolean all_views_equal_flag;
  int     num_views_minus1;
  Boolean z_near_flag;
  Boolean z_far_flag;
  Boolean z_axis_equal_flag;
  int     common_z_axis_reference_view;
  Boolean d_min_flag;
  Boolean d_max_flag;
  int     depth_representation_type;

  int     depth_info_view_id[MAX_CODEVIEW];
  int     z_axis_reference_view[MAX_CODEVIEW];
  int     disparity_reference_view[MAX_CODEVIEW];

  DepthAcquisitionInfoSEI*       depth_acquisition_update_info_sei;
...
typedef struct
{
  Boolean ref_viewing_distance_flag;
  int32 num_ref_displays_minus1;

  Boolean additional_shift_present_flag [MAX_DISPLAYS];
  int32 num_sample_shift_plus512 [MAX_DISPLAYS];
  Boolean three_dimensional_extension_flag;

  ReferenceDisplayInfoSEI* reference_display_update_info_sei;
...
#if EXT3D
extern void PrepareReferenceDisplaySEIMessage(VideoParameters *p_Vid);
extern void PrepareAggregationNestingSEIMessage(VideoParameters *p_Vid);
#endif
...
#if EXT3D
extern void ClearDepthRepresentationInfo(SEIParameters *p_SEI);
extern void UpdateDepthRepresentationInfo(VideoParameters *p_Vid);
extern void ClearMultiviewAcquisitionInfo(SEIParameters *p_SEI);
extern void UpdateMultiviewAcquisitionInfo(VideoParameters *p_Vid);
extern void Clear3DVCScalableNesting(SEIParameters *p_SEI);
extern void Update3DVCScalableNesting(VideoParameters *p_Vid);
extern void ClearReferenceDisplayInfo(SEIParameters *p_SEI);
extern void UpdateReferenceDisplayInfo(VideoParameters *p_Vid);
#endif
