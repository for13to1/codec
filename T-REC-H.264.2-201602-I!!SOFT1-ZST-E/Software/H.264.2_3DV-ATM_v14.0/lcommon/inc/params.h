...
struct inp_par_enc
{
...
#if EXT3D
  char          ThreeDVConfigFileName[FILE_NAME_SIZE];  
  VideoDataFile InputFile[MAX_CODEVIEW];
  char          ThreeDVReconFile[MAX_CODEVIEW][FILE_NAME_SIZE];
  int           ViewCodingOrder[MAX_CODEVIEW];
  int           ViewQPOffset[MAX_CODEVIEW];
  char          ReorderAtAnchor[MAX_CODEVIEW][5];
  char          ReorderAtNonAnchor[MAX_CODEVIEW][5];
  int           NumOfFwdAnchorRefs[MAX_CODEVIEW];
  int           NumOfBwdAnchorRefs[MAX_CODEVIEW];
  int           NumOfFwdNoAnchorRefs[MAX_CODEVIEW];
  int           NumOfBwdNoAnchorRefs[MAX_CODEVIEW];
  int           FwdAnchorRefs[MAX_CODEVIEW][MAX_VIEWREFERENCE];
  int           BwdAnchorRefs[MAX_CODEVIEW][MAX_VIEWREFERENCE];
  int           FwdNoAnchorRefs[MAX_CODEVIEW][MAX_VIEWREFERENCE];
  int           BwdNoAnchorRefs[MAX_CODEVIEW][MAX_VIEWREFERENCE];
  int           NumOfViews;                                   //!< number of views to encode (1=1view, 2=2views)

  int           ViewPresentFlag[MAX_CODEVIEW];
  int           NumOfCodingView;

  int           CompatibilityCategory;                        //!<0: MVC-compatible 1: AVC-compatible

  int           GradualViewRefresh;
  int           InterPredictionAtAnchorOff;

  int      VSD;
  double   dispCoeff;
  double   dispCoeffBase;
  int      vsdWeight;
  int      dWeight;
  double   dRatio;

  char    VSDCfg[1024];
  int     posCam[MAX_VIEWS];
  double  dispCoeffSynPos;
  double  dispCoeffSynDir;

  int AcquisitionIdx;

  int CamOrder[MAX_CODEVIEW];
  //!< camera parameters
  double IntrinsicMatrix[MAX_CODEVIEW][3][3];
  double ExtrinsicMatrix[MAX_CODEVIEW][3][3];

  //!< Considering the variable of   camera parameters from one picture to another picture
  //!<in some sequence(GT_Fly), so, these variables were defined as pointer
  double *TranslationVector[MAX_CODEVIEW][3];
  double *ZNears[MAX_CODEVIEW];
  double *ZFars[MAX_CODEVIEW];

  //!<parameters for VSRS
  int DepthType;
  int Precision;
  int Filter;
  int BoundaryNoiseRemoval;
  int SynthesisMode;

  //!<precision for MVC update information
  int PrecFocalLength;
  int PrecPrincipalPoint;
  ...
  int PrecTranslation;
  int MantissaLengthDepthRange;

  // NOKIA_DISP_CALC_B0150
  int DisparityParamPrec;

  //int texture;
  int  ThreeDVCoding;            //!<Enable joint texture-depth encoding
  char ThreeDVCodingOrder[INPUT_TEXT_SIZE];//!<the texture-depth coding order

  int DepthRangeBasedWP;
  ...
  int  NonlinearDepthCfg;
  char NonlinearDepthModelCfg[2048];  
  int  NonlinearDepthThresholdCfg;
  ...
  char          ReferenceDisplayFile[FILE_NAME_SIZE];
  int           ReferenceDisplayInfoFlag;
  Boolean       ReferenceViewingDistanceFlag;
  int           NumberOfDiplays;
  double        ReferenceBaseline[MAX_DISPLAYS]; 
  int           BaselinePrecision;
  double        ReferenceWidth[MAX_DISPLAYS];
  int           WidthPrecsion;
  double        ReferenceViewingDistance[MAX_DISPLAYS];
  int           ViewingDistancePrecision;
  Boolean       NumSampleShiftFlag[MAX_DISPLAYS];
  int           NumSampleShift[MAX_DISPLAYS]; 
#endif
  ...
  int DisableTLRefReorder;
  int TLRefReorderMethod;
  int TLYRMMCOMethod;
  char NumberReferenceTL[INPUT_TEXT_SIZE];
  int  NumReferenceTL[MAX_TEMPORAL_LEVELS];
#endif
...
//////////////////////////////////////////////////////////////////////////
  ...
  int alc_search_range_y;  //!< vertical size of search zone for alc (in qpels)
  int alc_search_range_x;  //!< horizontal size of search zone for alc (in qpels)

  // NOKIA_DEPTH_SEI_C0162
  int DepthRepresentationInfoFlag;
  int MultiviewAcquisitionInfoFlag;

  int RLESkip;          //<! enables RLESkip

  int VSP_Enable;

  // @DT: Depth Grid
  int depth_hor_mult_minus1;
  int depth_ver_mult_minus1;
  int depth_hor_rsh;
  int depth_ver_rsh;
