...
#if EXT3D // G0239_FLEXRES_WRITE_OUT_PIC_FIX
    //Xuehui: original code didn't take  horizontal padding into account,here we fix it
    if (size_x == o_size_x && size_y == o_size_y)
    {
      for(i=0;i<size_y;i++)
        for(j=0;j<size_x;j++)
        {
          imgX[i][j]=buf[i*size_x+j];
        }
    }
...
#if EXT3D
  if(p_Inp->InputFile[0].is_interleaved)
  {
