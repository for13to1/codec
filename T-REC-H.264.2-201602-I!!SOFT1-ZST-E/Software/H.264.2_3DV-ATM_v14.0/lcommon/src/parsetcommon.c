...
#if EXT3D
depth_parameter_set_rbsp_t *AllocDPS ()
{
  depth_parameter_set_rbsp_t *p;

  if ((p=calloc (1, sizeof (depth_parameter_set_rbsp_t))) == NULL)
    no_mem_exit ("AllocDPS: DPS");
  get_mem_acquisition_info(&(p->acquisition_info));
  init_acquisition_info(p->acquisition_info);
  return p;
}
#endif
...
#if EXT3D
 void FreeDPS (depth_parameter_set_rbsp_t *dps)
 {
   assert (dps != NULL);
   if(dps->acquisition_info){
     free(dps->acquisition_info->focal_length_x_ae);
     free(dps->acquisition_info->focal_length_y_ae);
     free(dps->acquisition_info->principal_point_x_ae);
     free(dps->acquisition_info->principal_point_y_ae);
     free(dps->acquisition_info->translation_ae);
     free(dps->acquisition_info->depth_near_ae);
     free(dps->acquisition_info->depth_far_ae);
     free(dps->acquisition_info);
   }
   free (dps);
 }
#endif
...
int dps_is_equal(depth_parameter_set_rbsp_t *dps1, depth_parameter_set_rbsp_t *dps2)
{
  ...
  if ((!dps1->Valid) || (!dps2->Valid))
    return 0;

  equal &= (dps1->dep_parameter_set_id == dps2->dep_parameter_set_id);
  equal &= (dps1->num_views_minus1 == dps2->num_views_minus1);
  equal &= (dps1->acquisition_info == dps2->acquisition_info);
  return equal;
}
#endif
