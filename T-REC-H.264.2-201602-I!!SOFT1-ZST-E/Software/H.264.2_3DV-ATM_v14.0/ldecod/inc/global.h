...
#if EXT3D
extern BlockPos* PicPosText;
extern BlockPos* PicPosDepth;
#endif
...
#if EXT3D
  BiContextType mb_vsskip_flag_contexts [3][NUM_MB_TYPE_CTX];
  BiContextType vspflag_contexts [3][NUM_MB_TYPE_CTX];
#endif
...
#if EXT3D 
  BiContextType ALC_flag_ctx[NUM_MB_ALC_CTX];
  BiContextType ALCskip_flag_ctx[NUM_MB_ALC_CTX];
#endif
} MotionInfoContexts;
...
typedef struct Motion_info
{
  ...
  int ref_idx;
  int list_idx;
  int direction;//!<0:  inter, 1: inter-view, 2: view synthesis
  double match_error;

} MotionInfo;

typedef struct Motion_group
{
  MotionInfo* mv_info;
...
typedef struct macroblock
{
  ...
  char          skip_flag;
#if EXT3D
  char          vsskip_flag;        //!< explicit flag for view synthesis skip mode
  char          direct_type_flag;   //!< explicit flag for direct type
#endif
  short         DFDisableIdc;
  ...
  MotionGroup** direction_mv_group_l0;
  MotionGroup** direction_mv_group_l1;
  MotionGroup*  best_mv_group_l0;
  MotionGroup*  best_mv_group_l1;
  int           best_direction[3];

  int           num_of_valid_candidates[2];
  int           d_mvp_been_prepared;

  int           virtual_depth_flag;
  imgpel        virtual_depth_pred[MB_BLOCK_SIZE][MB_BLOCK_SIZE];

  int           block_based_vsp;//!<0: VSP is not necessary for current MB; 1: VSP is necessary for current MB; 2: VSP for current MB has been synthesized
  ...
  short         iDoNBDV[4][16/BLOCK_VSP_T][16/BLOCK_VSP_T];
#endif
...
#if EXT3D 
  int is_alc_mode;          //!< enables alc for macroblock
  int is_alc_flag_possible; //!< enables alc for current mbtype
  char (*readStdRefPictureIdx) (struct macroblock *currMB, struct syntaxelement *currSE, struct datapartition *dP, char b8mode, int list);

  void (*GetInterviewMVPrediction)(struct macroblock *currMB,MotionVector*pmv,int block_y,int block_x,
    int blockshape_y,int blockshape_x,int * disparity_table);

  void (*GetInterviewMVPredictionMax)(struct macroblock *currMB,MotionVector*pmv,int block_y,int block_x,
    int blockshape_y,int blockshape_x,int * disparity_table);
...
#if EXT3D
typedef struct nalunitheader3dvcext_tag
{
  unsigned int view_id;
...
typedef struct slice
{
...
#if EXT3D
  depth_parameter_set_rbsp_t *active_dps;
#endif
  ...
  int               AdaptiveLuminanceCompensation;  //!< enables alc
  Boolean           bIVRefExist;  //!< true when interview refernece frame exists
  int               idxInterviewRef[2]; //!< stores ref index of the first interview frame
  int               alc_slice_flag;       //!< enables alc for current slice

  int               RLESkipTex;         //<! enables RLESkip in texture

  Boolean           bVspRefExist;   //!< true, if there are VSP ref in L0 or L1;    false, otherwise    -Dong
  int               idxVspRef[2];   //!< the ref idx of VSP picture in L0, L1.    -Dong

  int               Harmonize_VSP_IVP;
...
#if EXT3D
  NALUnitHeader3dVCExt_t NaluHeader3dVCExt;

  int                    slice_header_pred_flag;

  int                    pre_slice_header_src;
  int                    pre_ref_lists_src;
  int                    pre_pred_weight_table_src;
  int                    pre_dec_ref_pic_marking_src;
...
typedef struct video_par
{
...
#if EXT3D
  depth_parameter_set_rbsp_t *active_dps;
  depth_parameter_set_rbsp_t *DepParSet[MAXDPS];
#endif
...
#if EXT3D
  seq_parameter_set_3dv_extension SeqParSet3DVExtension[MAXSPS];
  seq_parameter_set_3dv_extension*active_sps_3dv_extension;
#endif
...
#if EXT3D
  int                       base_view_frm_num;
  int                       enable_mvd_format;                  //!< data for depth map was also included in the bitstream 
  struct storable_picture  *p_dual_picture;
  int                       text_view_coding_order[MAX_CODEVIEW];
  int                       depth_view_coding_order[MAX_CODEVIEW];
  int                       camera_order[MAX_CODEVIEW];

  int                       ViewCompOrderDepthFlag[2*MAX_CODEVIEW];
  int                       ViewCompOrderVOIdx[2*MAX_CODEVIEW];

  int                       acquisition_idx;

  double                    dec_principal_point_x[2][MAX_CODEVIEW];
  double                    dec_focal_length_x[2][MAX_CODEVIEW];//!<0:texture 1:depth
  double                    dec_translation[MAX_CODEVIEW];
  double                    dec_depth_near[MAX_CODEVIEW];
  double                    dec_depth_far[MAX_CODEVIEW];

  FILE                     *dec_HHI_fast_vs_cam_file;

  int                       disparity_param_prec;
  int                   ****disparity_lut; //!< disparity look up table [source_view_idx][target_view_idx][0:quarter-pel, 1:half-pel][depth_value]
  
  int                      *forward_disparity_table;
  int                      *backward_disparity_table;


  int                       SliceHeaderPred;
  struct slice_header_dual_params **slice_header_dual;
  int                     **dec_view_flag;
  ...
  int                       SEI_DepthType[MAX_CODEVIEW];
  int                       SEI_NonlinearDepthNum[MAX_CODEVIEW];
  char                     *SEI_NonlinearDepthPoints[MAX_CODEVIEW];

  int                       low_res_depth;
  int                       width_ori;
  int                       height_ori;
  ResizeParameters         *p_depth_upsample_params;
  int                       NormalizeResolutionDepth;

  // @DT: Depth grid crop
  int                       depth_pic_width_in_mbs_minus1;
  int                       depth_pic_height_in_map_units_minus1;
  int                       depth_hor_mult;
  int                       depth_ver_mult;
  int                       depth_hor_rsh;
  int                       depth_ver_rsh;

  int                       depth_frame_cropping_flag;
  int                       depth_frame_crop_left_offset;
  int                       depth_frame_crop_right_offset;
  int                       depth_frame_crop_top_offset;
  int                       depth_frame_crop_bottom_offset;

  int                       DepthCropLeftCoord;
  int                       DepthCropRightCoord;
  int                       DepthCropTopCoord;
  int                       DepthCropBottomCoord;

  int                       grid_pos_num_views;
  int                       grid_pos_x[MAX_CODEVIEW+1];
  int                       grid_pos_y[MAX_CODEVIEW+1];

  int                       vs_ok[2][MAX_CODEVIEW];        //!<0: texture 1:depth
  int                       decode_ok[2][MAX_CODEVIEW];    //!<0: texture 1:depth

  struct storable_picture  *left_refer_pic;
  struct storable_picture  *right_refer_pic;
  struct storable_picture  *left_refer_depth;
  struct storable_picture  *right_refer_depth;
  imgpel                 ***VSP_MB_pred;
#endif
...
#if EXT3D
  struct snr_par       *snr_3dv[2][MAX_CODEVIEW];
  int                   num_of_views;
#else
  struct snr_par       *snr;
#endif
  ...
  Slice **ppSliceList;
#if EXT3D
  char  *text_intra_block;
  char  *text_intra_block_JV[MAX_PLANE];
  char  *depth_intra_block;
  char  *depth_intra_block_JV[MAX_PLANE];
#endif
  char  *intra_block;
  ...
  int text_width;
  int text_height;
  int text_width_cr;                               //!< width chroma  
  int text_height_cr;                              //!< height chroma
  int depth_width;
  int depth_height;
  int depth_width_cr;                               //!< width chroma  
  int depth_height_cr;   
#if ITRI_INTERLACE
  int is_texture_frame;
#endif
#endif
  int width;
  int height;
...
#if EXT3D
  byte    **text_ipredmode;                  //!< prediction type [90][74]
  byte    **text_ipredmode_JV[MAX_PLANE];
  byte  ****text_nz_coeff;
  int     **text_siblock;
  int     **text_siblock_JV[MAX_PLANE];
  byte    **depth_ipredmode;                  //!< prediction type [90][74]
  byte    **depth_ipredmode_JV[MAX_PLANE];
  byte  ****depth_nz_coeff;
  int     **depth_siblock;
  int     **depth_siblock_JV[MAX_PLANE];
#endif
...
#if EXT3D
  Macroblock *text_mb_data;               //!< array containing all MBs of a whole frame
  Macroblock *text_mb_data_JV[MAX_PLANE]; //!< mb_data to be used for 4:4:4 independent mode
  Macroblock *depth_mb_data;               //!< array containing all MBs of a whole frame
  Macroblock *depth_mb_data_JV[MAX_PLANE]; //!< mb_data to be used for 4:4:4 independent mode
#endif
...
#if EXT3D
  ResizeParameters *p_switch_upsample_params;
  int mixed_res;
#endif
...
#if EXT3D
  int p_out_3dv[2][MAX_CODEVIEW];     //!< file descriptor to output YUV file for 3DV
#else
...
#if EXT3D
  imgpel **text_imgY_ref;
  imgpel ***text_imgUV_ref;
  imgpel **depth_imgY_ref;
  imgpel ***depth_imgUV_ref;
#endif
...
#if EXT3D
  int *text_qp_per_matrix;
  int *text_qp_rem_matrix;
  int *depth_qp_per_matrix;
  int *depth_qp_rem_matrix;
#endif
  int *qp_per_matrix;
  int *qp_rem_matrix;
...
#if EXT3D
  struct storable_picture *no_reference_picture[2]; //!< dummy storable picture for recovery point
#else
  ...
  // dpb
#if EXT3D
  struct decoded_picture_buffer *p_Dpb[2];
  struct decoded_picture_buffer *p_Dpb_legacy[2]; // This is the old JM dpb method and will be removed at some point
#else
  struct decoded_picture_buffer *p_Dpb;
...
#if EXT3D // REF_DISP_SEI_A0163
  ReferenceDisplayInfoSEI      *seiReferenceDisplay;
  MultiviewAcquisitionInfoSEI  *seiMultiviewAcquisition;
  int32                         NumDisplays;
  int32                         NumSampleShiftPlus512[MAX_DISPLAYS];
  Boolean                       FileOpen;
  Boolean                       RefDispSEI;
  Boolean                       RefDispSEIUpdated;
  int32                         RefDispSEIPOC;
#endif
...
#if EXT3D
  DecodedPicList *pTextDecOuputPic;
  DecodedPicList *pDepthDecOuputPic;
#endif
...
typedef struct inp_par
{
...
#if EXT3D
  char                outfile[2][FILE_NAME_SIZE];                      //!< Decoded YUV 4:2:0 output
  VideoDataFile       InputFile[MAX_CODEVIEW];                         //!<a dummy for decoder
  char                ThreeDVRefFile[2][MAX_CODEVIEW][FILE_NAME_SIZE];  //!< Optional YUV 4:2:0 reference file for SNR measurement
  char                CameraFile[FILE_NAME_SIZE];
  int                 OutputYUVRecFile;
  int                 NormalizeResolutionDepth;
...
#if EXT3D 
  char                RefDispOutputFile[FILE_NAME_SIZE];
#endif
  ...
  int poc_scale;
#if EXT3D
  int write_uv[2];                   //!< 0:texture 1:depth
#else
  int write_uv;
#endif
  int silent;
...
#if EXT3D
extern int  init_text_global_buffers( VideoParameters *p_Vid );
extern void free_text_global_buffers( VideoParameters *p_Vid );
extern int  init_depth_global_buffers( VideoParameters *p_Vid );
extern void free_depth_global_buffers( VideoParameters *p_Vid );
extern void set_picture_global_buffers(VideoParameters *p_Vid);
#else
...
extern void nal_unit_header_3dvc_extension(NALUnitHeader3dVCExt_t *NaluHeader3dVCExt, struct bit_stream *bitstream);
#endif
...
#if EXT3D
extern void get_nbdv          ( Macroblock* currMB, short* disparity );
extern int  get_nbdv_temporal ( Macroblock* currMB, short* disparity );
extern int  get_nbdv_spatial  ( Macroblock* currMB, short* disparity );
#endif
