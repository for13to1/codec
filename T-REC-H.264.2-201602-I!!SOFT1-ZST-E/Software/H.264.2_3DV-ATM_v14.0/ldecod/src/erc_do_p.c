                          ...
                          int picSizeX, int picSizeY, char *yCondition)
{
  ...
  UNREFERENCED_PARAMETER(picSizeY);
...
#if EXT3D
      init_lists_for_non_reference_loss(p_Vid->p_Dpb[src->is_depth], dst->slice_type, p_Vid->ppSliceList[0]->structure);
#else
...
#if EXT3D
    store_picture_in_dpb(p_Vid->p_Dpb[picture->is_depth], picture);
#else
