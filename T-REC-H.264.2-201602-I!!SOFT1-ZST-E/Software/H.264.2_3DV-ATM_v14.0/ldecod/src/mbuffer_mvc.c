...
#if EXT3D
  picLX = get_short_term_pic(currSlice->p_Dpb, picNumLX,currViewID);
#else
...
/*#if PRINTREFLIST
  ...
  if((p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH
    ||p_Vid->profile_idc==ThreeDV_HIGH||p_Vid->profile_idc==ThreeDV_EXTEND_HIGH) && currSlice->current_slice_nr==0)
#else
  if((p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH) && currSlice->current_slice_nr==0)
#endif
  {
...
void reorder_ref_pic_list_mvc(Slice *currSlice, int cur_list, int **anchor_ref, int **non_anchor_ref, int view_id, int anchor_pic_flag, int currPOC, int listidx)
{
  ...
  {
#if EXT3D
    if (reordering_of_pic_nums_idc[i] > 6)
...
#if EXT3D
    if( currSlice->Harmonize_VSP_IVP && p_Vid->isTextureFirst )
    {
      int curr_voidx=GetVOIdx(p_Vid,currSlice->view_id);
      if(!p_Vid->vs_ok[currSlice->is_depth][curr_voidx])
        get_viewsyn_picture(currSlice);
    }
#endif
