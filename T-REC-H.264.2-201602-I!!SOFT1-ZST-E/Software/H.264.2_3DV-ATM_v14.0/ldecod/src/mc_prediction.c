...
************************************************************************
* \brief
*    block single list prediction with alc
************************************************************************
*/
static void mc_prediction_alc(imgpel **mb_pred, int ver_block_size, int hor_block_size, int ioff)
{
  PiLC_TMPelCorHist_2DArray( mb_pred, ioff, ver_block_size, hor_block_size);
  ALC_TurnOFF();
}
#endif
...
* \brief
*    Integer positions
*    Supports horizontal sizes more or less than MB_BLOCK_SIZE
************************************************************************
*/ 
static void get_block_00Ex(imgpel **block, imgpel *cur_img, int span, int hor_block_size, int ver_block_size)
{
  int j;
  // in order to support thin templates like 1-pel
  for (j = 0; j < ver_block_size; ++j)
  { 
    memcpy(block[j], cur_img, hor_block_size * sizeof(imgpel));
    cur_img += span;    
  }
}
#endif
  ...
  else
  {
    ...
    if (dx == 0 && dy == 0)
    {
      ...
      if( (1 == currMB->is_alc_mode))
        get_block_00Ex( block, &cur_imgY[y_pos][x_pos], curr_ref->iLumaStride, hor_block_size, ver_block_size);
      else
...
#if EXT3D
static void get_viewsyn_block(Macroblock* currMB,imgpel*** block, StorablePicture* curr_depth_pic,StorablePicture* ref_img_pic,int* disparity_table)
{
  VideoParameters* p_Vid=currMB->p_Vid;
  imgpel** depth_map=p_Vid->isTextureFirst ? NULL : curr_depth_pic->imgY;
  imgpel** ref_luma=ref_img_pic->imgY;
  imgpel** ref_chroma_U=ref_img_pic->imgUV[0];
  imgpel** ref_chroma_V=ref_img_pic->imgUV[1];
  imgpel*** syn_block=p_Vid->VSP_MB_pred;

  int luma_width_minus1=p_Vid->width-1;
  int chroma_width_minus1=p_Vid->width_cr-1;
  int start_x = currMB->pix_x,start_y = currMB->pix_y;
  int end_x = start_x+MB_BLOCK_SIZE,end_y=start_y+MB_BLOCK_SIZE;

  int pos0,pos1,pos2,pos3,pos4,pos5;
  int y,x;
  int disparity_mul4;
  int pos,dx,chroma_comp;
  ...
  {
    imgpel* ref_line_Y=ref_luma[y];
    imgpel* ref_line_U=ref_chroma_U[y>>1];
    imgpel* ref_line_V=ref_chroma_V[y>>1];
   
  for(x=start_x;x<end_x;++x)
    {
      if(p_Vid->isTextureFirst)
      {
        disparity_mul4 = currMB->iDoNBDV[3][(y%MB_BLOCK_SIZE)/BLOCK_VSP_T][(x%MB_BLOCK_SIZE)/BLOCK_VSP_T]; //find the disparity vector from iDoNBDV  
      }
      else
      {
        // int disparity_mul4=disparity_table[depth_map[y>>1][x>>1]];
        ...
        intY  = iClip3(p_Vid->DepthCropTopCoord, p_Vid->DepthCropBottomCoord,(y-y%BLOCK_VSP_T+grid_posy) *p_Vid->depth_ver_mult>>p_Vid->depth_ver_rsh);
        intX  = iClip3(p_Vid->DepthCropLeftCoord,p_Vid->DepthCropRightCoord, (x-x%BLOCK_VSP_T+grid_posx) *p_Vid->depth_hor_mult>>p_Vid->depth_hor_rsh);
        intYN = iClip3(p_Vid->DepthCropTopCoord, p_Vid->DepthCropBottomCoord,(y-y%BLOCK_VSP_T+grid_posy+BLOCK_VSP_T-1) *p_Vid->depth_ver_mult>>p_Vid->depth_ver_rsh);
        intXN = iClip3(p_Vid->DepthCropLeftCoord,p_Vid->DepthCropRightCoord, (x-x%BLOCK_VSP_T+grid_posx+BLOCK_VSP_T-1) *p_Vid->depth_hor_mult>>p_Vid->depth_hor_rsh);

        D1 = depth_map[intY][intX];                  
        D2 = depth_map[intY][intXN];  
        D3 = depth_map[intYN][intX];
        D4 = depth_map[intYN][intXN];
        disparity_mul4=disparity_table[max(D1,max(D2,max(D3,D4)))];
      }
      pos=(x<<2)+disparity_mul4;
      dx=pos&0x03;
      chroma_comp=((y-start_y)%2==0)&&((x-start_x)%2==0);

      pos=pos>>2;

      if(dx==0)
      {
        pos=iClip3(0,luma_width_minus1,pos);

        syn_block[0][y-start_y][x-start_x]=ref_line_Y[pos];
      }
      else
      {
        imgpel b;
        pos0=iClip3(0,luma_width_minus1,pos-2);
        pos1=iClip3(0,luma_width_minus1,pos-1);
        pos2=iClip3(0,luma_width_minus1,pos);
        pos3=iClip3(0,luma_width_minus1,pos+1);
        pos4=iClip3(0,luma_width_minus1,pos+2);
        pos5=iClip3(0,luma_width_minus1,pos+3);

        b=(imgpel)iClip3(0,255,(ref_line_Y[pos0]+ref_line_Y[pos5]
        -5*(ref_line_Y[pos1]+ref_line_Y[pos4])+
          20*(ref_line_Y[pos2]+ref_line_Y[pos3])+16)>>5);

        if(dx==1)
        {
          syn_block[0][y-start_y][x-start_x]=(b+ref_line_Y[pos2]+1)>>1;
        }
        else if(dx==2)
        {
          syn_block[0][y-start_y][x-start_x]=b;
        }
        else if(dx==3)
        {
          syn_block[0][y-start_y][x-start_x]=(b+ref_line_Y[pos3]+1)>>1;
        }
      }
      ...
      if(chroma_comp)
      {
        ...
        pos0=iClip3(0,chroma_width_minus1,pos);
        pos1=iClip3(0,chroma_width_minus1,pos+1);
        syn_block[1][(y-start_y)>>1][(x-start_x)>>1]=(imgpel)iClip3(0,255,(w0*ref_line_U[pos0]+w1*ref_line_U[pos1]+4)>>3);
        syn_block[2][(y-start_y)>>1][(x-start_x)>>1]=(imgpel)iClip3(0,255,(w0*ref_line_V[pos0]+w1*ref_line_V[pos1]+4)>>3);
      }
    }
  }
}
...
************************************************************************
* \brief
*    Set the motion vector predictor of direct mode
************************************************************************
*/
void set_mvp_direct_params(Macroblock* currMB, MotionVector *pmvl0, MotionVector *pmvl1, char *l0_rFrame, char *l1_rFrame)
{
  int        best_ref[2]      =  {-1,-1};
  ...
  best_ref[1]=currMB->best_ref[1];
  best_mv[1]=currMB->best_mvp[1];

  *pmvl0=best_mv[0];
  *pmvl1=best_mv[1];

  *l0_rFrame=(char)best_ref[0];
  *l1_rFrame=(char)best_ref[1];
...
#if EXT3D
  if (*l0_rFrame >=0)
    currMB->GetMVPredictor (currMB, mb, pmvl0, *l0_rFrame, mv_info, LIST_0, 0, 0, 16, 16, 0);

  if (*l1_rFrame >=0)
    currMB->GetMVPredictor (currMB, mb, pmvl1, *l1_rFrame, mv_info, LIST_1, 0, 0, 16, 16, 0);
#else
  if (*l0_rFrame >=0)
...
#if EXT3D
    if (currMB->mb_field && ((((p_Vid->active_pps->weighted_pred_flag)||(currSlice->is_depth&&currSlice->depth_range_wp))&&
      (type==P_SLICE|| type == SP_SLICE))||
      (((p_Vid->active_pps->weighted_bipred_idc==1)||(currSlice->is_depth&&currSlice->depth_range_wp)) && (type==B_SLICE))))
#else
...
static void perform_mc_single(Macroblock *currMB, ColorPlane pl, StorablePicture *dec_picture, int pred_dir, int i, int j, int block_size_x, int block_size_y)
{
  ...
  if( 1 == currMB->is_alc_mode)
  {
    vecTM_x = vec1_x - (MAX_TEMPLATE_SIZE << 2);
    vecTM_y = vec1_y - (MAX_TEMPLATE_SIZE << 2);

    if  ((vecTM_x >=0 && vecTM_y >=0) &&
        ((i4<<2)-MAX_TEMPLATE_SIZE >= 0 && (j4<<2)-MAX_TEMPLATE_SIZE >= 0) &&
        ((vecTM_x + (block_size_x + MAX_TEMPLATE_SIZE)*4) <= (list->size_x << 2) && (vecTM_y + (block_size_y + MAX_TEMPLATE_SIZE)*4) <= (list->size_y << 2))
        )
    {
        get_block_luma(  list, vecTM_x, vecTM_y,  
                block_size_x + MAX_TEMPLATE_SIZE, block_size_y + MAX_TEMPLATE_SIZE, ALC_RetTM_buf(),
                shift_x,maxold_x,maxold_y, ALC_RetTM_tempbuf(),max_imgpel_value,no_ref_value, currMB);
        ALC_GetTM(  ALC_RetTM_buf(), MAX_TEMPLATE_SIZE, MAX_TEMPLATE_SIZE, currSlice->p_Vid->dec_picture->imgY, currMB->pix_y + j*4, currMB->pix_x + i*4, 
  ...
  {
    mc_prediction_alc(&currSlice->mb_pred[pl][joff], block_size_y, block_size_x, ioff);
  }
#else
  mc_prediction(&currSlice->mb_pred[pl][joff], block_size_y, block_size_x, ioff, tmp_block_l0); 
#endif
...
#if EXT3D
  int weighted_bipred_idc = ((p_Vid->active_pps->weighted_bipred_idc)||(dec_picture->is_depth&&currSlice->depth_range_wp));
#else
...
#if EXT3D
  int l0_ref_idx  = (currMB->mb_field && ((weighted_bipred_idc == 1)||(dec_picture->is_depth&&currSlice->depth_range_wp))) ? 
    l0_refframe >> 1: l0_refframe;
  int l1_ref_idx  = (currMB->mb_field && ((weighted_bipred_idc == 1)||(dec_picture->is_depth&&currSlice->depth_range_wp))) ? 
    l1_refframe >> 1: l1_refframe;
#else
...
void perform_mc(Macroblock *currMB, ColorPlane pl, StorablePicture *dec_picture, int pred_dir, int i, int j, int block_size_x, int block_size_y)
{
...
#if EXT3D
  if(currMB->block_based_vsp==1)
  {
    VideoParameters* p_Vid = currMB->p_Vid;
    imgpel*** vsp_pred = p_Vid->VSP_MB_pred;
    get_viewsyn_block(currMB,vsp_pred,p_Vid->p_dual_picture,p_Vid->left_refer_pic?p_Vid->left_refer_pic:p_Vid->right_refer_pic,p_Vid->forward_disparity_table);
    currMB->block_based_vsp = 2;
  }
  
  if (pred_dir != 2)
  {
    if (currSlice->weighted_pred_flag||(currSlice->is_depth&&currSlice->depth_range_wp))
    {
  ...
  {
    if (currSlice->weighted_bipred_idc||(currSlice->is_depth&&currSlice->depth_range_wp))
