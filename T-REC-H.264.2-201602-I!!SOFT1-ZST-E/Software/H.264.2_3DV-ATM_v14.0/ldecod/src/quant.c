 ...
 ***********************************************************************
 */
void init_text_qp_process(VideoParameters *p_Vid)
{
  ...
  if (p_Vid->text_qp_per_matrix == NULL)
    if ((p_Vid->text_qp_per_matrix = (int*)malloc((MAX_QP + 1 +  bitdepth_qp_scale)*sizeof(int))) == NULL)
      no_mem_exit("init_qp_process: p_Vid->qp_per_matrix");

  if (p_Vid->text_qp_rem_matrix == NULL)
    if ((p_Vid->text_qp_rem_matrix = (int*)malloc((MAX_QP + 1 +  bitdepth_qp_scale)*sizeof(int))) == NULL)
      no_mem_exit("init_qp_process: p_Vid->qp_rem_matrix");

  for (i = 0; i < MAX_QP + bitdepth_qp_scale + 1; i++)
  {
    p_Vid->text_qp_per_matrix[i] = i / 6;
    p_Vid->text_qp_rem_matrix[i] = i % 6;
  }
}

void free_text_qp_matrices(VideoParameters *p_Vid)
{
  if (p_Vid->text_qp_per_matrix != NULL)
  {
    free (p_Vid->text_qp_per_matrix);
    p_Vid->text_qp_per_matrix = NULL;
  }

  if (p_Vid->text_qp_rem_matrix != NULL)
  {
    free (p_Vid->text_qp_rem_matrix);
    p_Vid->text_qp_rem_matrix = NULL;
  }
}
 ...
 ***********************************************************************
 */
void init_depth_qp_process(VideoParameters *p_Vid)
{
  int bitdepth_qp_scale = imax(p_Vid->bitdepth_luma_qp_scale,p_Vid->bitdepth_chroma_qp_scale);
  int i;
  ...
  // We should allocate memory outside of this process since maybe we will have a change of SPS 
  // and we may need to recreate these. Currently should only support same bitdepth
  if (p_Vid->depth_qp_per_matrix == NULL)
    if ((p_Vid->depth_qp_per_matrix = (int*)malloc((MAX_QP + 1 +  bitdepth_qp_scale)*sizeof(int))) == NULL)
      no_mem_exit("init_qp_process: p_Vid->qp_per_matrix");

  if (p_Vid->depth_qp_rem_matrix == NULL)
    if ((p_Vid->depth_qp_rem_matrix = (int*)malloc((MAX_QP + 1 +  bitdepth_qp_scale)*sizeof(int))) == NULL)
      no_mem_exit("init_qp_process: p_Vid->qp_rem_matrix");

  for (i = 0; i < MAX_QP + bitdepth_qp_scale + 1; i++)
  {
    p_Vid->depth_qp_per_matrix[i] = i / 6;
    p_Vid->depth_qp_rem_matrix[i] = i % 6;
  }
}

void free_depth_qp_matrices(VideoParameters *p_Vid)
{
  if (p_Vid->depth_qp_per_matrix != NULL)
  {
    free (p_Vid->depth_qp_per_matrix);
    p_Vid->depth_qp_per_matrix = NULL;
  }

  if (p_Vid->depth_qp_rem_matrix != NULL)
  {
    free (p_Vid->depth_qp_rem_matrix);
    p_Vid->depth_qp_rem_matrix = NULL;
  }
}
#endif
