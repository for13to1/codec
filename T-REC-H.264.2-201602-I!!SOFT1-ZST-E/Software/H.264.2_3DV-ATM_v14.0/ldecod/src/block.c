...
************************************************************************
* \brief
*    Performs iDCT4x4 for one 4x4 block of currMB
************************************************************************
*/
void iMBtrans4x4Ex(Macroblock *currMB, ColorPlane pl, int smb, int Block4x4_Y, int Block4x4_X)
{
  Slice *currSlice = currMB->p_Slice;
  ...
  if (currMB->is_lossless && currMB->mb_type == I16MB)
  {
    printf("iMBtrans4x4Ex: currMB->is_lossless or currMB->mb_type == I16MB! Is not supported!\n");
    return;
  }
  else 
  if (smb || currMB->is_lossless == TRUE)
  {
    printf("iMBtrans4x4Ex: smb or currMB->is_lossless! Is not supported!\n");
    return;
  }
  else
  {
    ...
    inverse4x4(cof, mb_rres, Block4x4_Y, Block4x4_X);
    sample_reconstruct (&(currSlice->mb_rec[pl][Block4x4_Y]), &(currSlice->mb_pred[pl][Block4x4_Y]), &(mb_rres[Block4x4_Y]), Block4x4_X, Block4x4_X, 4, 4, currMB->p_Vid->max_pel_value_comp[pl], DQ_BITS);
    // construct picture from 4x4 blocks
    copy_image_data_4x4(&curr_img[currMB->pix_y + Block4x4_Y],&currSlice->mb_rec[pl][Block4x4_Y], currMB->pix_x + Block4x4_X, Block4x4_X);      
  } 
}
#endif
...
************************************************************************
* \brief
*    This function is almost similar to iMBtrans8x8 but it optionally makes transform for blocks identified by :
*    Blk00_TRNSF_DO Blk01_TRNSF_DO
*    Blk10_TRNSF_DO Blk11_TRNSF_DO
************************************************************************
*/
void iMBtrans8x8Ex(Macroblock *currMB, ColorPlane pl, Boolean Blk00_TRNSF_DO, Boolean Blk01_TRNSF_DO, Boolean Blk10_TRNSF_DO, Boolean Blk11_TRNSF_DO)
{
  StorablePicture *dec_picture = currMB->p_Slice->dec_picture;
  imgpel **curr_img = pl ? dec_picture->imgUV[pl - 1]: dec_picture->imgY;
  ...
  {
  itrans8x8(currMB, pl, 0, 0);
  copy_image_data_8x8(&curr_img[currMB->pix_y], &(currMB->p_Slice->mb_rec[pl][0]), currMB->pix_x, 0);
  }

  if (Blk01_TRNSF_DO)
  {
  itrans8x8(currMB, pl, 8, 0);
  copy_image_data_8x8(&curr_img[currMB->pix_y], &(currMB->p_Slice->mb_rec[pl][0]), currMB->pix_x + 8, 8);
  }
  if (Blk10_TRNSF_DO)
  {
  itrans8x8(currMB, pl, 0, 8);
  copy_image_data_8x8(&curr_img[currMB->pix_y + 8], &(currMB->p_Slice->mb_rec[pl][8]), currMB->pix_x, 0);
  }
  if (Blk11_TRNSF_DO)
  {
  itrans8x8(currMB, pl, 8, 8);
  copy_image_data_8x8(&curr_img[currMB->pix_y + 8], &(currMB->p_Slice->mb_rec[pl][8]), currMB->pix_x + 8, 8);
  }
}
...
************************************************************************
* \brief
*    Performs reconstruct for choosed 8x8 luma blocks of currMB if it is nedeed
************************************************************************
*/
void iTransformExLuma8(Macroblock *currMB, ColorPlane pl, int smb, Boolean Blk00_TRNSF_DO, Boolean Blk01_TRNSF_DO, Boolean Blk10_TRNSF_DO, Boolean Blk11_TRNSF_DO)
{
  Slice *currSlice = currMB->p_Slice;
  ...
  if (pl)
  {
  printf("iTransformExLuma8: incorrect index of color plane!\n");
    return;
  }
  if ((currMB->cbp & 15) != 0 || smb)
  {
    if(currMB->luma_transform_size_8x8_flag) // 8x8 inverse transform
    {
      iMBtrans8x8Ex(currMB, pl, Blk00_TRNSF_DO, Blk01_TRNSF_DO, Blk10_TRNSF_DO, Blk11_TRNSF_DO);    
    }
    else
    {
      printf("iTransformExLuma8: incorrect luma_transform_size_8x8_flag!\n");
      return;
    }
  }
  else
  {
  if ( currMB->luma_transform_size_8x8_flag || (0 == (currMB->cbp & 15)) ) // 8x8 inverse transform
  {
    curr_img = dec_picture->imgY;
    // Perform 8x8 idct
    if (Blk00_TRNSF_DO)    
    copy_image_data_8x8(&curr_img[currMB->pix_y], &(currSlice->mb_pred[pl][0]), currMB->pix_x, 0);      
    if (Blk01_TRNSF_DO)    
    copy_image_data_8x8(&curr_img[currMB->pix_y], &(currSlice->mb_pred[pl][0]), currMB->pix_x + 8, 8);      
    if (Blk10_TRNSF_DO)    
    copy_image_data_8x8(&curr_img[currMB->pix_y + 8], &(currSlice->mb_pred[pl][8]), currMB->pix_x, 0);      
    if (Blk11_TRNSF_DO)
    copy_image_data_8x8(&curr_img[currMB->pix_y + 8], &(currSlice->mb_pred[pl][8]), currMB->pix_x + 8, 8);
    }
  }
}
...
************************************************************************
* \brief
*    Performs reconstruct for choosed 4x4 luma block of currMB if it is nedeed
************************************************************************
*/
void iTransformExLuma4(Macroblock *currMB, ColorPlane pl, int smb, int Blk4x4_PosY, int Blk4x4_PosX)
{
  Slice *currSlice = currMB->p_Slice;
  StorablePicture *dec_picture = currSlice->dec_picture;
  imgpel **curr_img;
  if (pl)
  {
    printf("iTransformExLuma4: incorrect index of color plane!\n");
    return;
  }
  else if ((currMB->cbp & 15) != 0 || smb)
  {
    if(currMB->luma_transform_size_8x8_flag == 0) // 4x4 inverse transform
    {
      //iMBtrans4x4(currMB, pl, smb);
      iMBtrans4x4Ex(currMB, pl, smb, Blk4x4_PosY, Blk4x4_PosX);
    }
    else // 8x8 inverse transform
    {
      printf("iTransformExLuma4: incorrect luma_transform_size_8x8_flag!\n");
      return;
    }
  }
  else
  {
    curr_img = dec_picture->imgY;
    // construct picture from 4x4 blocks
    copy_image_data_4x4(&curr_img[currMB->pix_y + Blk4x4_PosY],&(currSlice->mb_pred[pl][Blk4x4_PosY]), currMB->pix_x + Blk4x4_PosX, Blk4x4_PosX);          
   }
}
...
************************************************************************
* \brief
*    Performs reconstruct for chroma
************************************************************************
*/
void iTransformExChroma(Macroblock *currMB, ColorPlane pl, int smb)
{
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  StorablePicture *dec_picture = currSlice->dec_picture;
