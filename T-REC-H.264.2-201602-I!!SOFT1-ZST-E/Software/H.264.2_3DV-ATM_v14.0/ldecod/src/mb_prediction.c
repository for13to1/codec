...
int mb_pred_skip(Macroblock *currMB, ColorPlane curr_plane, imgpel **currImg, StorablePicture *dec_picture)
{
...
#if EXT3D
  if (1 == currSlice->alc_slice_flag && currMB->is_alc_mode == 1)
  {
    int idxInterviewRef = currSlice->idxInterviewRef[LIST_0];
    PicMotionParams *mv_info = &(currSlice->dec_picture->mv_info[currMB->block_y][currMB->block_x]);
    if( currSlice->bIVRefExist && idxInterviewRef == mv_info->ref_idx[LIST_0])
    {
  ...
  {
    if (currMB->vsskip_flag || currMB->direct_type_flag)
    {
      int i,j;
      
      //if (currMB->direct_type_flag) // never happen
      //  printf("y");
      ...
      for(j=0;j<16;j+=BLOCK_VSP_T)
      {
        for(i=0;i<16;i+=BLOCK_VSP_T)
        {
          PicMotionParams *mv; 
          int k,l;
          perform_mc(currMB, curr_plane, dec_picture, LIST_0, i/BLOCK_SIZE, j/BLOCK_SIZE, BLOCK_VSP_T, BLOCK_VSP_T);
          for(k=j;k<j+BLOCK_VSP_T;k+=4)
          {
            for(l=i;l<i+BLOCK_VSP_T;l+=4)
            {
              mv = &dec_picture->mv_info[currMB->block_y+k/BLOCK_SIZE][currMB->block_x+l/BLOCK_SIZE];
...
int mb_pred_p_inter8x8(Macroblock *currMB, ColorPlane curr_plane, StorablePicture *dec_picture)
{
  ...
  {
#if EXT3D
    int mv_mode  = ( currMB->mb_type != P8x8 && currMB->p_Vid->isTextureFirst ) ? SMB8x8 : currMB->b8mode[block8x8];
#else
  ...
  if (currMB->is_alc_mode)
  {
    for (k = k_start; k < k_end; k += k_inc)
    {
      ...
      // MV 8x8:
       //similar to the encoder block by block:
        if(mv_mode == SMB8x8)
        {
          perform_mc(currMB, curr_plane, dec_picture, pred_dir, i, j, BLOCK_SIZE_8x8, BLOCK_SIZE_8x8);
          if (currMB->luma_transform_size_8x8_flag)
          {
            if ( 0== i && 0 == j)
              iTransformExLuma8(currMB, curr_plane, smb, TRUE, FALSE, FALSE, FALSE);
            else if ( 2== i && 0 == j)
              iTransformExLuma8(currMB, curr_plane, smb, FALSE, TRUE, FALSE, FALSE);
            else if ( 0== i && 2 == j)
              iTransformExLuma8(currMB, curr_plane, smb, FALSE, FALSE, TRUE, FALSE);
            else
              iTransformExLuma8(currMB, curr_plane, smb, FALSE, FALSE, FALSE, TRUE);
          }
          else
          {
            for (y =j * 4; y < j * 4 + block_size_y; y+=4)
              for (x =i * 4; x < i * 4 + block_size_x; x+=4)
                iTransformExLuma4(currMB, curr_plane, smb, y, x);  
          }
        }
        else
        {
          for (y =j * 4; y < j * 4 + block_size_y; y+=4)
            for (x =i * 4; x < i * 4 + block_size_x; x+=4)
            {
              perform_mc(currMB, curr_plane, dec_picture, pred_dir, (x>>2), (y>>2), 4, 4);
              iTransformExLuma4(currMB, curr_plane, smb, y, x);    
            }
        }
      }
  ...
  if (0 == currMB->is_alc_mode)
    ...
    iTransformExChroma(currMB, curr_plane, smb); 
#else
  iTransform(currMB, curr_plane, smb); 
#endif
...
int mb_pred_p_inter16x8(Macroblock *currMB, ColorPlane curr_plane, StorablePicture *dec_picture)
{
  ...
  if (currMB->is_alc_mode)
  {
    ...
    else
    {
      for (i =0; i < 8; i+=4)
        for (j = 0; j < 16; j+=4)
          iTransformExLuma4(currMB, curr_plane, smb, i, j);
    }
...
int mb_pred_b_d8x8spatial(Macroblock *currMB, ColorPlane curr_plane, imgpel **currImg, StorablePicture *dec_picture)
{
...
#if EXT3D
  MotionVector pmvl0_skip,pmvl1_skip;
  ...
  if (currMB->vsskip_flag || currMB->direct_type_flag)
  {
    // Set the ref idx to the vsp
    l0_rFrame = (char)currSlice->idxVspRef[0];
    l1_rFrame = (char)currSlice->idxVspRef[1];

    for(j4=0;j4<16;j4+=BLOCK_VSP_T)
    {
      for(i4=0;i4<16;i4+=BLOCK_VSP_T)
      {
        PicMotionParams *mv; 
        int k,l;
        if(l0_rFrame<0)
        ...
        {
          currMB->GetInterviewMVPredictionMax(currMB,&(disparity_mv),i4,j4,BLOCK_VSP_T,BLOCK_VSP_T,p_Vid->forward_disparity_table);

          if(p_Vid->isTextureFirst)
          {
            pmvl0_skip.mv_x = currMB->iDoNBDV[3][j4/BLOCK_VSP_T][i4/BLOCK_VSP_T];
            pmvl0_skip.mv_y = 0;
          }
          else
            pmvl0_skip=disparity_mv;
        }
        if(l1_rFrame<0)
        {
          pmvl1_skip = zero_mv;
        ...
        {
          currMB->GetInterviewMVPredictionMax(currMB,&(disparity_mv),i4,j4,BLOCK_VSP_T,BLOCK_VSP_T,p_Vid->backward_disparity_table);

          if(p_Vid->isTextureFirst)
          {
            pmvl1_skip.mv_x = currMB->iDoNBDV[3][j4/BLOCK_VSP_T][i4/BLOCK_VSP_T];
            pmvl1_skip.mv_y = 0;
          }
          else
            pmvl1_skip=disparity_mv;
        }
        for(k=j4;k<j4+BLOCK_VSP_T;k+=4)
        {
          for(l=i4;l<i4+BLOCK_VSP_T;l+=4)
          {
            mv = &dec_picture->mv_info[currMB->block_y+k/BLOCK_SIZE][currMB->block_x+l/BLOCK_SIZE];
            mv->mv[LIST_0] = pmvl0_skip;
            mv->mv[LIST_1] = pmvl1_skip;
            mv->ref_pic[LIST_0] = (l0_rFrame>=0) ? currSlice->listX[LIST_0][(int)l0_rFrame]:NULL;
            mv->ref_pic[LIST_1] = (l1_rFrame>=0) ? currSlice->listX[LIST_1][(int)l1_rFrame]:NULL;
            mv->ref_idx[LIST_0] = l0_rFrame;
            mv->ref_idx[LIST_1] = l1_rFrame;
          }
        }

      }
    }
    pred_dir=(l0_rFrame>=0)&&(l1_rFrame>=0)?2:((l0_rFrame>=0)?0:1);
    
//    if ( currMB->vsskip_flag && pred_dir == 2 )
//      printf("u");
    ...
    for(j4=0;j4<16;j4+=BLOCK_VSP_T)
    {
      for(i4=0;i4<16;i4+=BLOCK_VSP_T)
      {
        perform_mc(currMB, curr_plane, dec_picture, pred_dir, i4/BLOCK_SIZE, j4/BLOCK_SIZE, BLOCK_VSP_T, BLOCK_VSP_T);
      }
    }
    for (j4 = currMB->block_y; j4 < currMB->block_y + BLOCK_MULTIPLE; j4 ++)
    {
      for (i4 = currMB->block_x; i4 < currMB->block_x + BLOCK_MULTIPLE; i4 ++)
      {
        mv_info = &dec_picture->mv_info[j4][i4];
    ...
    flag = 0; //goto vsp_point; // @DT: Remove goto!!
  }
  else
  {
    if((currSlice->is_depth==0)&&(currSlice->depth_based_mvp_flag))
    {
      ...
      set_mvp_direct_params(currMB, &pmvl0, &pmvl1, &l0_rFrame, &l1_rFrame);
    }
    else
