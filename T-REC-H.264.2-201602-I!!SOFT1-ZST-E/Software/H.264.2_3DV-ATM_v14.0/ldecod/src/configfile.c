...
{
#if EXT3D
  fprintf( stderr, "\n   ldecod [-h] [-d defdec_texture.cfg -depd defdec_depth.cfg]" 
    "{[-f curenc1_texture.cfg] [-depf curenc1_depth.cfg]...[-f curencN_texture.cfg][-depf curencN_depth.cfg]}"
    " {[-p EncParam1=EncValue1]..[-p EncParamM=EncValueM]}"
    "{[-depp EncParam1=EncValue1]..[-depp EncParamM=EncValueM]}\n\n"
    "## Parameters\n\n"

    "## Options\n"
    "   -h :  prints function usage\n"
    "   -d :  use <defdec.cfg> as default file for texture parameter initializations.\n"
    "         If not used then file defaults to encoder.cfg in local directory.\n"
    "   -depd :  use <defdec.cfg> as default file for depth parameter initializations.\n"
    "         If not used then file defaults to encoder.cfg in local directory.\n"
    "   -f :  read <curencM.cfg> for reseting selected decoder parameters for texture.\n"
    "         Multiple files could be used that set different parameters\n"
    "   -depf :  read <curencM.cfg> for reseting selected decoder parameters for depth.\n"
    "         Multiple files could be used that set different parameters\n"
    "   -p :  Set parameter <DecParamM> to <DecValueM>.\n"
    "         See default decoder_texture.cfg file for description of texture parameters for texture.\n\n"
    "   -depp :  Set parameter <DecParamM> to <DecValueM>.\n"
    "         See default decoder_depth.cfg file for description of all parameters for depth.\n\n"

    "## Examples of usage:\n"
    "   ldecod\n"
    "   ldecod  -h\n"
    "   ldecod  -d default_texture.cfg\n"
    "   ldecod  -d default_texture.cfg -depd default_depth.cfg\n"
    "   ldecod  -f curenc1_texture.cfg\n"
    "   ldecod  -f curenc1_texture.cfg depf currenc1_depth.cfg\n"
...
void ParseCommand(InputParameters *p_Inp, int ac, char *av[])
{
  ...
  if(filename)
  {
    ...
    if (NULL != content)
    {
      //error (errortext, 300);
#if EXT3D
      ParseContent (p_Inp, Map, content, (int) strlen(content),0);
#else
