...
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb)
{
  ...
  int num_of_views=(p_Vid->active_subset_sps)&&(p_Vid->active_subset_sps->Valid)? p_Vid->active_subset_sps->num_views_minus1+1:1;
  ...
  if(p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH
    ||p_Vid->profile_idc==ThreeDV_HIGH||p_Vid->profile_idc==ThreeDV_EXTEND_HIGH)
    p_Dpb->size= GetMaxDecFrameBuffering(p_Vid) ;
  else
    p_Dpb->size = getDpbSize(active_sps);

  if(active_sps->profile_idc == MVC_HIGH || active_sps->profile_idc == STEREO_HIGH
    ||p_Vid->profile_idc==ThreeDV_HIGH||p_Vid->profile_idc==ThreeDV_EXTEND_HIGH)
    p_Dpb->size = (p_Dpb->size)*(num_of_views) + 2;
  ...
  for (i = 0; i < p_Dpb->size; i++)
  {
    ...
    p_Dpb->fs[i]->p_out =NULL;
  }

  /* allocate a dummy storable picture */

  p_Vid->no_reference_picture[is_depth] = alloc_storable_picture (p_Vid, FRAME, p_Vid->width, p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);
  ...
  if((p_Vid->conceal_mode !=0)&&(NULL==p_Vid->last_out_fs))
...
void re_init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb)
{
  ...
  assert(!active_sps->is_depth);

  iDpbSize = getDpbSize(active_sps);

  if(active_sps->profile_idc == MVC_HIGH || active_sps->profile_idc == STEREO_HIGH
  ||active_sps->profile_idc==ThreeDV_HIGH||active_sps->profile_idc==ThreeDV_EXTEND_HIGH)
    iDpbSize = iDpbSize*(num_of_views) + 2;
  p_Dpb->num_ref_frames = active_sps->num_ref_frames;
    ...
    for (i=0; i<p_Dpb->ref_frames_in_buffer; i++)
    {
      if (p_Dpb->fs_ref[i]->is_used==3)
      {
        ...
        if ((p_Dpb->fs_ref[i]->frame->used_for_reference)&&
          (p_Dpb->fs_ref[i]->frame->view_id==currSlice->view_id)&&(!p_Dpb->fs_ref[i]->frame->is_long_term))
#else
...
#if EXT3D
  if ((currSlice->svc_extension_flag==0)||(currSlice->nal_unit_type== NALU_TYPE_3DV_EXT))
#else
...
void init_mbaff_lists(VideoParameters *p_Vid, Slice *currSlice)
{
  ...
  for (i=2;i<6;i++)
  {
    ...
    {
#if EXT3D
      currSlice->listX[i][j] = p_Vid->no_reference_picture[currSlice->is_depth];
#else
 ...
 */
#if EXT3D
StorablePicture*  get_short_term_pic(DecodedPictureBuffer *p_Dpb, int picNum,int view_id)
#else
...
#if EXT3D
          &&(p_Dpb->fs_ref[i]->frame->view_id==view_id) // @DT: Newly enclosed by 3D macro
#endif
           )
...
#if EXT3D
  return p_Vid->no_reference_picture[is_depth];
#else
...
void reorder_ref_pic_list(Slice *currSlice, int cur_list)
{
  ...
  for (i=0; reordering_of_pic_nums_idc[i]!=3; i++)
  {
...
#if EXT3D
      reorder_short_term(currSlice, cur_list, num_ref_idx_lX_active_minus1, picNumLX, &refIdxLX, currSlice->view_id);
#else
#if MVC_EXTENSION_ENABLE
    ...
    {
#if EXT3D
      reorder_long_term(currSlice, currSlice->listX[cur_list], num_ref_idx_lX_active_minus1, long_term_pic_idx[i], &refIdxLX, currSlice->view_id);
#else
#if MVC_EXTENSION_ENABLE
...
static void idr_memory_management(DecodedPictureBuffer *p_Dpb, StorablePicture* p)
{
  ...
  if (p->no_output_of_prior_pics_flag)
  {
...
#if EXT3D
      int threeDV=(svc_extension_flag==0)||(p_Vid->ppSliceList[0]->nal_unit_type== NALU_TYPE_3DV_EXT);
      if (threeDV || p_Dpb->fs[i]->view_id == p->view_id)
      {
        free_frame_store(p_Dpb->fs[i]);
        p_Dpb->fs[i] = alloc_frame_store();
        size++;
...
#if EXT3D
      int threeDV=(svc_extension_flag==0)||(p_Vid->ppSliceList[0]->nal_unit_type== NALU_TYPE_3DV_EXT);
      if(threeDV|| p_Dpb->fs_ref[i]->view_id == p->view_id)
    ...
    {
#if EXT3D
      int threeDV=(svc_extension_flag==0)||((p_Vid->ppSliceList[0]->svc_extension_flag==1)&&(p->is_depth));
      if (threeDV || p_Dpb->fs_ltref[i]->view_id == p->view_id)
#elif (MVC_EXTENSION_ENABLE)
...
#if EXT3D
    if(p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH
      ||p_Vid->profile_idc==ThreeDV_HIGH||p_Vid->profile_idc==ThreeDV_EXTEND_HIGH) //if (svc_extension_flag == 0)
      flush_dpb(p_Dpb, -1);
    else
...
static void sliding_window_memory_management(DecodedPictureBuffer *p_Dpb, StorablePicture* p)
{
  ...
  if (p_Dpb->ref_frames_in_buffer==p_Dpb->num_ref_frames - p_Dpb->ltref_frames_in_buffer)
  {
    ...
    if ( p_Dpb->p_Vid->profile_idc == MVC_HIGH || p_Dpb->p_Vid->profile_idc == STEREO_HIGH
      || p_Dpb->p_Vid->profile_idc == ThreeDV_HIGH||p_Dpb->p_Vid->profile_idc==ThreeDV_EXTEND_HIGH)
#else
    if ( p_Dpb->p_Vid->profile_idc == MVC_HIGH || p_Dpb->p_Vid->profile_idc == STEREO_HIGH )
#endif
    {
...
#if EXT3D
    write_lost_non_ref_pic(p_Dpb, poc, *(p_Dpb->fs[pos]->p_out));
#else
...
#if EXT3D
  write_stored_frame(p_Vid, p_Dpb->fs[pos], *(p_Dpb->fs[pos]->p_out));
#else
...
void dpb_combine_field_yuv(VideoParameters *p_Vid, FrameStore *fs)
{
  ...
  fs->frame->SEI_NonlinearDepthNum=fs->top_field->SEI_NonlinearDepthNum;
  memcpy(fs->frame->SEI_NonlinearDepthPoints, fs->top_field->SEI_NonlinearDepthPoints, sizeof(fs->top_field->SEI_NonlinearDepthPoints));
#endif
#endif
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
    if ((currSlice->abs_diff_view_idx_minus1[LIST_0] = calloc(MAX_VIEWREFERENCE+1,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: abs_diff_view_idx_minus1_l0");
#else
...
#if MVC_EXTENSION_ENABLE||EXT3D
#if EXT3D
    if ((currSlice->abs_diff_view_idx_minus1[LIST_1] = calloc(MAX_VIEWREFERENCE+1,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: abs_diff_view_idx_minus1_l1");
#else
...
************************************************************************
* \brief
*    get the new mvc update information for this NAL(if needed)
*
************************************************************************
*/
static void set_disparity_lut(VideoParameters* p_Vid, ThreeDVAcquisitionInfo* curr_3dv_acquisition_info, int num_of_views)
{
  int source_voidx, target_voidx;

  int depth_range=p_Vid->max_depth_map_value;
  int log2_disp_prec_lut=2;
  int log2_div_luma   = p_Vid->bitdepth_luma + p_Vid->disparity_param_prec + 1 - log2_disp_prec_lut;
  int log2_div_chroma = log2_div_luma + 1;

  int i_temp_scale, i_temp_disp, i_disp_luma, i_disp_chroma;
  int j,jn;
  //FILE *p=fopen("disp_table_dec.txt","a+");
  ...
  for (source_voidx=0; source_voidx<num_of_views; source_voidx++)
  {
    for (target_voidx=0; target_voidx<num_of_views; target_voidx++)
    {
      for( j = 0; j < depth_range; j++ )
      {
        ...
        p_Vid->disparity_lut[source_voidx][target_voidx][ 0 ][ j ] = i_disp_luma;
        p_Vid->disparity_lut[source_voidx][target_voidx][ 1 ][ j ] = i_disp_chroma;
...
void get_camera_depth_range_info(VideoParameters* p_Vid)
{
  depth_parameter_set_rbsp_t *dps = p_Vid->active_dps;
  ThreeDVAcquisitionInfo* curr_3dv_acquisition_info;
  int num_of_views;
  int k=0;
  int view_id=0;
  int voidx=0;
  ...
  if(dps!=NULL)
  {
    curr_3dv_acquisition_info=dps->acquisition_info;
    num_of_views=dps->num_views_minus1+1;
  }
  else
  {
    curr_3dv_acquisition_info=p_Vid->DepParSet[0]->acquisition_info;
    num_of_views=p_Vid->num_of_views;
  }

  for(k=0;k<num_of_views;++k)
  {
    view_id=p_Vid->camera_order[k];
    voidx=GetVOIdx(p_Vid,view_id);
    p_Vid->dec_depth_near[voidx]=curr_3dv_acquisition_info->depth_near_ae->rec[k];
    p_Vid->dec_depth_far[voidx]=curr_3dv_acquisition_info->depth_far_ae->rec[k];
  }

  set_disparity_lut(p_Vid, curr_3dv_acquisition_info, num_of_views);
}

void get_disparity_table(VideoParameters* p_Vid)
{
  subset_seq_parameter_set_rbsp_t* subset_sps=p_Vid->active_subset_sps;
  int view_id=p_Vid->dec_picture->view_id;
  int curr_voidx=GetVOIdx(p_Vid,view_id);
  int ref_view_id=0;
  int ref_voidx=0;
  ...
  // The acquisition info is every updated at acquisition idx==2
  if(dps!=NULL)
  {
    curr_3dv_acquisition_info=dps->acquisition_info;
    num_of_views=dps->num_views_minus1+1;
  ...
  if (p_Vid->dec_picture->anchor_pic_flag)
  {
    if(subset_sps->num_anchor_refs_l0[curr_voidx])
    {
      ref_view_id=subset_sps->anchor_ref_l0[curr_voidx][0];
      ref_voidx=GetVOIdx(p_Vid,ref_view_id);
      p_Vid->forward_disparity_table=p_Vid->disparity_lut[curr_voidx][ref_voidx][0];
    }
    if(subset_sps->num_anchor_refs_l1[curr_voidx])
    {
      ref_view_id=subset_sps->anchor_ref_l1[curr_voidx][0];
      ref_voidx=GetVOIdx(p_Vid,ref_view_id);
      p_Vid->backward_disparity_table=p_Vid->disparity_lut[curr_voidx][ref_voidx][0];
    }
  }
  else
  {
    if(subset_sps->num_non_anchor_refs_l0[curr_voidx])
    {
      ref_view_id=subset_sps->non_anchor_ref_l0[curr_voidx][0];
      ref_voidx=GetVOIdx(p_Vid,ref_view_id);
      p_Vid->forward_disparity_table=p_Vid->disparity_lut[curr_voidx][ref_voidx][0];
    }
    if(subset_sps->num_non_anchor_refs_l1[curr_voidx])
    {
      ref_view_id=subset_sps->non_anchor_ref_l1[curr_voidx][0];
      ref_voidx=GetVOIdx(p_Vid,ref_view_id);
      p_Vid->backward_disparity_table=p_Vid->disparity_lut[curr_voidx][ref_voidx][0];
    }
  }
