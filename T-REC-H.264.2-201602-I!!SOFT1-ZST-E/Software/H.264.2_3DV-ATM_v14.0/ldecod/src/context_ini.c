...
#if EXT3D 
#define PBIARI_CTX_INIT2_1(ii,jj,ctx,tab,qp) \
{ \
  for (i=0; i<ii; ++i) \
  for (j=0; j<jj; ++j) \
  { \
  biari_init_context (qp, &(ctx[i][j]), tab[i][j]); \
  } \
}
#endif
...
#if EXT3D 
#define PBIARI_CTX_INIT1_1(jj,ctx,tab,qp) \
{ \
  { \
  for (j=0; j<jj; ++j) \
  { \
  biari_init_context ( qp, &(ctx[j]), &(tab[j][0]) ); \
  } \
  } \
}
#endif
...
#if EXT3D
    PBIARI_CTX_INIT2 (3, NUM_MB_TYPE_CTX,   mc->mb_vsskip_flag_contexts, INIT_VS_SKIP,    model_number, qp);
    PBIARI_CTX_INIT2 (3, NUM_MB_TYPE_CTX,   mc->vspflag_contexts,     INIT_MB_TYPE,    model_number, qp);
#endif
...
#if EXT3D 
    PBIARI_CTX_INIT1_1(NUM_MB_ALC_CTX, mc->ALC_flag_ctx, INIT_MB_ALC_FLAG_CTX, qp);
    PBIARI_CTX_INIT1_1(NUM_MB_ALC_CTX, mc->ALCskip_flag_ctx, INIT_MB_ALCSKIP_FLAG_CTX, qp);
#endif
  }
}
